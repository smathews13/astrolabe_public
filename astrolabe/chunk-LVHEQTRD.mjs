...
function buildTiles(ids, rows, warehouseAttribution = EMPTY_WAREHOUSE_QUERY_ATTRIBUTION, resourceActivity = [], genie, genieReason = "") {
...
function genieAccountingTiles(ids, accounting, reason) {
  ...
  const tiles = period.instances.map((instance) => {
    ...
    return {
      ...
      remedy: "",
      note: instance.attribution === "query-history-allocation" ? "Allocated by user-day Query History execution share." : "Matched by user-day Query History to this configured space.",
      evidence: { billingRows: null, astrolabeQueries: null },
...
function buildQuestionAttribution(runs, tiles, limit) {
  ...
  const attributed = newest.map((run) => {
    const parts = [];
    if (run.totalTokens !== null && run.totalTokens >= 0 && typeof servingSpend === "number" && Number.isFinite(servingSpend) && totalRecordedTokens > 0) {
      parts.push({
        id: "serving-endpoint",
        label: "Model serving",
        /*
