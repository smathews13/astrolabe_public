...
var FAILURE_TAXONOMY = {
  ...
  IDEMPOTENCY_KEY_MALFORMED: {
    ...
    uiMessage: "The Idempotency-Key header must be 8 to 200 characters of letters, digits, dot, colon, underscore or hyphen. It was not used, so this request was not protected against being run twice, and it is refused rather than run without the protection you asked for.",
    trace: "record_failure",
    alert: "info",
    mayGenerateAnswer: false
  },
  BUDGET_APPROVAL_REQUIRED: {
    ...
    retryable: false,
    mayRerouteOrReidentify: false,
    uiMessage: "Measured month-to-date spend reached the monthly app budget. An administrator must approve continued usage before a new question can start.",
    trace: "may_have_no_trace",
    alert: "info",
    mayGenerateAnswer: false
  },
...
function failureDefinition(code) {
...
var SP_IDENTITY_ENABLED_SETTING = "sp-identity-enabled";
var SP_EXECUTION_OAUTH = "oauth";
var SP_EXECUTION_SERVICE_PRINCIPAL = "service_principal";
var ASSIGNED_SERVICE_PRINCIPAL = "assigned_service_principal";
var SP_IDENTITY_MINTING_UNAVAILABLE = "Databricks Apps cannot mint a token for another service principal from the signed-in user's OAuth scopes. This app can only obtain one by reading that principal's OAuth secret from Databricks Secrets (the scope and key you name on the persona) using the app's own identity, then exchanging it. If the app cannot read that secret, questions stay on OAuth.";
var SP_GRANT_RESOURCE_TYPES = [
  "SERVING_ENDPOINT",
  "SQL_WAREHOUSE",
  "CATALOG",
  "SCHEMA",
...
var CAPABILITY_MAX = 180;
var CAPABILITY_COUNT_MAX = 12;
var SP_PERSONA_GRANT_COUNT_MAX = 24;
var SECRET_REF_MAX = 128;
var CLIENT_ID_MAX = 64;
var SpPersonaWriteSchema = external_exports.object({
  displayName: external_exports.string().trim().min(1).max(NAME_MAX),
  clientId: external_exports.string().trim().min(8).max(CLIENT_ID_MAX).regex(/^[0-9a-fA-F-]{8,}$/, "That does not look like a service principal application id."),
  secretScope: external_exports.string().trim().min(1).max(SECRET_REF_MAX),
  secretKey: external_exports.string().trim().min(1).max(SECRET_REF_MAX)
});
var SpPersonaPatchSchema = SpPersonaWriteSchema.partial().refine((value) => Object.keys(value).length > 0, {
  message: "Nothing to update."
});
var SpCapabilitySchema = external_exports.string().trim().min(1).max(CAPABILITY_MAX);
var SpCapabilitiesSchema = external_exports.array(SpCapabilitySchema).max(CAPABILITY_COUNT_MAX);
...
function spGrantIdentifierFault(type, raw) {
...
var SpGrantSchema = external_exports.object({
...
var SpGrantsSchema = external_exports.array(SpGrantSchema).max(SP_PERSONA_GRANT_COUNT_MAX).refine((grants2) => new Set(grants2.map(spGrantKey)).size === grants2.length, {
  message: "The permissions contain an exact duplicate."
});
var SpPersonaDefinitionFields = external_exports.object({
  displayName: external_exports.string().trim().min(1).max(NAME_MAX),
...
var uniqueCapabilities = (capabilities2) => !capabilities2 || new Set(capabilities2.map((capability) => capability.toLocaleLowerCase())).size === capabilities2.length;
var SpPersonaDefinitionWriteSchema = SpPersonaDefinitionFields.refine(
...
var SpPersonaDefinitionPatchSchema = external_exports.object({
...
var SpIdentityModeSchema = external_exports.object({
  enabled: external_exports.boolean()
});
var SpAssignmentWriteSchema = external_exports.object({
  email: external_exports.string().trim().min(3).max(320),
  personaId: external_exports.string().trim().min(1).max(80).nullable()
});

// server/lib/sql-query-tags.ts
import { createHash } from "node:crypto";
var SQL_QUERY_TAG_LIMIT = 20;
var SQL_QUERY_TAG_TEXT_LIMIT = 128;
...
// server/lib/token-rejection.ts
var TOKEN_LAYER = "the forwarded user token";
...
function presentedTokenAge(token, now = /* @__PURE__ */ new Date()) {
  ...
  const seconds = Math.round((exp * 1e3 - now.getTime()) / 1e3);
  return seconds <= 0 ? { kind: "expired", expiresAt, secondsAgo: Math.abs(seconds) } : { kind: "live", expiresAt, secondsLeft: seconds };
}
function readableSpan(seconds) {
...
var REFUSED = "Databricks refused the sign-in this app forwarded for you (HTTP 401) before it looked at any permission, so nothing was run and nothing about your own access was established. This is not a permission you are missing, and no grant made to you would change it.";
var NEW_SIGN_IN = {
  kind: "ui",
...
var SESSION_IS_NOT_THE_WORKSPACE = "Signing out of Databricks does not clear this app\u2019s sign-in: it is held by the proxy in front of the app, not by the workspace.";
function tokenRejection(input) {
  const said = input.apiMessage?.trim() ? ` Databricks said: ${input.apiMessage.trim()}` : " Databricks gave no reason with it.";
  if (input.age.kind === "expired") {
    ...
    return {
      ...
      evidence: `The presented token states that it expired at \`${input.age.expiresAt}\`, which is ${ago} before this check ran, and Databricks answered HTTP 401.${said}`,
      explanation: `${REFUSED} The sign-in it forwarded had run out ${ago} ago, which is the ordinary way for this to happen and the one version of it that clears itself.`,
      remedy: {
        kind: "ui",
  ...
  if (input.age.kind === "live") {
    ...
    return {
      ...
      evidence: `The presented token states that it does not expire until \`${input.age.expiresAt}\`, which is ${left} after this check ran, so it had not run out when Databricks answered HTTP 401.${said}`,
      explanation: `${REFUSED} The sign-in it forwarded had not run out: it is good for another ${left}. Reloading this page hands over the same one and gets the same answer, so that is not the thing to try.`,
      remedy: {
  ...
  return {
    ...
    evidence: `Databricks answered HTTP 401.${said} The presented token\u2019s own expiry could not be read here (${input.age.why}), so whether it had run out was not established either way.`,
    explanation: `${REFUSED} Whether the sign-in had simply run out cannot be read from here, so this does not guess. The step below is the one that works whether it had or not.`,
    remedy: {
      kind: NEW_SIGN_IN.kind,
      statement: NEW_SIGN_IN.statement,
      guidance: SESSION_IS_NOT_THE_WORKSPACE
...
var DICTIONARY_GENIE_TABLES = ["data_dictionary"];
var DATA_CONTRACT_TABLES = [...DATA_GENIE_TABLES, ...DICTIONARY_GENIE_TABLES];
...
var AGE_NOT_SUPPLIED = {
  kind: "unreadable",
  why: "this check was not given the token to read"
};
var USER_TOKEN_HEADER = "x-forwarded-access-token";
function forwardedUserToken(req) {
  const raw = req.header(USER_TOKEN_HEADER);
  return typeof raw === "string" && raw.trim() ? raw.trim() : null;
}
function diagnoseUserToken(req, isDevelopmentIdentity) {
  if (forwardedUserToken(req)) return { kind: "present" };
  if (isDevelopmentIdentity) {
    return {
      kind: "absent-local",
      summary: "This session is not running behind Databricks Apps, so there is no signed-in user and nothing to forward a token for. Nothing is wrong with the deployment.",
      layer: "local development"
    };
  }
  return {
    kind: "absent-in-apps",
    summary: "Databricks Apps did not forward a user token with this request. The app is not acting on the user\u2019s behalf at all. This is a platform-side state, not a failure of any check. Either user authorization is not enabled for this workspace, or the app has not been stopped and started since `user_api_scopes` last changed. A redeploy alone does not apply a scope change.",
    layer: "app configuration"
  };
}
function looksLikeMissingScope(message) {
  const text2 = message.toLowerCase();
  return text2.includes("does not have required scopes") || text2.includes("insufficient_scope") || text2.includes("scope") && (text2.includes("oauth") || text2.includes("token"));
}
var GENIE_SCOPE = "dashboards.genie";
var ALL_APIS_SCOPE = "all-apis";
function scopesFromToken(token) {
  ...
  try {
    const payload = segments[1].replace(/-/g, "+").replace(/_/g, "/");
    claims2 = JSON.parse(Buffer.from(payload, "base64").toString("utf8"));
  } catch {
    return null;
  }
  const raw = claims2?.scope ?? claims2?.scopes;
  if (typeof raw === "string") {
    const scopes = raw.split(/\s+/).filter(Boolean);
    return scopes.length ? scopes : null;
  }
  if (Array.isArray(raw)) {
    const scopes = raw.filter((entry) => typeof entry === "string" && entry.length > 0);
    return scopes.length ? scopes : null;
  }
  return null;
}
function tokenGrantsGenie(token) {
...
function tableGrant(table, principal) {
...
function aclField(principal) {
  return principal.includes("@") ? "user_name" : "service_principal_name";
}
function warehouseGrant(warehouseId, principal) {
  return {
    kind: "cli",
    statement: `databricks permissions update warehouses ${warehouseId} --json '{"access_control_list":[{"${aclField(principal)}":"${principal}","permission_level":"CAN_USE"}]}'`,
...
var SQL_ACCESS_ENTITLEMENT = "databricks-sql-access";
var WORKSPACE_ACCESS_ENTITLEMENT = "workspace-access";
var UNKNOWN_USER_ID = "<numeric-user-id>";
function entitlementGrant(userId) {
  const target = userId && userId.trim() ? userId.trim() : UNKNOWN_USER_ID;
  return {
    kind: "cli",
    statement: `databricks api patch /api/2.0/preview/scim/v2/Users/${target} --json '{"schemas":["urn:ietf:params:scim:api:messages:2.0:PatchOp"],"Operations":[{"op":"add","path":"entitlements","value":[{"value":"${SQL_ACCESS_ENTITLEMENT}"},{"value":"${WORKSPACE_ACCESS_ENTITLEMENT}"}]}]}'`,
...
function genieSpaceGrant(spaceId, principal) {
  return {
    kind: "cli",
    statement: `databricks permissions update genie ${spaceId} --json '{"access_control_list":[{"${aclField(principal)}":"${principal}","permission_level":"CAN_RUN"}]}'`,
...
var ABSENCE_MARKERS = ["table_or_view_not_found", "cannot be found", "no such table"];
var CATALOG_REFUSED = /catalog\s+'([^']+)'\s+is not accessible/i;
var SCHEMA_REFUSED = /schema\s+'([^']+)'\s+is not accessible/i;
function matches(text2, markers) {
  return markers.some((marker) => text2.includes(marker));
}
function classifyDenial(message, table) {
  const text2 = message.toLowerCase();
  if (matches(text2, PERMISSION_MARKERS)) {
  ...
  if (matches(text2, ABSENCE_MARKERS)) return { kind: "hidden-or-absent" };
...
function classify(message, table, principal) {
  ...
  if (denial.kind === "unrecognised") {
    return {
      table,
      status: "error",
      detail: `The check did not complete, so your access to ${table} is unknown rather than refused. This is not a permission result and should not be read as one.`,
      apiMessage: message
    };
  }
  if (denial.kind === "hidden-or-absent") {
    return {
      table,
      status: "denied",
      detail: `${table} did not resolve for you. Unity Catalog hides objects it cannot traverse, so a missing grant and a table that is not there report identically. This is either no SELECT, no USE SCHEMA on the schema, no USE CATALOG on the catalog, or a table that genuinely does not exist. The grants below cover the first three; if they run and this still fails, the table is the one that is missing.`,
  ...
  const scope = denial.objectKind === "table" ? `${denial.permission} on ${denial.object}` : `${denial.permission} on ${denial.objectKind} ${denial.object}, which ${table} is inside`;
  return {
    table,
    status: "denied",
    detail: `You do not hold ${scope}. The API said so in those terms rather than hiding the object, so this is a grant that is missing and not a table that is absent.`,
...
var UNKNOWN_PRINCIPAL = "<your-username>";
function missingScopeBlock(apiMessage) {
  return {
    kind: "no-sql-scope",
    summary: "Your token reached Databricks and was refused for lacking the `sql` scope, so no statement was run and nothing about your own permissions was established. This is not a permission you are missing. It is a scope the app is missing, and no grant made to you will change it.",
    layer: "app configuration",
    remedy: {
      kind: "cli",
      statement: "# 1. `sql` must be in user_api_scopes (resources/player_insights_app.app.yml).\n# 2. A scope is applied when the app STARTS. A redeploy leaves it inert:\ndatabricks apps stop <app-name>\ndatabricks apps start <app-name>",
...
var VERIFICATION_BUDGET_MS = 12e4;
  ...
  for (const table of tables) {
    if (now() - startedAt >= budgetMs) {
      verdicts.push({
        table,
        status: "error",
        detail: `Not checked: the access check reached its ${Math.round(budgetMs / 1e3)} s budget after ${verdicts.length} table(s). Nothing is known about your access to ${table} either way.`
      });
      continue;
    }
    let result;
...
function scimUserFilter(email) {
  return { filter: `userName eq ${email}` };
}
function readScimEntitlements(body) {
  const resources = body?.Resources;
  if (!Array.isArray(resources) || resources.length === 0) {
    return {
      kind: "unavailable",
      why: "the SCIM search returned no user for that address, so this token either cannot read that account or the address is not the one Databricks knows them by"
    };
  }
  const user = resources[0];
  const entitlements = Array.isArray(user.entitlements) ? user.entitlements.map(
    (entry) => typeof entry === "string" ? entry : entry?.value ?? null
...
function entitlementLookupVia(get) {
  return async (email) => {
    ...
    try {
      ...
      return { kind: "unavailable", why: `the SCIM call failed: ${error.message}` };
    }
  };
}
async function refineWarehouseDenial(warehouseId, principal, apiMessage, lookup) {
  if (!lookup) return warehouseDeniedBlock(warehouseId, principal, apiMessage);
  let reading;
  try {
    reading = await lookup(principal);
  } catch (error) {
    reading = { kind: "unavailable", why: `the lookup threw: ${error.message}` };
  }
...
async function verifyWarehouseAccess(warehouseId, probe, principal = UNKNOWN_PRINCIPAL, entitlements, age = AGE_NOT_SUPPLIED) {
  let result;
  try {
    result = await probe();
  } catch (error) {
  ...
  if (byStatus) {
    return byStatus.kind === "warehouse-denied" ? refineWarehouseDenial(warehouseId, principal, result.message, entitlements) : byStatus;
  }
  ...
  return {
    ...
    summary: `SQL warehouse ${warehouseId} did not answer \`SELECT 1\`, and did not refuse it for a permission either. It is most likely stopped, starting, or unhealthy. Nothing about your permissions was established; try again once it is running.`,
    layer: "SQL warehouse availability",
    apiMessage: result.message
  };
}
function warehouseDeniedBlock(warehouseId, principal, apiMessage, entitlement) {
  const base = `You do not hold CAN_USE on SQL warehouse ${warehouseId}, so no statement could be run as you at all. No table was checked, and none of them should be assumed either way. This is one missing grant on one workspace object, not a verdict on your Unity Catalog access.`;
  const qualifier = entitlement?.kind === "held" ? ` Your account does carry the \`${SQL_ACCESS_ENTITLEMENT}\` entitlement, which is checked because a missing one is refused identically, so the warehouse\u2019s own permissions really are the thing to look at.` : entitlement?.kind === "unknown" ? ` A missing \`${SQL_ACCESS_ENTITLEMENT}\` entitlement produces this exact refusal and could not be checked here (${entitlement.why}), so it is not ruled out. Read it before anybody edits the warehouse ACL:
...
function entitlementDeniedBlock(warehouseId, principal, userId, apiMessage) {
  return {
    kind: "no-sql-entitlement",
    summary: `Your account does not carry the \`${SQL_ACCESS_ENTITLEMENT}\` entitlement, so no SQL statement can run as you anywhere in this workspace. This was read from SCIM, not inferred from the refusal. An entitlement is an assignment on your identity, not a permission on warehouse ${warehouseId}, and no CAN_USE added to that warehouse would change it. A workspace admin can grant it with the command below. No table was checked.`,
...
function classifyWarehouseStatus(status, warehouseId, principal, apiMessage, age = AGE_NOT_SUPPLIED) {
  ...
  if (status === 404) {
    return {
      ...
      summary: `Databricks has no SQL warehouse \`${warehouseId}\` to answer for (HTTP 404). Either that id does not exist in this workspace, or it is not visible to you at all. The API reports both the same way and this cannot tell them apart. Either way no statement was run, and nothing about your permissions was established.`,
      ...
      remedy: {
        ...
        guidance: "Run by a workspace admin: a warehouse that comes back is one you cannot see, and one still absent is a deployment to correct."
      },
      apiMessage
    };
  }
...
function describeImpact(outcome) {
...
function genieSpaceLabel(parts) {
  const title = (parts.title ?? "").trim();
  const id = (parts.id ?? "").trim();
  const role = (parts.role ?? "").trim();
  if (title && id) return `${title} \xB7 ${id}`;
  if (role && title) return `${role} \xB7 ${title}`;
  if (role && id) return `${role} \xB7 ${id}`;
  return title || id || role || "Genie space";
}
function configValue(entries, key) {
...
function accessDependenciesFrom(sources) {
  const configuration = sources.configuration ?? [];
  const env = sources.env ?? {};
  const fromConfig = (key, envVar) => {
  ...
  const tables = manifest.length > 0 ? manifest : listed.length > 0 ? listed : qualifyDataContractTables(
...
function missingGenieScopeLimit(evidence) {
  return {
    what: "Whether the Genie spaces are shared with you.",
    why: `Reading a Genie space needs the \`${GENIE_SCOPE}\` scope on the forwarded token, and this token does not carry it (${evidence}). That is a scope the app is missing rather than a permission you are missing, and no grant made to you would change it. Add \`${GENIE_SCOPE}\` to \`user_api_scopes\`, then STOP and START the app: a scope is applied when the app starts, and a redeploy leaves it inert.`
  };
}
...
async function verifyGenieAccess(spaces, probe, principal = UNKNOWN_PRINCIPAL, scopeState = null, age = AGE_NOT_SUPPLIED) {
  if (spaces.length === 0) {
    return {
      verdicts: [],
      notChecked: {
        what: "Whether the Genie spaces are shared with you.",
        why: "The agent\u2019s configuration named no Genie spaces, so there was nothing to ask about on your behalf. This is a configuration state and says nothing about your permissions."
      }
    };
  }
  if (scopeState === false) {
  ...
  const refusedForScope = results.find(
    (result) => !result.ok && looksLikeMissingScope(result.message)
  );
  if (refusedForScope) {
    return {
      verdicts: [],
      notChecked: missingGenieScopeLimit(
        `Databricks refused the call and said so: ${refusedForScope.message}`
      )
    };
  }
  return {
...
function limitsOfThisCheck(servingChecked, genie, tablesChecked = 0) {
  const servingSaw = servingChecked.length > 0 ? "Also checked as the agent serving principal (what preflight reported for that identity, not a claim about who executes later asks): " + servingChecked.map((check) => `${check.label} (${check.status})`).join("; ") + "." : void 0;
  const genieLimit = genie?.notChecked ? { ...genie.notChecked, insteadAs: servingSaw } : genie && genie.verdicts.length > 0 ? {
    ...
    what: "Whether the Genie spaces are shared with you.",
    why: `Reading a Genie space needs the \`${GENIE_SCOPE}\` scope on the forwarded token. This run did not get as far as asking, so nothing about your access to the spaces was established either way.`,
    insteadAs: servingSaw
  };
  const limits = [
    genieLimit,
    ...tablesChecked === 0 ? [
      {
        ...
        why: "The app could not learn which tables this release may read (no PLAYER_INSIGHTS_DECLARED_MANIFEST / PLAYER_INSIGHTS_TABLES, and no catalog+schema to qualify the committed data contract), so no SELECT was run on your behalf. A pass above means you can run a statement, not that you could read the data."
      }
    ] : [],
    {
      what: "Whether a row filter or a column mask narrows what you would see.",
      why: "Neither reports itself. A filtered query succeeds and returns fewer rows, so a green above means the grant exists, not that you would see every row behind a figure. There is no error to check for and nothing here can detect it."
    }
  ];
  return limits;
}
async function verifyAccess(input, probes) {
  const age = input.presentedToken ?? AGE_NOT_SUPPLIED;
  const genie = probes.genieSpace ? await verifyGenieAccess(
    input.genieSpaces ?? [],
    probes.genieSpace,
    input.principal,
...
function isVerified(outcome) {
  return !outcome.blocked && (outcome.warehouseVerified === true || outcome.verdicts.length > 0) && outcome.verdicts.every((verdict) => verdict.status === "ok") && !(outcome.genie ?? []).some((verdict) => verdict.status === "denied");
}
function verificationSummary(outcome) {
  const tables = outcome.ok === 1 ? "1 table" : `${outcome.ok} tables`;
  const head = outcome.ok === 0 ? "Verified you can run a statement on the SQL warehouse under your own token. No table was checked, so nothing about your access to the data behind an answer was established" : `Verified you hold CAN_USE on the SQL warehouse and SELECT on ${tables} under your own token`;
  return `${head}. ${genieClause(outcome.genie)} Row-level filters and column masks were not checked and are not covered by this.`;
}
function genieClause(genie) {
  ...
  return `${head} ${unknown.length} did not answer, so ${unknown.length === 1 ? "it is" : "they are"} unknown rather than granted: ${unknown.map((verdict) => verdict.label).join("; ")}.`;
}
...
var GENIE_PROBE_TIMEOUT_MS = 15e3;
function genieSpaceProbeFor(options) {
  ...
  return async (spaceId) => {
    ...
    try {
      ...
      return {
        ok: false,
        message: timedOut ? `Genie did not answer within ${timeoutMs} ms, so this check did not complete.` : `Genie could not be reached: ${error.message}`
      };
    }
...
function statementExecutorFor(options) {
  const call = options.fetchImpl ?? fetch;
  ...
  return async (statement) => {
    ...
    try {
      ...
      return {
        ok: false,
        message: timedOut ? `The SQL warehouse did not answer within ${timeoutMs} ms, so this check did not complete.` : `The SQL warehouse could not be reached: ${error.message}`
      };
    }
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
...
var CORRELATION_HEADER = "x-pia-correlation-id";
var CORRELATION_PREFIX = "req-";
var CORRELATION_SHAPE = /^req-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/;
function mintCorrelationId() {
  return `${CORRELATION_PREFIX}${crypto.randomUUID()}`;
}
function usableCorrelationId(value) {
  if (typeof value !== "string") return null;
  const trimmed = value.trim();
  return CORRELATION_SHAPE.test(trimmed) ? trimmed : null;
}

// server/lib/identity-subject.ts
var CLOCK_SKEW_SECONDS = 60;
var SUBJECT_CLAIMS = ["sub", "email", "upn", "preferred_username", "username"];
function readTokenClaims(token) {
  const segments = token.split(".");
  if (segments.length !== 3) return null;
  try {
    const payload = segments[1].replace(/-/g, "+").replace(/_/g, "/");
    const claims2 = JSON.parse(Buffer.from(payload, "base64").toString("utf8"));
    if (!claims2 || typeof claims2 !== "object" || Array.isArray(claims2)) return null;
    return claims2;
  } catch {
    return null;
  }
}
function tokenSubjects(token) {
  const claims2 = readTokenClaims(token);
  if (!claims2) return [];
  const found = [];
...
function bindTokenToUser(token, email, options = {}) {
  ...
  return {
    kind: "unverifiable",
    why: "the token names its subject as an identifier rather than an address, which cannot be compared with the signed-in email without guessing"
  };
}
...
function isVerified2(binding) {
  return binding.kind === "bound";
}
function describeBinding(binding, email) {
  switch (binding.kind) {
    case "bound":
      return `the forwarded token names ${email}, which is who the request is from`;
    case "mismatch":
      return `the request is from ${email} and the forwarded token was issued to ${binding.subject}. Refusing with ${FAILURE_TAXONOMY.IDENTITY_MISMATCH.code} rather than running either of them.`;
    case "expired":
...
// server/lib/identity-binding.ts
var SIGNED_IN_USER = "signed_in_user";
var ASSIGNED_SERVICE_PRINCIPAL2 = "assigned_service_principal";
var APP_SERVICE_PRINCIPAL = "app_service_principal";
function decideIdentity(req, options) {
  ...
  return {
    ...
    email: options.signedInAs,
    token: token ?? "",
    verified: isVerified2(binding),
    mode: SIGNED_IN_USER,
    requestId,
    correlationId
  };
}
function executionIdentityClaim(decision) {
  return { mode: decision.mode, verified: decision.verified };
}
function refusedIdentityClaim() {
...
function disclosableRefusal(refusal) {
  return refusal.code === "IDENTITY_MISMATCH" ? "The signed-in user could not be matched to the credential this request carried." : "This request carried no identity the app could execute it as.";
}
function authorizationFailureFor(status) {
...
var SP_PERSONAS_TABLE = appTable("sp_personas");
var SP_PERSONA_DEFINITIONS_TABLE = appTable("sp_persona_definitions");
var SP_ASSIGNMENTS_TABLE = appTable("sp_assignments");
...
function definitionFromRow(row) {
  ...
  const legacy = row.legacy_capabilities === null || row.legacy_capabilities === void 0 ? structured.length === 0 ? storedCapabilities : [] : capabilities(row.legacy_capabilities);
  return {
    id: text(row.id),
    displayName: text(row.display_name),
...
var enabledCache = /* @__PURE__ */ new WeakMap();
var SP_IDENTITY_ENABLED_TTL_MS = 15e3;
function forgetSpIdentityEnabled() {
  enabledCache = /* @__PURE__ */ new WeakMap();
}
async function isSpIdentityEnabled(client, options = {}) {
...
async function writeSpIdentityEnabled(client, enabled, updatedBy) {
  await writeStoredSetting(client, {
    resourceId: SP_IDENTITY_ENABLED_SETTING,
    value: enabled ? "true" : "false",
    intent: "active",
    note: enabled ? "Assigned users run warehouse, Genie, and agent calls as their service-principal persona." : "Questions run as the signed-in OAuth user.",
    updatedBy
  });
  forgetSpIdentityEnabled();
  return enabled;
}
async function listSpPersonas(client) {
  try {
...
async function readSpPersona(client, id) {
  try {
    const result = await client.lakebase.query(
      `SELECT id, display_name, client_id, secret_scope, secret_key, updated_at, updated_by
         FROM ${SP_PERSONAS_TABLE}
...
async function insertSpPersona(client, write, updatedBy) {
  const id = randomUUID();
  const result = await client.lakebase.query(
...
async function updateSpPersona(client, id, write, updatedBy) {
...
async function deleteSpPersona(client, id) {
  try {
...
async function insertSpPersonaDefinition(client, write, updatedBy) {
  const id = randomUUID();
  const structured = write.grants;
  const legacy = structured.length > 0 ? write.legacyCapabilities : [.../* @__PURE__ */ new Set([...write.capabilities, ...write.legacyCapabilities])];
...
async function updateSpPersonaDefinition(client, id, write, updatedBy) {
  ...
  const legacy = write.legacyCapabilities ?? (write.capabilities ? write.capabilities.filter((capability) => !currentStructuredSummaries.has(capability)) : current.legacyCapabilities ?? []);
  const compatibility = [...structured.map(spGrantSummary), ...legacy];
  const next = {
    displayName: write.displayName ?? current.displayName,
...
async function assignmentForEmail(client, email) {
...
async function writeSpAssignment(client, email, personaId, updatedBy) {
  const normalised = normalizeAdminEmail(email);
  if (!normalised) return null;
  if (!personaId) {
...
var SP_TOKEN_CACHE_MAX_ENTRIES = 256;
var SP_TOKEN_CACHE_MAX_TTL_MS = 24 * 60 * 6e4;
var tokenCache = new ExpiringLruCache(SP_TOKEN_CACHE_MAX_ENTRIES, SP_TOKEN_CACHE_MAX_TTL_MS);
var EXPIRY_SKEW_MS = 6e4;
var DEFAULT_EXPIRES_IN = 3600;
function describeSpTokenMinting(env = process.env) {
  ...
  if (!host) {
    return {
      available: false,
      detail: "This app does not know its workspace URL, so it cannot exchange a service-principal secret for a token. Questions stay on OAuth."
    };
  }
  if (!clientId || !clientSecret) {
    return {
      available: false,
      detail: "This app has no service-principal credentials of its own, so it cannot read a persona secret from Databricks Secrets. " + SP_IDENTITY_MINTING_UNAVAILABLE
    };
  }
  return {
    available: true,
...
async function defaultReadSecret(scope, key) {
...
async function defaultExchange(input) {
  ...
  const expiresIn = typeof parsed.expires_in === "number" && Number.isFinite(parsed.expires_in) ? parsed.expires_in : DEFAULT_EXPIRES_IN;
  return { token, expiresInSeconds: expiresIn };
}
async function mintPersonaToken(persona, deps = {}) {
  ...
  const exchange = deps.exchange ?? ((input) => defaultExchange({ ...input, fetchImpl: deps.fetchImpl }));
  try {
    const secret = await readSecret(persona.secretScope, persona.secretKey);
    if (!secret) {
      return {
        ok: false,
        reason: `The app could not read the secret ${persona.secretScope}/${persona.secretKey} for ${persona.displayName}. Questions for this person stay on OAuth until the app identity can GET that secret.`
      };
    }
    ...
    const message = error.message || "The token exchange failed.";
    if (/scope|insufficient|permission|403|401/i.test(message)) {
      return {
        ok: false,
        reason: `The app could not mint a token for ${persona.displayName}: ${message} ` + SP_IDENTITY_MINTING_UNAVAILABLE
      };
    }
    return {
      ok: false,
...
// server/lib/execution-credential.ts
...
function executionPersona(persona) {
  return { id: persona.id, displayName: persona.displayName, clientId: persona.clientId };
}
async function resolveExecutionCredential(req, store, deps = {}) {
...
async function attachExecutionCredential(req, store, deps = {}) {
  const credential = await resolveExecutionCredential(req, store, deps);
  attached.set(req, credential);
  return credential;
}
function executionCredentialMiddleware(store, deps = {}) {
  return function attach(req, _res, next) {
    if (!req.path.startsWith("/api/")) {
      next();
      return;
    }
    attachExecutionCredential(req, store, deps).then(() => next()).catch((error) => {
...
function overlayAssignedPersona(decision, req) {
  ...
  return {
    ...
    mode: ASSIGNED_SERVICE_PRINCIPAL2,
    verified: true,
    persona: credential.persona
  };
}
function servingIdentityFields(identity) {
  if (identity.mode === ASSIGNED_SERVICE_PRINCIPAL2 && identity.persona) {
...
async function describeSpIdentity(req, store, deps = {}) {
  const env = deps.env ?? process.env;
  const minting = describeSpTokenMinting(env);
  const enabled = await isSpIdentityEnabled(store);
  const email = signedInEmail(req);
...
// shared/declared-scopes.ts
var DECLARED_SCOPES_VAR = "PLAYER_INSIGHTS_USER_API_SCOPES";
function declaredUserApiScopes(env = process.env) {
...
var WORKSPACE_READ_USER_API_SCOPE = "workspace.workspace:read";
var LAKEBASE_USER_API_SCOPE = "postgres";
var OPTIONAL_USER_API_SCOPES = [
...
function isOptionalUserApiScope(name) {
  return OPTIONAL_USER_API_SCOPES.includes(name);
}

// shared/stated-cause.ts
var UNDETERMINED = "undetermined";

// shared/fresh-sign-in.ts
function quotedScopes(scopes) {
  const names = scopes.map((scope) => `\`${scope}\``);
  if (names.length <= 1) return names[0] ?? "nothing";
  return `${names.slice(0, -1).join(", ")} and ${names[names.length - 1]}`;
}
var NEW_SIGN_IN2 = "Open this app again in a private browsing window, and sign in there.";
var PRIVATE_WINDOW_NAMES = "Chrome and Edge call it Incognito or InPrivate. Safari and Firefox call it a Private Window.";
var SIGN_OUT_DOES_NOT_CLEAR = "Signing out of Databricks does not clear this app\u2019s sign-in.";
function freshSignIn() {
  return {
    kind: "ui",
    statement: `${NEW_SIGN_IN2}
...
var INERT_TOKEN_SCOPES = /* @__PURE__ */ new Set([
  "openid",
  "profile",
  "email",
  "offline_access",
...
function scopeStems(scope) {
  return new Set(
    scope.split(/[.:\-_/]+/).map((word) => word.toLowerCase()).filter((word) => word.length > 2 && !["read", "write", "api", "apis", "all"].includes(word))
  );
}
