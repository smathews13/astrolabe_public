    ...
    try {
      ...
      worker.once("message", (message) => {
        ...
        if (result?.ok === true && typeof result.text === "string") {
          const text9 = result.text.slice(0, task.maxChars);
          void this.settle(
            task,
            text9.trim() ? text9 : new PdfTextError(
              "no-text",
...
function takeawayWhenTablesLanded(output, evidence) {
  const text9 = output.trim();
  if (!UNANSWERED_LINE.test(text9)) return output;
...
function buildSpendByUser(input) {
  ...
  const appendComponent = (id, unit, total, weights, usable, reason) => {
    ...
    const allocated = total === null ? /* @__PURE__ */ new Map() : direct.length > 0 ? new Map(direct.map((row2) => [row2.email.toLowerCase(), unit === "USD" ? row2.usd : row2.dbu])) : usable ? allocateDeterministically(total, weights) : /* @__PURE__ */ new Map();
    ...
    for (const email of users) {
      const found = direct.find((row2) => row2.email.toLowerCase() === email);
...
function buildUserMonitoringPage(input) {
  ...
  const emails = /* @__PURE__ */ new Set([...profiles.keys(), ...runs.keys(), ...activity.keys(), ...input.roles.keys()]);
...
function normalizeAnswerCaveat(caveat, evidence = {}) {
  const text9 = caveat.trim();
  if (!text9) return null;
  const scope = UNTAGGED_TABLE_SCOPE.exec(text9);
  if (scope) {
  ...
  if (REVIEW_SQL_AND_SOURCES.test(text9)) {
  ...
  if (NO_GOVERNED_READ.test(text9) || GOVERNED_READ_PARAPHRASE.test(text9) || NOT_GROUNDED.test(text9) || NO_SQL.test(text9) || NO_EVIDENCE_TOOL.test(text9) || NO_MLFLOW_TRACE.test(text9) || PROCESS_SECTION_LINE.test(text9)) {
    return null;
  }
  if (INCOMPLETE_FORMAT.test(text9)) {
    return "This answer is degraded: the response format was incomplete. Retry the question before using this result.";
  }
  if (MATERIAL_WARNING.test(text9)) return text9;
  if (INTERNAL_PROCESS_NEGATIVE.test(text9)) return null;
  if (FIGURE_LIMIT.test(text9) || FIGURE_LIMIT_RANKED.test(text9)) return null;
  return text9;
}
function normalizeReaderText(value, evidence = {}, field2 = "narrative") {
  const text9 = value.trim();
  if (!text9) return "";
  if (field2 === "takeaway" && PROCESS_ONLY_TAKEAWAY.test(text9)) {
...
var MLFLOW_TRACE_ID = /^tr-[0-9a-f]+$/i;
var MLFLOW_HEX32 = /^[0-9a-f]{32}$/i;
var UUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
function isMlflowTraceId(value) {
  return typeof value === "string" && MLFLOW_TRACE_ID.test(value.trim());
}
function asMlflowTraceId(value) {
  if (typeof value !== "string") return "";
  const text9 = value.trim();
  if (!text9 || text9.toLowerCase().startsWith("trace-") || /NO_OP/i.test(text9)) return "";
  if (UUID.test(text9)) return "";
  if (MLFLOW_TRACE_ID.test(text9)) return `tr-${text9.slice(3).toLowerCase()}`;
  if (MLFLOW_HEX32.test(text9)) return `tr-${text9.toLowerCase()}`;
  return "";
}
...
function servingTraceCandidates(payload) {
  ...
  const buckets = [record.custom_outputs, record.databricks_output, record];
...
function bindServingMlflowTraceId(answer, platformTraceId) {
  const recorded2 = asMlflowTraceId(platformTraceId);
  if (isMlflowTraceId(answer.trace.id) || !recorded2) return answer;
  return { ...answer, trace: { ...answer.trace, id: recorded2 } };
}
function withoutUntracedTimeline(trace) {
...
function projectToolOutput(text9, tool = "") {
  ...
  if (!value.startsWith("ERROR:")) return projectNamedToolOutput(text9, tool);
...
function isCannedFirstLine(text9) {
  const value = text9.trim();
...
function stripAppendedHumanLabels(text9) {
  const cut = text9.search(/\nHuman labels:\s*\n/);
  return (cut >= 0 ? text9.slice(0, cut) : text9).trim();
}
function firstGuidelineSentence(text9) {
  const stem = stripAppendedHumanLabels(text9);
...
function extraJudgesFromSettings(settings) {
  ...
  for (const id of settings.enabledMultiTurnJudges) {
    ...
    const text9 = id === "conversational_guidelines" ? fallback : definition.guidelines.trim();
    extras.push({
      name: id,
      guidelines: text9 ? [text9] : [],
      kind: "multi-turn"
    });
  }
...
function labelingIdsFromBody(body) {
  ...
  const record = body;
  const nested = record.session && typeof record.session === "object" ? record.session : record;
  const text9 = (value) => typeof value === "string" ? value.trim() : value != null ? String(value) : "";
  return {
    sessionId: text9(nested.labeling_session_id ?? nested.session_id ?? nested.id),
    runId: text9(nested.mlflow_run_id ?? nested.run_id),
    name: text9(nested.name)
  };
}
...
function classifyGenieMiss(note) {
  const text9 = note.toLowerCase();
  if (/warehouse/.test(text9) && /start|starting|pending|warming|not running|stopped|resuming/.test(text9)) {
    return "warehouse";
  }
  if (/warehouse still starting|compute is starting|cluster is starting/.test(text9)) {
    return "warehouse";
  }
  if (/cancel|cancelled|canceled|timed out|timeout|wait ran out|50\s*s|deadline/.test(text9)) {
    return "timeout";
  }
  return "error";
}
...
function sqlLooksRefused(sql, note) {
  const text9 = `${sql} ${note}`.toLowerCase();
  return /refused|sql refused|identity_required|restricted column|not read-only/.test(text9);
}
...
${answer.sql}`);
  ...
  const text9 = parts.join("\n\n");
  if (text9.length <= MAX_JUDGE_CONTEXT_CHARS) return { text: text9, truncated: false };
  return { text: text9.slice(0, MAX_JUDGE_CONTEXT_CHARS), truncated: true };
}
...
function clamp(raw2, max) {
  const text9 = typeof raw2 === "string" ? raw2.trim() : "";
  return text9.length > max ? text9.slice(0, max) : text9;
}
function pointer(raw2) {
  const text9 = clamp(raw2, IDENTIFIER_MAX);
  return text9 === "" ? null : text9;
}
...
var COST_BUDGET_WITHHELD_TILE_IDS = /* @__PURE__ */ new Set(["foundation-model", "index-rebuild-job"]);
...
async function queryMeasurement(appkit, req, period, now) {
  ...
  } = await import("./ops-routes-X7F7NSIM.mjs");
  ...
  const built = (await import("./ops-billing-J73HUC5L.mjs")).buildCostStatement(resolved.ids, range);
...
function messageFrom(body, fallback) {
  ...
  const record = body;
  return text7(record.message) || text7(record.error) || text7(record.error_code) || fallback;
}
...
async function readAppContext(appName, host, reader, now) {
  ...
  if (appName && host) {
    try {
      ...
      result = { workspaceId: workspaceIdFromAppUrl(textOf3(app.url)) };
    } catch {
    }
  }
...
function benchmarkRunTrace(row2) {
  ...
  const identity = {
    ...
    conversationId: null,
    createdAt: timestamp(row2.created_at),
    prompt: text8(metrics.prompt) ?? `Benchmark suite: ${text8(row2.suite_id) ?? "unknown"}`,
  ...
  return {
    ...
    benchmark: {
      ...
      // depend on.
      ...metrics,
      suiteId: text8(metrics.suiteId) ?? text8(row2.suite_id),
...
function extractLiveText(value) {
  ...
  for (const key of ["content", "output", "messages", "choices"]) {
    ...
    if (Array.isArray(items)) {
      for (const item of items) {
        const text9 = extractLiveText(item);
  ...
  for (const key of ["message", "data", "response", "result", "body"]) {
    if (record[key]) {
      const text9 = extractLiveText(record[key]);
      if (text9) return text9;
    }
  }
  return null;
...
function rejectionStatus(error) {
  ...
  const message = error instanceof Error ? error.message : text8(error) ?? "";
