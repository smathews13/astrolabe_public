...
var WHERE_THE_LISTS_ARE = "The Connected as section of the Connections page lists what your sign-in carries and what this app asks for.";
...
function servingFacts(body) {
  ...
  const routes = Array.isArray(record(body.traffic_config).routes) ? record(body.traffic_config).routes.map(record) : [];
...
function sql(statement, guidance = "") {
...
var READS_METADATA = "GET, as the signed-in user";
var SERVING_ENDPOINT_KIND = "serving-endpoint";
var SERVING_ENDPOINTS = [
  {
    id: "agent-endpoint",
...
var ANSWER_PATH_ENDPOINT_IDS = SERVING_ENDPOINTS.filter(
  ...
  if (index && index.includes(".")) {
    subjects.push({
      ...
      observe: (body) => {
        ...
        const state = text(body.status?.detailed_state);
...
function indexContentAt(body) {
  ...
  for (const key of ["triggered_update_status", "continuous_update_status"]) {
    const stamp = text(status[key]?.last_processed_commit_timestamp);
    if (stamp && !Number.isNaN(new Date(stamp).getTime())) return stamp;
  }
  return "";
}
function servingEndpointSubject(id, label, name, note) {
  return {
    id,
    kind: SERVING_ENDPOINT_KIND,
    name,
    label: `${label} \xB7 ${name}`,
    ...
    grant: (principal) => cli(
      `databricks permissions update serving-endpoints ${name} --json '{"access_control_list":[{"user_name":"${principal || "<the signed-in user>"}","permission_level":"CAN_QUERY"}]}'`
    )
  };
}
...
async function runProbe(subject, options) {
  ...
  try {
    ...
    return finish({ kind: "unreachable", message: error?.message ?? String(error) });
  }
}
...
function withDerivedSemanticIndex(configured) {
...
function withSemanticFollowUps(checks, configured) {
  ...
  const answered = (id) => checks.some((check2) => check2.id === id);
  const added = [];
  ...
  const indexVerdict = checks.find((check2) => check2.id === "semantic-index");
...
function withManifestRollup(checks) {
  ...
  const failed2 = tables.filter((entry) => entry.status === "failed");
  const unverified = tables.filter((entry) => entry.status === "unverified");
  const status = failed2.length > 0 ? "failed" : unverified.length > 0 ? "unverified" : "ok";
  ...
  const scope = failed2.length === 0 && refusedScopes.size === 1 && unverified.length === tables.length ? [...refusedScopes][0] : "";
  return [
    ...checks,
    {
...
function browseScopeUnavailableDetail(scope) {
  return `Browsing is unavailable because your sign-in does not carry \`${scope}\`. Nothing was established about which assets exist. Enter the value by hand, or sign in again after this app asks for that permission.`;
}
function browseAppsHasNoScopeDetail(family) {
  return `Browsing is unavailable because Databricks Apps has no ${family} scope to forward. Nothing was established about which assets exist. Enter the value by hand.`;
}

// server/lib/discovery-control.ts
var DISCOVERY_MAX_CONCURRENCY = 4;
var DISCOVERY_CACHE_TTL_MS = 1e4;
var DISCOVERY_CACHE_MAX_ENTRIES = 128;
function abortError(signal) {
  return signal.reason instanceof Error ? signal.reason : new DOMException("Aborted", "AbortError");
}
var DiscoveryLimiter = class {
  ...
  run(signal, work) {
    ...
    return new Promise((resolve, reject) => {
      const queued = { signal, run: work, resolve, reject };
      if (signal) {
        const abort = () => {
...
var discoveryLimiter = new DiscoveryLimiter();
var DiscoveryPageCache = class {
...
// server/lib/browse-assets.ts
var BROWSE_TIMEOUT_MS = 15e3;
var BROWSE_PAGE_SIZE = 100;
var BROWSE_PAGE_LIMIT = 5;
var BROWSE_SCOPE_BY_PATH = {
  "/api/2.1/unity-catalog/catalogs": "catalog.catalogs:read",
  "/api/2.1/unity-catalog/schemas": "catalog.schemas:read",
  "/api/2.1/unity-catalog/tables": "catalog.tables:read",
...
function scopeForBrowsePath(path) {
...
function unavailable(kind, scope) {
  return {
    status: "unavailable",
    kind,
...
function unavailableNoAppsScope(kind, family) {
  return {
    status: "unavailable",
    kind,
...
function failed(kind, detail, error = "", incompleteReason = "failed") {
  return { status: "failed", kind, detail, error, incomplete_reason: incompleteReason };
}
function ok(kind, items, nextPageToken = "", path = "", page = 1) {
...
async function workspaceGet(pathAndQuery, options) {
  ...
  try {
    ...
    return { kind: "unreachable", message: error?.message ?? String(error) };
  }
}
function browseBlockedByScope(input) {
  const scope = scopeForBrowsePath(input.apiPath);
  if (!scope) return null;
  const declared = input.declaredScopes === void 0 ? declaredUserApiScopes() : input.declaredScopes;
...
function interpretBrowseAnswer(input) {
  ...
  const code = text2(body.error_code);
  const message = text2(body.message);
...
function catalogItems(body) {
  ...
  return { items, next_page_token: text2(body.next_page_token) };
}
function schemaItems(body) {
  ...
  for (const row of rows) {
    if (!row || typeof row !== "object") continue;
    const record2 = row;
    const fullName = text2(record2.full_name);
    const name = text2(record2.name);
    const schemaName = name.includes(".") ? name.slice(name.indexOf(".") + 1) : name || (fullName.includes(".") ? fullName.slice(fullName.indexOf(".") + 1) : fullName);
...
function tableItems(body) {
  ...
  for (const row of rows) {
    if (!row || typeof row !== "object") continue;
    const record2 = row;
    const fullName = text2(record2.full_name) || text2(record2.name);
    if (!fullName) continue;
    const short = fullName.includes(".") ? fullName.split(".").pop() : fullName;
    items.push({
      id: fullName,
      label: short,
      secondary: text2(record2.table_type),
      expandable: false
    });
  }
  return { items, next_page_token: text2(body.next_page_token) };
...
function warehouseItems(body) {
  ...
  for (const row of rows) {
    if (!row || typeof row !== "object") continue;
    const record2 = row;
    const id = text2(record2.id);
    if (!id) continue;
    items.push({
      id,
      label: text2(record2.name) || id,
      secondary: text2(record2.state),
      expandable: false
    });
  }
  return { items, next_page_token: text2(body.next_page_token) };
...
function genieItems(body) {
  ...
  for (const row of rows) {
    if (!row || typeof row !== "object") continue;
    const record2 = row;
    const id = text2(record2.space_id) || text2(record2.id);
    if (!id) continue;
    items.push({
      id,
      label: text2(record2.title) || id,
      secondary: "",
      expandable: false
    });
  }
...
function servingEndpointItems(body) {
  ...
  for (const row of rows) {
    ...
    const ready = state && typeof state === "object" ? text2(state.ready) : "";
    items.push({
      id: name,
      label: name,
      secondary: text2(record2.task) || ready,
      expandable: false
    });
  }
  return { items, next_page_token: text2(body.next_page_token) };
...
function notebookItems(body) {
  ...
  for (const row of rows) {
    if (!row || typeof row !== "object") continue;
    const record2 = row;
    const path = text2(record2.path);
    if (!path) continue;
    const objectType = text2(record2.object_type).toUpperCase();
    if (objectType !== "DIRECTORY" && objectType !== "NOTEBOOK") continue;
    const label = path.includes("/") ? path.slice(path.lastIndexOf("/") + 1) : path;
    items.push({
      id: path,
      label: label || path,
      secondary: objectType === "NOTEBOOK" ? text2(record2.language) : "Directory",
...
async function validateNotebookPath(pathInput, options) {
  const path = pathInput.trim();
  if (!path.startsWith("/") || path.length > 1024) {
    return { ok: false, status: 400, detail: "Choose an absolute workspace notebook path." };
  }
  const apiPaths = ["/api/2.0/workspace/get-status", "/api/2.0/workspace/export"];
  for (const apiPath of apiPaths) {
    ...
    const suffix = apiPath.endsWith("/export") ? `path=${encodeURIComponent(path)}&format=SOURCE` : `path=${encodeURIComponent(path)}`;
    ...
    if (answer.status === 404 || text2(answer.body.error_code) === "RESOURCE_DOES_NOT_EXIST") {
    ...
    if (apiPath.endsWith("/get-status") && text2(answer.body.object_type).toUpperCase() !== "NOTEBOOK") {
      return { ok: false, status: 400, detail: "Choose a notebook, not a workspace folder." };
    }
  }
  return { ok: true, path };
}
function volumeItems(body) {
  ...
  for (const row of rows) {
    ...
    items.push({
      id: short,
      label: short,
      secondary: text2(record2.volume_type) || fullName,
      expandable: false
    });
  }
  return { items, next_page_token: text2(body.next_page_token) };
...
function vectorSearchEndpointItems(body) {
  ...
  for (const row of rows) {
    ...
    const state = status && typeof status === "object" ? text2(status.state) : "";
    const count = typeof record2.num_indexes === "number" ? `${record2.num_indexes} indexes` : "";
    items.push({
      id: name,
      label: name,
...
function vectorSearchIndexItems(body) {
  ...
  for (const row of rows) {
    ...
    items.push({
      id: name,
      label: short,
      secondary: text2(record2.index_type) || text2(record2.endpoint_name),
      expandable: false
    });
  }
  return { items, next_page_token: text2(body.next_page_token) };
...
function lakebaseProjectItems(body) {
  ...
  for (const row of rows) {
    ...
    const display = status && typeof status === "object" ? text2(status.display_name) : "";
...
function lakebaseDatabaseItems(body) {
  ...
  for (const row of rows) {
    ...
    const name = text2(record2.name);
    if (!name) continue;
    const short = name.includes("/databases/") ? name.slice(name.lastIndexOf("/databases/") + "/databases/".length) : name;
    items.push({
      id: name,
      label: short,
      secondary: name,
...
async function listLakebaseProjects(options) {
...
async function discoverConnectionTypes(options) {
  ...
  const unavailable2 = roots.filter((response) => response.status !== "ok" || response.items.length === 0).map((response) => ({
