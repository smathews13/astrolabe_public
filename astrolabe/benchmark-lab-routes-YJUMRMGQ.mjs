...
var ApplyBody = external_exports.object({
...
var ContractBody = LabContractSchema.partial();
var CancelBody = external_exports.object({
  runId: external_exports.string().trim().max(80).optional(),
  side: external_exports.enum(["baseline", "candidate"]).optional()
});
var RetryBody = external_exports.object({
  ...
  appkit.server.extend((app) => {
    ...
    app.post("/api/admin/benchmarks/lab/version", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The dataset version was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/split", async (req, res) => {
      ...
      try {
        ...
        const next = dataset.rows.map(labCaseFromRow).map((row) => {
          ...
          return parsed.data.split === "held_out" ? lockHeldOut(row, at) : { ...row, split: "tuning" };
        });
        ...
        res.status(503).json({
          ...
          message: `The split was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/review", async (req, res) => {
      ...
      try {
        ...
        const next = dataset.rows.map(labCaseFromRow).map(
          (row) => row.id === parsed.data.caseId ? { ...row, review: parsed.data.review } : row
        );
        ...
        res.status(503).json({
          ...
          message: `The review status was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/retire", async (req, res) => {
      ...
      try {
        const dataset = await readEvalDataset(appkit, { maxAgeMs: 0 });
        const next = dataset.rows.map(labCaseFromRow).map(
          (row) => row.id === parsed.data.caseId ? { ...row, retired: parsed.data.retired } : row
        );
        await writeCases(appkit, next, actor);
        res.json({ lab: await workspace(appkit) });
      } catch (error) {
        res.status(503).json({
          ...
          message: `The case was not updated: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/duplicate", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The edge case was not added: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/import-traces", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `Those questions were not added: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/known-failure", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The known failure was not saved: ${error.message}`
        });
      }
    });
    app.put("/api/admin/benchmarks/lab/contract", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The contract was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/cancel-run", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The cancel request was not recorded: ${error.message}`
        });
      }
    });
    ...
    app.post("/api/admin/benchmarks/lab/align-preview", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `Guidelines were not previewed: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/lab/align-commit", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `Guidelines were not saved: ${error.message}`
        });
      }
    });
      ...
      try {
        ...
        try {
          ...
          promotedPrompt = {
            ...
            note: identifier ? `The production alias was not moved: ${error.message}` : "No Prompt Registry name is set."
          };
        }
        ...
        res.status(503).json({
          ...
          message: `The candidate was not applied: ${error.message}`
        });
      }
    });
  });
