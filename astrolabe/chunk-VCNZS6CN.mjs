...
function sortedBars(values, kind) {
  return [...values].map(([key, count3]) => ({ key, label: label(kind, key), count: count3 })).sort((left, right) => right.count - left.count || left.key.localeCompare(right.key));
}
...
function readUserActivitySpendEvidence(rows) {
  const first = rows[0];
  return {
    users: rows.map((row2) => ({ email: text2(row2.user_email).toLowerCase(), activeMinutes: number(row2.active_minutes) })).filter((row2) => row2.email && row2.activeMinutes > 0),
...
var GOVERNANCE_FACT = "PII redaction on \xB7 role-gated";
...
function pocContractView(input) {
  ...
  return {
    ...
    dataset: `${version} \xB7 ${input.counts.cases} cases \xB7 ${input.counts.heldOut} held out`,
    ...
    snapshotDetail: snapshotId ? `${snapshotId} \xB7 ${version} \xB7 ${input.counts.cases} cases \xB7 ${scorerLine} \xB7 baseline ${input.contract.baselineRunId || "not set"} \xB7 candidate ${input.contract.candidateRunId || "not set"}` : "",
...
async function queryMeasurement(appkit, req, period, now) {
  ...
  } = await import("./ops-routes-QVU2JNJV.mjs");
  ...
  const built = (await import("./ops-billing-23ZR5F3W.mjs")).buildCostStatement(resolved.ids, range);
