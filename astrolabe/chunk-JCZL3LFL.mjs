...
import {
  ...
  recordAdminAction,
  removalRefusal2 as removalRefusal,
  removeAdmin,
  seedAdminEmails
...
var AddBody = external_exports.object({ email: external_exports.string().trim().max(320) });
async function readAdded(store, req) {
  try {
    ...
    console.warn("[admin] The stored admin list could not be read for the Settings editor:", error.message);
...
function runnerFor(req) {
  ...
  if (!host) {
    return {
      run: null,
      unavailable: "Not checked. The app does not know its own workspace URL, so it cannot reach a SQL warehouse to make a grant."
    };
  }
  ...
  appkit.server.extend((app) => {
    ...
    app.post("/api/admins", async (req, res) => {
      ...
      try {
        ...
        console.error(`[admin] ${email} could not be added:`, error.message);
        res.status(503).json({
          error: "admin_store_unavailable",
          detail: "The administrator was not added. The app stores added administrators in Lakebase, and it is not answering: reporting success here would leave a name on screen that no reload would keep."
        });
      }
    });
    app.delete("/api/admins/:email", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          error: "admin_store_unavailable",
          detail: "Nobody was removed. The list could not be read, and removing an administrator without knowing who else is on it could leave this deployment with none."
        });
        return;
      }
      ...
      try {
        ...
        if (withdrawal.revoked > 0 || withdrawal.refused.length > 0) {
          await recordAdminAction(appkit.lakebase, {
            ...
            subject: email,
            detail: `${actor} removed ${email}, and the access an earlier version of this app had granted them: ` + `${withdrawal.summary} ${withdrawal.note}`.trim()
          });
        }
