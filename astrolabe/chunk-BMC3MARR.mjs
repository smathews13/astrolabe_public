...
var BILLING_TAG = { key: "system_billing", value: "astrolabe" };
var RETIRED_BILLING_TAG_KEY = "astrolabe";
function billingTagPair() {
  return `${BILLING_TAG.key}=${BILLING_TAG.value}`;
}

// server/lib/warehouse-cancellation.ts
var ACTIVE_QUERY_STATUSES = ["QUEUED", "STARTED", "COMPILING", "COMPILED", "RUNNING"];
var ACTIVE_STATUS_SET = new Set(ACTIVE_QUERY_STATUSES);
var DEFAULT_SWEEP_DELAY_MS = 500;
var QUERY_HISTORY_PAGE_SIZE = 100;
var MAX_CANCELLATION_HISTORY_PAGES = 8;
var CANCELLATION_LOOKBACK_MS = 7 * 24 * 60 * 60 * 1e3;
var CANCELLATION_DEADLINE_MS = 1e4;
...
function parseQueryTags(value) {
  ...
  for (const [key, candidate] of Object.entries(value)) {
    ...
    if (tagValue !== void 0) tags.set(key, tagValue);
  }
  return tags;
}
...
function returnedActiveStatus(row, requested) {
...
function classifyCancellationError(error) {
...
var CancellationDeadlineError = class extends Error {
  constructor() {
...
async function scanPass(input) {
  ...
  while (input.coverage.pagesRead < input.coverage.maxPages) {
    ...
    for (const row of rows) {
      ...
      if (row.warehouse_id !== void 0 && row.warehouse_id !== input.warehouseId) continue;
...
function queryHistoryPage(value) {
  ...
  const record = value;
  return {
    res: Array.isArray(record.res) ? record.res : [],
...
async function createWorkspaceWarehouseCancellationTransport(input = {}) {
  ...
  const client = input.token ? new WorkspaceClient({
    host: input.host,
    token: input.token,
    authType: "pat"
  }) : new WorkspaceClient({});
...
var QUERY_HISTORY_PAGE_SIZE2 = 999;
var MAX_QUERY_HISTORY_PAGES = 40;
var MAX_QUERY_HISTORY_RANGE_MS = 30 * 24 * 60 * 60 * 1e3;
var MAX_QUERY_HISTORY_TOTAL_RANGE_MS = 366 * 24 * 60 * 60 * 1e3;
var QUERY_HISTORY_DEADLINE_MS = 2e4;
var EMPTY_WAREHOUSE_QUERY_ATTRIBUTION = {
  complete: false,
  astrolabeQueries: 0,
  totalQueries: 0,
...
function executionMilliseconds(row) {
  if (row.metrics && typeof row.metrics === "object") {
    const executionTime = finiteMilliseconds(row.metrics.execution_time_ms);
...
function interactiveRunId(row, runIds) {
  const tags = parseQueryTags(row.query_tags);
...
function genieSpaceId(row) {
  if (!row.query_source || typeof row.query_source !== "object") return "";
  const value = row.query_source.genie_space_id;
  return typeof value === "string" ? value.trim() : "";
}
function attributableUser(row) {
  const executedBy = typeof row.user_name === "string" ? row.user_name.trim().toLowerCase() : "";
  const executedAs = typeof row.executed_as_user_name === "string" ? row.executed_as_user_name.trim().toLowerCase() : "";
  if (!executedBy.includes("@") || executedBy !== executedAs) return "";
  return executedBy;
}
var QueryHistoryDeadlineError = class extends Error {
  constructor() {
...
function addReason(coverage, reason) {
...
async function readWarehouseQueryAttribution(input) {
  ...
  const askRuns = /* @__PURE__ */ new Map();
  ...
  let totalQueries = 0;
  const aggregate = (row) => {
    if (row.warehouse_id !== void 0 && row.warehouse_id !== warehouseId) {
    ...
    if (user) {
      const current = users.get(user) ?? { astrolabeExecutionMs: 0, genieSpaces: /* @__PURE__ */ new Map() };
  ...
  return {
    ...
    totalExecutionMs,
    askRuns: [...askRuns].sort(([left], [right]) => left.localeCompare(right)).map(([runId, executionMs]) => ({ runId, executionMs })),
    genieSpaces: [...genieSpaces].map(([spaceId, values]) => ({ spaceId, ...values })),
    users: [...users].sort(([left], [right]) => left.localeCompare(right)).map(([email, values]) => ({
      email,
      astrolabeExecutionMs: values.astrolabeExecutionMs,
      genieSpaces: [...values.genieSpaces].sort(([left], [right]) => left.localeCompare(right)).map(([spaceId, executionMs]) => ({ spaceId, executionMs }))
    })),
    coverage
  };
}
...
var COST_COMPONENTS = ["serving-endpoint", "sql-warehouse", "genie", "vector-search", "app-compute"];
...
var WORKSPACE_ESTIMATE_SUFFIX = ":workspace";
...
var BILLING_TAG_KEY = BILLING_TAG.key;
var BILLING_TAG_VALUE = BILLING_TAG.value;
var LIST_PRICE_SOURCE = "system.billing.list_prices";
var GENIE_LLM_UNAVAILABLE = "Genie LLM dollars unavailable by space: billing exposes surface and channel, not a Genie space ID.";
var GENIE_SQL_NOT_COMPLETE = "Only generated SQL is estimated from Query History execution time; Genie model cost is excluded.";
var TILED_PRODUCTS = /* @__PURE__ */ new Set(["MODEL_SERVING", "SQL", "VECTOR_SEARCH", "APPS"]);
var PRODUCT_REASONS = {
...
function buildCostStatement(ids, range) {
  ...
  for (const component of covered) {
    ...
    bind(marker, ids[matcher.parameter], matcher.type);
...
var ROW_KINDS = /* @__PURE__ */ new Set(["component", "coverage", "propagation", "range"]);
  ...
  for (const raw of dataArray) {
    ...
    if (wide) {
      ...
      rows.push({
        ...
        ...dbuRows === void 0 ? {} : { dbuRows: asCount(dbuRows) },
        ...billedSeconds === void 0 ? {} : { billedSeconds: asCount(billedSeconds) }
      });
      continue;
    }
...
function pricingFromRow(row) {
  ...
  let match = row.priceMatchStatus ?? "none";
...
function spendAmountFor(row, basis) {
  if (!row) return null;
...
function dbuAmountFor(row, basis) {
  if (!row) return null;
  const hasDbuEvidence = row.dbuRows === void 0 ? row.usageUnitCount === 1 : row.dbuRows > 0;
...
function buildHonesty(range, meta, tiles) {
...
function buildCoverage(input) {
  ...
  const propagation = input.propagationRows.filter((row) => row.component !== "JOBS").map((row) => {
...
function buildTiles(ids, rows, warehouseAttribution = EMPTY_WAREHOUSE_QUERY_ATTRIBUTION, resourceActivity = [], genie, genieReason = "", marginal) {
  ...
  for (const component of COST_COMPONENTS) {
    ...
    tiles.push(
      component === "serving-endpoint" && marginal ? marginalServingTile(tile, byComponent.get(component), marginal.interactive) : tile
    );
    if (component === "serving-endpoint") {
      tiles.push(
        marginal?.foundation ?? {
          ...
          unavailable: ids.foundationModel ? "Foundation-model billing was not read." : "Configured foundation model unavailable.",
          remedy: "",
          note: "",
          evidence: {
            billingRows: null,
...
function validRunDurationMs(run) {
...
function marginalServingTile(full, row, interactive) {
  ...
  return {
    ...
    unavailable: factor === null ? complete ? "Marginal serving cost needs priced endpoint billing intervals." : "Interactive Ask timing coverage is incomplete; full endpoint uptime is excluded." : "",
...
function genieAccountingTiles(ids, accounting, reason) {
  if (!accounting) {
    const spaces = ids.genieSpaces.length > 0 ? ids.genieSpaces : [{ id: "", label: "Unattributed Genie", tileId: "genie:unattributed" }];
  ...
  const tiles = period.instances.map((instance) => {
    const pricing = {
      ...EMPTY_PRICING,
      source: "list_prices",
    ...
    return {
      ...
      quality: instance.paidUsd === null ? "unknown" : instance.attribution === "query-history-allocation" ? "estimate" : "real",
      ...
      pricing,
      unavailable: instance.paidUsd === null ? "Charged Genie DBUs are measured, but USD is unavailable because pricing is incomplete." : "",
      remedy: "",
      note: instance.attribution === "query-history-allocation" ? `Allocated by user-day Query History execution share · billing through ${period.throughDay || "unavailable"}.` : `Matched by user-day Query History to this configured space · billing through ${period.throughDay || "unavailable"}.`,
      evidence: { billingRows: null, astrolabeQueries: null },
...
function componentTile(component, ids, byComponent, warehouseAttribution, resourceActivity) {
  ...
  if (!canAsk(component, ids)) {
    ...
    if (component === "vector-search") {
      ...
      const identityReason = !configuredIndex ? ids.vectorIdentityError || "The active Vector Search index was not carried into this app release." : !ids.vectorEndpoint ? ids.vectorIdentityError || "The active index did not identify its hosting endpoint." : ids.vectorEndpointIndexCount === null || ids.vectorEndpointIndexCount === void 0 ? ids.vectorIdentityError || "The hosting endpoint index count could not be read." : ids.vectorEndpointIndexCount > 1 ? `The hosting endpoint serves ${ids.vectorEndpointIndexCount} indexes; billing exposes endpoint identity only.` : ids.vectorIdentityError || "The active index-to-endpoint relationship was not established.";
      return withMeta({
        ...base,
        quality: "unknown",
        amount: null,
  ...
  const measuredActivity = component === "vector-search" ? resourceActivity.find((item) => item.tileId === "vector-search") : void 0;
  const evidence = {
    billingRows,
  ...
  if (component === "vector-search") {
    ...
    return withMeta({
      ...
      pricing,
      note: [
        noUsage ? "No billable usage in this period" : "Billing is endpoint-level; this endpoint hosts only the active index.",
        ids.vectorIdentityDrift
      ].filter(Boolean).join(" "),
      unavailable: amount === null && dbus === null ? unpricedUnavailable(pricing) || "No billing rows matched the hosting endpoint" : "",
      remedy: "",
      evidence
    });
  }
  if (component === "sql-warehouse") {
    ...
    return withMeta({
      ...
      dbus: dbus === null ? null : dbus * warehouseAttribution.astrolabeExecutionMs / warehouseAttribution.totalExecutionMs,
      pricing,
      note: "",
      unavailable: "",
      remedy: "",
...
function unknownPart(id, label, unavailable) {
  return { id, label, quality: "unknown", amount: null, unavailable };
}
function buildQuestionAttribution(runs, tiles, limit, warehouseAttribution = EMPTY_WAREHOUSE_QUERY_ATTRIBUTION) {
  ...
  const durationByRun = new Map(
    runs.map((run) => [run.runId, validRunDurationMs(run)]).filter((entry) => entry[1] !== null)
  );
  ...
  const attributed = newest.map((run) => {
    const parts = [];
    if ((durationByRun.has(run.runId) || !timingReported && run.totalTokens !== null) && typeof servingSpend === "number" && Number.isFinite(servingSpend) && (totalDurationMs > 0 || !timingReported && totalRecordedTokens > 0)) {
      parts.push({
        id: "serving-endpoint",
        label: "Model serving",
        /*
        ...
        amount: timingReported ? servingSpend * (durationByRun.get(run.runId) ?? 0) / totalDurationMs : servingSpend * (run.totalTokens ?? 0) / totalRecordedTokens,
        unavailable: ""
      });
    } else {
      parts.push(
        unknownPart(
          "serving-endpoint",
          "Model serving",
          !durationByRun.has(run.runId) ? "This run has no complete request interval." : "No marginal serving spend was measured."
        )
      );
    }
    if (run.totalTokens !== null && run.totalTokens >= 0 && typeof foundationSpend === "number" && Number.isFinite(foundationSpend) && totalRecordedTokens > 0) {
      parts.push({
        id: "foundation-model",
        label: "Foundation model tokens",
        quality: "per-token",
    ...
    const legacySql = warehouseAttribution.askRuns === void 0 || warehouseAttribution.askRuns.length === 0;
    if (typeof sqlSpend === "number" && Number.isFinite(sqlSpend) && (runSqlMs > 0 && totalAskSqlMs > 0 || legacySql && runsInRange > 0)) {
      parts.push({
        id: "sql-warehouse",
        label: "Ask SQL",
        quality: "estimate",
    ...
    return {
      ...
      ...run.cachedReadTokens === void 0 ? {} : { cachedReadTokens: run.cachedReadTokens },
      ...run.cacheWriteTokens === void 0 ? {} : { cacheWriteTokens: run.cacheWriteTokens },
      parts
    };
  });
  return {
...
export {
  ...
  WORKSPACE_ESTIMATE_SUFFIX,
  workspaceEstimateRow,
  RANGE_ROW,
  BILLING_TAG_KEY,
  BILLING_TAG_VALUE,
  LIST_PRICE_SOURCE,
  GENIE_LLM_UNAVAILABLE,
  GENIE_SQL_NOT_COMPLETE,
  EMPTY_PRICING,
  canAsk,
  resourceIdFor,
  vectorIndexName,
