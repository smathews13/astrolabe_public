...
test('a table the answer declared becomes a link; nothing else does', async ({ page }) => {
  ...
  // The source row below the answer names the same table and is the same link.
  // `.sources-row` since the strip and the "All sources" tab became one module.
  await expect(page.locator(`.sources-row a[data-entity="${DAILY}"]`)).toBeVisible();
