 ...
 * about the demo answer; it is the disclosure applied to ANY answer that carries
 * figures, SQL or stage timings without an MLflow trace id behind them, which a
 * live run can also produce. `discloseAnswerProvenance` adds it in
 * server/routes/insights-routes.ts and the browser renders it as one of the
 * answer's caveats. Answers stored before the removal still carry it in
 * `response_json`, and those rows have to keep rendering with the disclosure
 ...
 * nothing recorded the run, so the figures cannot be traced back to one. It makes
 * no claim about whether the data is real. `SYNTHETIC_DATA_CAVEAT` in
 * `agent/agent.py` is where that claim belongs, and only a deployment that
 * declared it gets it.
 */
export const REPRESENTATIVE_ANSWER_CAVEAT =
  'No MLflow trace was recorded for this answer, so the figures, SQL, and stage timings shown here cannot be traced back to the run that produced them.';
