...
function setupMonitoringRoutes(appkit, deps) {
  ...
  appkit.server.extend((app) => {
    ...
    app.get("/api/monitoring/people/:email", async (req, res) => {
      ...
      const assignedPersona = assignment ? personaCatalog.find((entry) => entry.id === assignment.personaId) : void 0;
      const assignedPersonaName = assignedPersona?.displayName.trim() ?? "";
      const persona = assignedPersona && assignedPersonaName && !/^(no persona|none|unassigned|n\/a|null|unknown)$/i.test(assignedPersonaName) ? { id: assignedPersona.id, name: assignedPersonaName } : null;
      const payload = {
        email: person,
        role,
        persona,
