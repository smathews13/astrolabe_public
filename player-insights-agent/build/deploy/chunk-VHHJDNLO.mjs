...
function normalizeWorkspaceHost(raw) {
  const trimmed = (raw ?? "").trim().replace(/\/+$/, "");
  if (!trimmed) return "";
  return /^https?:\/\//i.test(trimmed) ? trimmed : `https://${trimmed}`;
}
function workspaceAppsUrl(host, workspaceId) {
...
function accountConsoleUrlForWorkspace(host) {
  const hostname = (() => {
    try {
...
function unityCatalogPath(name, part, segment = "") {
  const parts = name.trim().split(".").filter((piece) => piece.length > 0);
  return parts.length === 3 ? `/explore/data/${segment}${parts.map(part).join("/")}` : null;
}
function workspacePath(object) {
  ...
  switch (object.kind) {
    ...
    case "schema":
      return object.catalog.trim() && object.schema.trim() ? `/explore/data/${part(object.catalog)}/${part(object.schema)}` : null;
    case "experiment":
...
function databricksLink(host, object) {
  const base = normalizeWorkspaceHost(host);
  if (!base) return null;
