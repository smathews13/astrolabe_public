...
function buildUserMonitoringPage(input) {
  ...
  const rows = [...emails].filter((email) => email.includes("@")).map((email) => {
  ...
  }).filter((row2) => (!search || row2.email.includes(search)) && (!input.role || row2.role === input.role)).sort((left, right) => {
...
async function queryMeasurement(appkit, req, period, now) {
  ...
  } = await import("./ops-routes-H2NMCDKS.mjs");
  ...
  const built = (await import("./ops-billing-AP7EVYIH.mjs")).buildCostStatement(resolved.ids, range);
...
async function readAppBudgetStatus(appkit, req, options = {}) {
  ...
  try {
    ...
    return unavailable2(period, readAt2, {
      ...
      detail: `Budget status unavailable: attributable month-to-date billing failed to read (${error.message}). New questions remain available.`,
      fingerprint,
      unit: configured[0].unit,
      budget: configured[0].budget
    });
  ...
  if (!measurement) {
    return unavailable2(period, readAt2, {
      ...
      detail: "Budget status unavailable: attributable month-to-date billing could not be read. New questions remain available.",
      fingerprint,
      unit: configured[0].unit,
      budget: configured[0].budget
    });
  ...
  if (complete.length === 0) {
    ...
    return unavailable2(period, measurement.readAt, {
      ...
      detail: "Budget status unavailable/partial: current attributable coverage is incomplete or billing is delayed. New questions remain available.",
      fingerprint,
      unit: first.unit,
      budget: first.budget,
