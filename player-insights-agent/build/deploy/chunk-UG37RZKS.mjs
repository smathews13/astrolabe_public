...
function resolveApplyPlan(input) {
  ...
  const target2 = text(input.target) || "<your-target>";
  return {
    knobs,
    notes,
    hasOverrides: knobs.length > 0,
    command: `TARGET=${target2} bundle/apply-declaration.sh --apply --i-am-deploying`
  };
}
...
function metadataOnly(kind, name, label, detail, nextAction) {
...
function resourceTagInventory(input = {}) {
  ...
  const warehouseId = text5(environment.DATABRICKS_SQL_WAREHOUSE_ID);
  if (warehouseId) {
    targets.push(
      target({
        kind: "sql-warehouse",
  ...
  const lakebase = text5(environment.LAKEBASE_ENDPOINT);
...
function tagStateDetail(state, retired = false) {
...
function technicalDetail(error) {
  return errorText(error).replace(/Bearer\s+[A-Za-z0-9._~+/=-]+/gi, "Bearer [redacted]").replace(/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi, "[principal redacted]").replace(/(client[_ -]?secret|access[_ -]?token|authorization)\s*[:=]\s*[^\s,}]+/gi, "$1=[redacted]").slice(0, 2e3);
}
function isPermissionError(error) {
  return errorStatus(error) === 401 || errorStatus(error) === 403 || /PERMISSION_DENIED|FORBIDDEN|UNAUTHORI[ZS]ED|INSUFFICIENT_SCOPE/i.test(`${errorCode(error)} ${errorText(error)}`);
}
...
async function insideDeadline(operation, deadline, now, signal) {
...
async function retryTransient(operation, policy, signal) {
...
function permissionResult(target0, error) {
  const appIdentity = target0.identity === "app-service-principal";
  const subject = appIdentity ? `the Astrolabe app service principal CAN_MANAGE on AI Search endpoint “${target0.name}”` : target0.kind === "serving-endpoint" ? `CAN_MANAGE on serving endpoint “${target0.name}”` : target0.kind === "sql-warehouse" ? `CAN_MANAGE or IS_OWNER on SQL warehouse “${target0.name}”` : target0.kind === "lakebase-project" ? `owner or management permission on Lakebase project “${target0.name.replace(/^projects\//, "")}”` : `CAN_MANAGE on AI Search endpoint “${target0.name}”`;
  const scope = !appIdentity && target0.requiredScope ? ` Ensure the app requests \`${target0.requiredScope}\`, then restart it.` : "";
  return {
    ...target0,
    status: "permission-required",
    detail: appIdentity ? `A workspace administrator must grant ${subject}.` : `The signed-in administrator needs ${subject}.${scope}`,
    nextAction: `Grant ${subject}.${scope}`,
    technicalDetail: error ? technicalDetail(error) : void 0
  };
}
function failedResult(target0, error) {
  if (isPermissionError(error)) return permissionResult(target0, error);
  return {
    ...target0,
    status: "failed",
    detail: isRetryable(error) ? "Databricks did not complete this retryable operation within the bounded retry policy." : "Databricks rejected this tag operation.",
    nextAction: isRetryable(error) ? "Retry this unresolved resource." : "Open technical details and correct the request.",
    technicalDetail: technicalDetail(error)
  };
}
function staticResult(target0) {
  return {
    ...target0,
    status: target0.support === "unsupported" ? "unsupported" : "not-applicable"
  };
}
function requiredScopePresent(token, required) {
  if (!required) return true;
  const scopes = scopesFromToken(token);
  if (!scopes) return true;
  return scopes.includes("all-apis") || scopes.includes(required);
}
async function tagSupported(target0, platform, signal) {
...
async function mapConcurrent(values, concurrency, signal, mapper) {
...
function resultKey(value) {
  return `${value.kind}\0${value.name}`;
}
function summarize(results, updatedAt = (/* @__PURE__ */ new Date()).toISOString()) {
...
async function applyAstrolabeTags(input) {
  ...
  const platform = input.platform ?? (token ? createWorkspaceTagPlatform({
  ...
  const policy = {
    ...
    baseDelayMs: Math.max(0, Math.min(2e3, input.retry?.baseDelayMs ?? 250)),
    timeBudgetMs: Math.max(1e3, Math.min(15e3, input.retry?.timeBudgetMs ?? 12e3)),
  ...
  let vectorPlatform = input.vectorPlatform ?? input.platform ?? null;
  let vectorPlatformError;
  ...
  if (index) {
    ...
    let endpointName = mode === "unresolved" && priorIndex?.name === index.name && priorEndpoint?.label.startsWith("AI Search endpoint · ") ? priorEndpoint.name : "";
  ...
  for (const item of inventory) {
    ...
    const missingObo = item.identity !== "app-service-principal" && (!token || !requiredScopePresent(token, item.requiredScope));
    ...
    async ({ target: item, platform: selectedPlatform }) => {
      try {
        return await retryTransient(
          () => tagSupported(item, selectedPlatform, input.signal),
          policy,
          input.signal
        );
      } catch (error) {
...
var DatabricksTagError = class extends Error {
  constructor(message, status, code, retryAfterMs2) {
    super(message);
    this.status = status;
    this.code = code;
...
function createWorkspaceTagPlatform(input) {
  ...
  const request = async (path, options = {}) => {
    ...
    const body = await response.json().catch(() => ({}));
    if (!response.ok) {
      const message = typeof body.message === "string" ? body.message : typeof body.error === "string" ? body.error : `Databricks answered HTTP ${response.status}.`;
  ...
  return {
    async getServingTags(name, signal) {
      const body = await request(`/api/2.0/serving-endpoints/${encode(name)}`, { signal });
      return body.tags ?? [];
    },
    ...
    async getWarehouseTags(id, signal) {
      const body = await request(`/api/2.0/sql/warehouses/${encode(id)}`, {
        signal
      });
    ...
    async getLakebaseTags(name, signal) {
      const body = await request(`/api/2.0/postgres/${name}`, { signal });
    ...
    async getVectorIndexEndpoint(indexName, signal) {
      const body = await request(`/api/2.0/vector-search/indexes/${encode(indexName)}`, {
        signal
      });
...
async function createAppVectorTagPlatform() {
  const { WorkspaceClient } = await import("./vendor-databricks-sdk-experimental.mjs");
...
// server/lib/resource-tag-state.ts
import { randomUUID } from "node:crypto";
var RESOURCE_TAG_RESULT_ID = "resource-tags-current-result";
var RESOURCE_TAG_RESULT_MAX_BYTES = 64 * 1024;
var StoredResultSchema = external_exports.object({
  kind: external_exports.string().max(80),
  name: external_exports.string().max(1024),
  label: external_exports.string().max(1200),
  support: external_exports.enum(["supported", "unsupported", "not-applicable"]),
  billingAttribution: external_exports.boolean(),
  status: external_exports.enum(["tagged", "already-correct", "permission-required", "failed", "unsupported", "not-applicable"]),
  detail: external_exports.string().max(2e3),
  nextAction: external_exports.string().max(2e3),
  requiredScope: external_exports.string().max(100).optional(),
  identity: external_exports.enum(["obo", "app-service-principal"]).optional(),
  technicalDetail: external_exports.string().max(2e3).optional()
});
var StoredSummarySchema = external_exports.object({
  headline: external_exports.string().max(500),
  supportedTotal: external_exports.number().int().nonnegative(),
  supportedCovered: external_exports.number().int().nonnegative(),
  tagged: external_exports.number().int().nonnegative(),
  alreadyCorrect: external_exports.number().int().nonnegative(),
  supportedFailed: external_exports.number().int().nonnegative(),
  permissionRequired: external_exports.number().int().nonnegative(),
  unsupported: external_exports.number().int().nonnegative(),
  notApplicable: external_exports.number().int().nonnegative(),
  results: external_exports.array(StoredResultSchema).max(32),
  updatedAt: external_exports.string().max(100)
});
var READ = `SELECT value
  FROM ${APP_SCHEMA}.deployment_settings
  WHERE resource_id = $1`;
var WRITE = `INSERT INTO ${APP_SCHEMA}.deployment_settings
...
var CLEAR_RESOURCE_TAG_RESULT = `WITH deleted AS (
...
function parseSummary(value) {
  ...
  try {
    return StoredSummarySchema.parse(JSON.parse(value));
  } catch {
    return null;
  }
}
async function readResourceTagResult(store) {
  const result = await store.lakebase.query(READ, [RESOURCE_TAG_RESULT_ID]);
  return parseSummary(result.rows[0]?.value);
}
async function writeResourceTagResult(store, summary, actor) {
  let serialized;
  try {
...
async function clearResourceTagResult(store, actor) {
...
// server/routes/resource-tag-routes.ts
var ApplyBody = external_exports.strictObject({
  mode: external_exports.enum(["unresolved", "full"]).default("unresolved")
...
function setupResourceTagRoutes(appkit, dependencies) {
  ...
  appkit.server.extend((app) => {
    app.get("/api/settings/resource-tags", async (_req, res) => {
      try {
        ...
        console.error("[resource-tags] Saved result could not be read:", error.message);
        res.status(503).json({
    ...
    app.post("/api/settings/resource-tags", async (req, res) => {
      ...
      try {
        ...
        console.error("[resource-tags] Apply failed before a result could be saved:", error.message);
        res.status(503).json({
    ...
    app.delete("/api/settings/resource-tags", async (req, res) => {
      try {
        ...
        console.error("[resource-tags] Clear failed:", error.message);
        res.status(503).json({
...
async function notebookAgentSyncEnabled(appkit) {
  try {
...
async function buildApplyResponse(req, appkit) {
  ...
  const target2 = textEnv(process.env.PLAYER_INSIGHTS_TARGET) || textEnv(process.env.DATABRICKS_BUNDLE_TARGET) || "<your-target>";
  const plan = resolveApplyPlan({ intended, notebook, target: target2 });
