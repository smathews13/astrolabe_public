...
import {
  alignGuidelinesToHumans,
  loadCasesForAlignment,
  promotePromptAlias,
  promptTemplateFromPromote,
  readLabState,
  snapshotWorkingCopy
} from "./chunk-XPFBHDUB.mjs";
import {
...
async function startLabelingSession(client, input) {
  const name = input.name.trim() || `PIA SME review ${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}`;
...
function createSqlExecutor(options) {
  ...
  return async (sql) => {
    ...
    try {
      ...
      return {
        ok: false,
        note: timedOut ? "SQL cancelled or timed out after 50s." : `The SQL warehouse could not be reached: ${error.message}`
      };
    }
  };
}
...
var GENIE_ACCURACY_POLL_MS = 2e3;
var GENIE_ACCURACY_TIMEOUT_MS = 9e4;
var MissingSqlGateError = class extends Error {
  missing;
  selected;
...
function extractGenieSql(payload) {
  ...
  if (Array.isArray(attachments)) {
    for (const entry of attachments) {
      ...
      const sql = text2(query.query) || text2(query.sql) || text2(attachment.query) || text2(attachment.sql);
      if (sql) return sql;
    }
  }
  return text2(message.query) || text2(root.query) || text2(root.sql);
}
function extractGenieResultTable(payload) {
  ...
  for (const entry of attachments) {
    ...
    const names = columnsRaw.map((columnEntry, index) => {
      ...
      const column = asRecord2(columnEntry);
      return text2(column?.name) || text2(column?.display_name) || `col_${index}`;
    });
...
function messageStatus(payload) {
  const root = asRecord2(payload);
  const message = asRecord2(root?.message) ?? root;
  return text2(message?.status).toUpperCase();
}
function conversationIds(payload) {
  ...
  return {
    conversationId: text2(conversation?.id) || text2(root.conversation_id),
    messageId: text2(message?.id) || text2(root.message_id)
  };
}
async function jsonRequest(call, url, token, method, body) {
  ...
  const payload = await response.json().catch(() => ({})) ?? {};
  if (!response.ok) {
    const detail = text2(payload.message) || text2(payload.error) || `workspace returned HTTP ${response.status}`;
...
async function resolveTable(sql, already, executor) {
...
function excludedCase(row, note, durationMs, extras = {}) {
...
function workspaceMonitorFromError(error) {
...
var GenieAccuracyBody = external_exports.object({
...
var LastSuiteBody = LastSuiteSchema.extend({
  ...
  appkit.server.extend((app) => {
    ...
    app.post("/api/admin/benchmarks/live-monitoring", async (req, res) => {
      ...
      try {
        ...
        workspace = {
          status: "blocked",
          note: `Workspace monitoring could not be reached: ${error.message} Sampled Ask turns are still scored in this app.`,
          scorers: []
        };
      }
    ...
    app.put("/api/admin/benchmarks/dataset", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          detail: `The dataset was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/dataset/curate", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          error: "eval_dataset_store_unavailable",
          detail: `Those questions were not added: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/align-guidelines", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `Guidelines were not updated: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/promote", async (req, res) => {
      ...
      try {
        ...
        if (targetKind === "prompt-registry") {
          try {
            ...
            promotedPrompt = {
              ...
              note: promptName ? `The production alias was not moved: ${error.message} The next Ask still uses the guidance saved from this promote.` : "No Prompt Registry name is set. Next Ask still uses the saved guidance from this promote."
            };
          }
        } else {
          promotedPrompt = {
        ...
        res.status(503).json({
          ...
          message: `The winner was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/review-app", async (req, res) => {
      ...
      try {
        ...
        session = {
          ...
          url: "",
          status: "blocked",
          note: `Review App could not be started: ${error.message} SMEs can still label thumbs and SQL correct on this tab.`,
      ...
      try {
        ...
        res.status(session.status === "open" ? 200 : 503).json({
          session,
          message: session.note || error.message
        });
      }
    });
    app.post("/api/admin/benchmarks/score-thread", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `That thread was not scored: ${error.message}`
        });
      }
    });
    app.put("/api/admin/benchmarks/prompt-registry", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The Prompt Registry name was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/last-suite", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The last suite was not saved: ${error.message}`
        });
      }
    });
    ...
    app.post("/api/admin/benchmarks/rollback", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `The rollback was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/compare-history", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `Bake-off history was not saved: ${error.message}`
        });
      }
    });
    app.post("/api/admin/benchmarks/known-failure", async (req, res) => {
      ...
      try {
        ...
        res.status(503).json({
          ...
          message: `That case was not marked: ${error.message}`
        });
      }
    });
    app.post("/api/benchmarks/genie-accuracy", async (req, res) => {
      ...
      try {
        ...
        console.warn("[eval-dataset] Last Genie run was not saved:", error.message);
      }
      try {
        const current = await readFlywheelState(appkit, { maxAgeMs: 0 });
        ...
        console.warn("[eval-flywheel] Accuracy history was not saved:", error.message);
      }
      res.json({ run });
    });
  });
