...
var SEMANTIC_INDEX_VARIABLE = "PLAYER_INSIGHTS_SEMANTIC_INDEX";
var ARCHITECTURE_EXPERIMENT_TIMEOUT_MS = 2e3;
...
function semanticIndexReport() {
  return {
    ...
    reason: "It is resolved when the model is logged and baked into the artifact, so changing it means logging a new model version rather than editing a setting here. A version logged before the endpoint reported this setting cannot say whether it has an index."
  };
}
...
function setupArchitectureRoutes(appkit) {
