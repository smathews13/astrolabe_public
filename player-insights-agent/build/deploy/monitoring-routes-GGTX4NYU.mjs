...
import{ANSWER_LANDED_SQL,APP_ACTIVITY_TABLE,APP_SESSION_TABLE,PLAN_APPROVAL_MESSAGE,PROSE_ONLY_DEGRADED_SQL,VERDICT_STAGE_EXEMPTION_SQL,appSessionDeployment,bindSynthesisIncompleteSql,chooseRows,markResponse,mlflowReference,overlayJoinSql,readStored,runRuntimeUsedFromStored,userEmail,workspaceLinksAllowed}from"./chunk-BGG4I77R.mjs";import{FAILURE_TAXONOMY,accessDependenciesFrom,executionToken,isFailureCode,listSpAssignments,listSpPersonas,statementRunnerFor,verifyTableAccess}from"./chunk-XPMTENJB.mjs";import{resolveExperimentId}from"./chunk-NCE55VGM.mjs";import{ExpiringLruCache}from"./chunk-66G4LGYE.mjs";import{normalizeWorkspaceHost}from"./chunk-3KZTQDF5.mjs";import"./chunk-6R3N3T2P.mjs";import{effectiveRole,invalidAdminEmail,readRosterForRequest,seedRoles}from"./chunk-RM4E52P7.mjs";import"./chunk-3LJPB2Y3.mjs";import"./chunk-K3M7H2A2.mjs";import{isMlflowTraceId}from"./chunk-TOE5V7KL.mjs";import"./chunk-DDLERORI.mjs";import{APP_SCHEMA}from"./chunk-YKLXTEAM.mjs";import"./chunk-A7SHUGSC.mjs";var OUTCOME_BY_STATE={REFUSED:"refused",FAILED:"failed",DEADLINE_EXCEEDED:"failed",PERSISTENCE_FAILED:"failed",CLARIFICATION_REQUIRED:"partial",CANCELLED:"partial"};function classifyOutcome(input){const state=(input.runState??"").trim().toUpperCase();const writerMissed=state==="DEADLINE_EXCEEDED"||input.synthesisIncomplete===true;if(input.answerLanded&&writerMissed&&state!=="REFUSED"){return"partial"}if(state&&OUTCOME_BY_STATE[state])return OUTCOME_BY_STATE[state];if(input.proseOnlyDegraded)return"partial";if(state&&state!=="SUCCEEDED")return"partial";if(input.answerLanded&&(state==="SUCCEEDED"||input.hasStoredAnswer)){return"completed"}if(input.traceHasFailedStage)return"failed";if(input.traceHasPartialStage)return"partial";if(state==="SUCCEEDED"||input.hasStoredAnswer)return"completed";return"partial"}function applyAdminOutcome(classified,overlayStatus){const word=(overlayStatus??"").trim().toLowerCase();if(word==="complete"||word==="completed")return"completed";if(word==="partial")return"partial";if(word==="failed")return"failed";if(word==="refused")return"refused";return classified}function applyAdminRating(classified,overlayRating){const word=(overlayRating??"").trim().toLowerCase();if(word==="unrated")return null;if(word==="up"||word==="down")return word;return classified}var CAUSE_BY_LAYER={authorization:"missing-grant",governance:"agent-rules",evidence:"agent-rules"};function classifyRefusal(code){const value=(code??"").trim();if(!isFailureCode(value))return"other";return CAUSE_BY_LAYER[FAILURE_TAXONOMY[value].layer]??"other"}function refusalSentence(code){const value=(code??"").trim();return isFailureCode(value)?FAILURE_TAXONOMY[value].uiMessage:null}var GRANT_CACHE_TTL_MS=3e4;var GRANT_CACHE_MAX_ENTRIES=256;function unresolvedGrants(now){return{resolved:false,verdicts:new Map,resolvedAt:now}}function conditioningFor(tables,grants){if(!grants.resolved)return null;for(const table of tables){const verdict=grants.verdicts.get(table);if(!verdict||verdict.status!=="denied")continue;return{table:verdict.missing?.object??table,permission:verdict.missing?.permission??"SELECT"}}return null}function cacheKey(key){return`${key.admin.trim().toLowerCase()}\0${key.window}`}var cache=new ExpiringLruCache(GRANT_CACHE_MAX_ENTRIES,GRANT_CACHE_TTL_MS);async function resolveGrants(options){const now=options.now??Date.now();const ttl=options.ttlMs??GRANT_CACHE_TTL_MS;const id=cacheKey(options.key);const cached=cache.get(id,now);if(cached)return cached;if(options.tables.length===0){const empty={resolved:true,verdicts:new Map,resolvedAt:now};cache.set(id,empty,now,ttl);return empty}if(!options.probe){const failed=unresolvedGrants(now);cache.set(id,failed,now,ttl);return failed}let outcome;try{outcome=await(options.verify??verifyTableAccess)(options.tables,options.probe,options.key.admin)}catch(error){console.warn(`[monitoring] Table permissions could not be resolved for ${options.key.admin}: ${error.message}. Everything is shown, and the page says the check could not run.`);const failed=unresolvedGrants(now);cache.set(id,failed,now,ttl);return failed}if(outcome.blocked){console.warn(`[monitoring] Table permissions not established for ${options.key.admin}: ${outcome.blocked.kind}. Everything is shown.`);const failed=unresolvedGrants(now);cache.set(id,failed,now,ttl);return failed}const verdicts=new Map;for(const verdict of outcome.verdicts)verdicts.set(verdict.table,verdict);const resolution={resolved:true,verdicts,resolvedAt:now};cache.set(id,resolution,now,ttl);return resolution}var EFFECTIVE_PERMISSIONS_PATH="/api/2.1/unity-catalog/effective-permissions/table";var TABLE_PATH="/api/2.1/unity-catalog/tables";var READ_PRIVILEGE="SELECT";var TABLE_POLICY_TTL_MS=10*6e4;var TABLE_POLICY_CACHE_MAX_ENTRIES=512;var PERSON_PRIVILEGE_TTL_MS=3e4;var PERSON_PRIVILEGE_CACHE_MAX_ENTRIES=2048;var tablePolicies=new ExpiringLruCache(TABLE_POLICY_CACHE_MAX_ENTRIES,TABLE_POLICY_TTL_MS);var personPrivileges=new ExpiringLruCache(PERSON_PRIVILEGE_CACHE_MAX_ENTRIES,PERSON_PRIVILEGE_TTL_MS);var inFlight=new Map;function once(key,work){const running=inFlight.get(key);if(running)return running;const started=work().finally(()=>inFlight.delete(key));inFlight.set(key,started);return started}async function readTableGrant(read,table,principal,now=Date.now()){const reading={table,canRead:null,missing:null,rowFilter:null,maskedColumns:null};const personKey=`${table}\0${principal.trim().toLowerCase()}`;const heldPrivileges=personPrivileges.get(personKey,now);if(heldPrivileges){reading.canRead=heldPrivileges.canRead;reading.missing=heldPrivileges.missing}else{try{const body=await once(`p:${personKey}`,()=>read(`${EFFECTIVE_PERMISSIONS_PATH}/${encodeURIComponent(table)}`,{principal}));const privileges=privilegesFrom(body,principal);if(privileges!==null){reading.canRead=privileges.includes(READ_PRIVILEGE);reading.missing=reading.canRead?null:`${READ_PRIVILEGE} missing`;personPrivileges.set(personKey,{canRead:reading.canRead,missing:reading.missing},now)}}catch(error){console.warn(`[monitoring] Effective permissions on ${table} could not be read: ${error.message}`)}}const heldPolicies=tablePolicies.get(table,now);if(heldPolicies){reading.rowFilter=heldPolicies.rowFilter;reading.maskedColumns=heldPolicies.maskedColumns}else{try{const body=await once(`t:${table}`,()=>read(`${TABLE_PATH}/${encodeURIComponent(table)}`));const policies=policiesFrom(body);reading.rowFilter=policies.rowFilter;reading.maskedColumns=policies.maskedColumns;tablePolicies.set(table,policies,now)}catch(error){console.warn(`[monitoring] Row filter and masks on ${table} could not be read: ${error.message}`)}}return reading}function privilegesFrom(body,principal){if(!body||typeof body!=="object")return null;const assignments=body.privilege_assignments;if(!Array.isArray(assignments))return null;const wanted=principal.trim().toLowerCase();const entries=assignments;const match=entries.find(entry=>{const owner=entry.principal;return typeof owner==="string"&&owner.trim().toLowerCase()===wanted});if(!match)return null;const privileges=match.privileges;if(!Array.isArray(privileges))return null;return privileges.map(entry=>{if(typeof entry==="string")return entry;const named=entry.privilege;return typeof named==="string"?named:""}).filter(value=>value!=="").map(value=>value.toUpperCase())}function policiesFrom(body){if(!body||typeof body!=="object")return{rowFilter:null,maskedColumns:null};const table=body;const rowFilter=table.row_filter!==void 0&&table.row_filter!==null?true:false;if(!Array.isArray(table.columns))return{rowFilter,maskedColumns:null};const masked=table.columns.filter(column=>{const mask=column.mask;return mask!==void 0&&mask!==null}).map(column=>{const name=column.name;return typeof name==="string"?name:""}).filter(name=>name!=="");return{rowFilter,maskedColumns:masked}}var QUESTION_PAGE_SIZE=50;var QUESTION_READ_LIMIT=100;var MONITORING_TOP_TABLE_LIMIT=5;var OFFSET_REFUSAL="Use the opaque cursor from pagination.nextCursor instead of an offset.";function encodeCursor(cursor){return Buffer.from(JSON.stringify(cursor),"utf8").toString("base64url")}function decodeCursor(raw){if(!raw)return null;try{const parsed=JSON.parse(Buffer.from(raw,"base64url").toString("utf8"));const askedAt=typeof parsed.askedAt==="string"?new Date(parsed.askedAt).toISOString():"";const id=typeof parsed.id==="string"?parsed.id.trim():"";return askedAt&&id?{askedAt,id}:null}catch{return null}}function monitoringCursor(askedAt,id){return encodeCursor({askedAt:new Date(askedAt).toISOString(),id})}function pageFrom(req){const limit=Number.parseInt(queryString(req.query.limit),10);const offset=Number.parseInt(queryString(req.query.offset),10);const rawCursor=queryString(req.query.cursor).trim();const cursor=decodeCursor(rawCursor);return{limit:Number.isFinite(limit)&&limit>0?Math.min(limit,QUESTION_READ_LIMIT):QUESTION_PAGE_SIZE,cursor,refusal:Number.isFinite(offset)&&offset>0?OFFSET_REFUSAL:rawCursor&&!cursor?"The Monitoring page cursor is invalid. Start again without a cursor.":""}}var MONITORING_QUESTIONS_QUERY=`
  WITH page AS (
