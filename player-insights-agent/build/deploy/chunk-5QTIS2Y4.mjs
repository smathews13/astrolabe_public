...
async function loadCasesForAlignment(client, runIds) {
  ...
  for (const id of runIds) {
    ...
    try {
      ...
      const listed = Array.isArray(metrics?.cases) ? metrics.cases : [];
      cases.push(...listed);
    } catch {
    }
  }
...
function guidelinesFromAlignBody(body) {
  ...
  const record = body;
  const instructions = record.instructions ?? record.guidelines ?? record.judge?.instructions ?? record.judge?.guidelines;
...
async function rewriteGuidelinesWithJudge(invoke, base, pairs) {
  ...
  if (input.invokeJudge) {
    try {
      ...
      return {
        ...
        agreement,
        method: "distill",
        note: `The judge model could not rewrite the rubric (${error.message}). Distilled replacement guidelines from the labels instead. ${agreementLine(agreement)}`,
        pairs
      };
    }
  }
...
function parsePromptVersion(body) {
  if (!body || typeof body !== "object") return "";
  const record = body;
  const version = record.version ?? record.prompt_version ?? record.prompt?.version;
  return version !== void 0 && version !== null ? String(version) : "";
}
function parsePromptTemplate(body) {
  if (!body || typeof body !== "object") return "";
  const record = body;
  const template = record.template ?? record.prompt_template ?? record.prompt?.template;
...
async function registerPromptVersion(client, name, template) {
...
async function setProductionAlias(client, name, version) {
...
async function loadProductionPrompt(client, name) {
  const encoded = encodeURIComponent(name);
  const alias = PRODUCTION_PROMPT_ALIAS;
  const paths = [
    `/api/2.0/mlflow/unity-catalog/prompts/${encoded}/aliases/${encodeURIComponent(alias)}`,
...
async function promotePromptAlias(client, input) {
  ...
  try {
    ...
    return {
      ...
      template,
      status: "blocked",
      note: `The production alias was not moved: ${error.message} Apps often cannot write Prompt Registry. The next Ask still uses the guidance saved from this promote.`
    };
  }
}
...
var KEY = "effective";
var BENCHMARK_LAB_TABLE = appTable("benchmark_lab");
var BENCHMARK_LAB_DDL = `CREATE TABLE IF NOT EXISTS ${BENCHMARK_LAB_TABLE} (
  id TEXT PRIMARY KEY,
...
async function readLabState(client, options = {}) {
  ...
  try {
    ...
    const parsed = raw === void 0 ? EMPTY_LAB_STATE : parseLabState(raw);
    cache.set(client, { value: parsed, at: now });
    return parsed;
  } catch (error) {
    console.warn("[benchmark-lab] Falling back to empty lab state:", error.message);
