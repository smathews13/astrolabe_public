...
import {
  ...
  unionTableNames
} from "./chunk-UG37RZKS.mjs";
import "./chunk-PXBJ4ROO.mjs";
import "./chunk-EKHCFYEY.mjs";
import {
  ANSWER_PATH_ENDPOINT_IDS,
  SERVING_ENDPOINT_KIND,
  probeConnections
} from "./chunk-BNK6UH5D.mjs";
import {
...
// server/lib/user-spend-metrics.ts
var DAY_MS = 864e5;
function unavailable(subtitle) {
  return { value: null, state: "unavailable", subtitle };
}
function spendGrowth(current, prior, subtitle) {
...
function buildUserSpendMetrics(input) {
  ...
  return {
    ...
    costPerQuestion: current.comparable && current.amount !== null && current.questions > 0 ? {
      value: current.amount / current.questions,
      state: "value",
      subtitle: `${current.questions.toLocaleString()} submitted questions`
    } : unavailable(
      current.questions > 0 ? `${current.questions.toLocaleString()} submitted questions` : "No submitted questions"
    ),
    averageDaily: current.comparable && current.amount !== null && current.coveredDays > 0 ? {
    ...
    appShare: current.appComparable && current.amount !== null && current.appTotal !== null && current.appTotal > 0 ? { value: current.amount / current.appTotal * 100, state: "value", subtitle: "of comparable app spend" } : unavailable("App total not comparable"),
...
function previousMonthStart(day) {
  const date = /* @__PURE__ */ new Date(`${monthStart(day)}T00:00:00Z`);
...
function userSpendComparisonWindows(latestCompleteDay) {
...
function buildGenieAccountingStatement(workspaceId, range, configuredSpaces = [], appActivity = []) {
...
function readGenieAccountingRows(rows) {
  ...
  for (const raw of rows) {
    const row = Array.isArray(raw) ? Object.fromEntries(GENIE_ACCOUNTING_COLUMNS.map((column, index) => [column, raw[index]])) : raw && typeof raw === "object" ? raw : {};
    ...
    parsed.push({
      ...
      ].includes(text(row.attribution_method)) ? text(row.attribution_method) : "unattributed",
      dbus,
...
function readGenieAppActivityRows(rows) {
  ...
  return rows.flatMap((raw) => {
    ...
    return usageDay && identity && spaceId && calls !== null && calls > 0 ? [{ usageDay, identity, spaceId, calls }] : [];
  });
}
...
function classifyGenieAccounting(rows, throughDay, configuredSpaces = []) {
  ...
  let sourceRows = 0;
  let newest = "";
  const human = (identity, usageDay) => {
    const key = `${usageDay.slice(0, 7)}\0${identity}`;
    const existing = users.get(key);
    if (existing) return existing;
    const created = { ...emptySlice("query-history-exact"), identity, instances: /* @__PURE__ */ new Map() };
  ...
  const perUser = [...users.values()].map((value) => userResult(value.identity, value, configured)).sort(
  ...
  const directDbus = rows.filter((row) => configured.has(row.spaceId) && row.attributionMethod.endsWith("-exact")).reduce((total, row) => total + row.dbus, 0);
  const allocatedDbus = rows.filter((row) => configured.has(row.spaceId) && row.attributionMethod.endsWith("-allocation")).reduce((total, row) => total + row.dbus, 0);
...
async function costIdentifiersFor(appkit, req, extras) {
  ...
  return {
    ...
    ids: {
      ...
      foundationModel: text2(configured["llm-endpoint"]) || text2(report?.configuration.find((entry) => entry.key === "llm_endpoint")?.value) || (process.env.PLAYER_INSIGHTS_LLM_ENDPOINT ?? "").trim(),
...
async function genieAppActivityAttribution(appkit, ids, range) {
  try {
    ...
    console.warn(`[ops] Genie app activity could not be read: ${error.message}`);
    return [];
  }
}
function questionRun(row) {
  ...
  return {
    ...
    requestId: text2(row.request_id),
    correlationId: text2(row.correlation_id),
    traceId: text2(row.trace_id),
    user: text2(row.user_email).toLowerCase(),
    startedAt: text2(row.created_at),
    ...
    ...nullableNumber(row.cached_read_tokens) === null ? {} : { cachedReadTokens: nullableNumber(row.cached_read_tokens) ?? 0 },
    ...nullableNumber(row.cache_write_tokens) === null ? {} : { cacheWriteTokens: nullableNumber(row.cache_write_tokens) ?? 0 },
    ...
    evidenceComplete: text2(row.evidence_complete).toLowerCase() === "true"
  };
}
  ...
  appkit.server.extend((app) => {
    ...
    app.get("/api/ops/cost", async (req, res) => {
      ...
      const [
        ...
        appkit.lakebase.query(USER_SPEND_RUNS_QUERY, [activityRange.from, activityRange.to]).then((result) => ({ available: true, users: readUserRunSpendEvidence(result.rows), reason: "" })).catch((error) => ({
          available: false,
          users: [],
          reason: `Run identity evidence could not be read: ${error.message}`
        })),
        appkit.lakebase.query(USER_ACTIVE_MINUTES_QUERY, [activityRange.from, activityRange.to]).then((result) => ({
          available: true,
        ...
        userBrowse || Boolean(spendUser) ? appkit.lakebase.query(USER_MONITORING_ACTIVITY_QUERY, [
          activityRange.from,
          activityRange.to,
          ...
          return { available: true, users, reason: "" };
        }).catch((error) => ({
          available: false,
          users: [],
          reason: `User interaction evidence could not be read: ${error.message}`
        })) : Promise.resolve({ available: true, users: [], reason: "" }),
        ...
        appkit.lakebase.query(QUESTION_COST_RUNS_QUERY, [range.from, range.to]).then((result) => ({
          available: true,
      ...
      const interactiveComplete = questionRunsRead.available && (questionRunsRead.runs[0]?.evidenceComplete ?? questionRunsRead.runs.length === 0);
      ...
      const userMonitoringFor = (spend) => {
        ...
        return buildUserMonitoringPage({
          spend: enrichmentReason.length === 0 ? spend : {
            ...spend,
            state: "partial",
      ...
      try {
        ...
        const foundationStatement = interactiveComplete ? buildFoundationCostStatement(ids, range, questionRunsRead.runs) : null;
        const [outcome, queryAttribution, genieOutcome, foundationOutcome] = await Promise.all([
          ...
          foundationStatement ? runStatement({
            host: workspace,
            token,
            warehouseId: warehouse,
            ...
            ok: false,
            message: questionRunsRead.reason || (!ids.foundationModel ? "No configured foundation model is available." : "Interactive Ask evidence is unavailable.")
          })
        ]);
        ...
        const genieMonth = genieOutcome.ok ? classifyGenieAccounting(
        ...
        const spendWithGenie = genieMonth ? {
          ...
          users: spendByUser.users.map((profile) => {
            ...
            const sum = (read) => configured.reduce((total, instance) => total + read(instance), 0);
        ...
        const latestCompleteDay = split.meta?.lastDay && split.meta.lastDay < today ? split.meta.lastDay : new Date(clock() - 864e5).toISOString().slice(0, 10);
        const comparisonWindows = spendUser ? userSpendComparisonWindows(latestCompleteDay) : null;
        const comparisonReads = /* @__PURE__ */ new Map();
        const readComparisonSpend = (targetRange) => {
          ...
          const read = (async () => {
            ...
            const [comparisonResourceActivity, comparisonGenieActivity, comparisonRuns, comparisonActivity] = await Promise.all([
              resourceActivityAttribution(appkit, ids, targetRange),
              genieAppActivityAttribution(appkit, ids, targetRange),
              appkit.lakebase.query(USER_SPEND_RUNS_QUERY, [targetRange.from, targetRange.to]).then((result) => readUserRunSpendEvidence(result.rows)).catch(() => []),
              appkit.lakebase.query(USER_ACTIVE_MINUTES_QUERY, [targetRange.from, targetRange.to]).then((result) => ({ available: true, ...readUserActivitySpendEvidence(result.rows) })).catch(() => ({
                available: false,
                users: [],
                recordedFrom: "",
                recordedThrough: ""
            ...
            const comparisonQuestionRead = await appkit.lakebase.query(QUESTION_COST_RUNS_QUERY, [targetRange.from, targetRange.to]).then((result) => result.rows.map((row) => questionRun(row))).catch(() => []);
            const comparisonInteractiveComplete = comparisonQuestionRead[0]?.evidenceComplete ?? comparisonQuestionRead.length === 0;
            ...
            const comparisonFoundationStatement = comparisonInteractiveComplete ? buildFoundationCostStatement(ids, targetRange, comparisonQuestionRead) : null;
            const [comparisonOutcome, comparisonQueryAttribution, comparisonGenieOutcome, comparisonFoundationOutcome] = await Promise.all([
              runStatement({
                host: workspace,
                token,
                warehouseId: warehouse,
              ...
              comparisonGenieStatement ? runStatement({
                host: workspace,
                token,
                warehouseId: warehouse,
                ...
                fetchImpl: deps.fetchImpl
              }) : Promise.resolve({ ok: false, message: "No Genie comparison statement." }),
              comparisonFoundationStatement ? runStatement({
                host: workspace,
                token,
                warehouseId: warehouse,
                ...
                fetchImpl: deps.fetchImpl
              }) : Promise.resolve({ ok: false, message: "No Foundation comparison statement." })
            ]);
            if (!comparisonOutcome.ok) return null;
            const comparisonSplit = splitBillingRows(readComponentRows(comparisonOutcome.rows));
            const comparisonGenieRows = comparisonGenieOutcome.ok ? readGenieAccountingRows(comparisonGenieOutcome.rows) : [];
            const comparisonGenie = comparisonGenieOutcome.ok ? classifyGenieAccounting(
            ...
            const comparisonFoundation = comparisonFoundationOutcome.ok ? foundationCostTile(ids, readFoundationBillingRows(comparisonFoundationOutcome.rows)) : foundationCostTile(ids, null, comparisonFoundationOutcome.message);
            ...
            const comparisonSpend = buildSpendByUser({
              ...
              direct: comparisonGenie?.users.flatMap(
                (user) => (user.instances ?? []).filter((instance) => Boolean(instance.spaceId)).map((instance) => ({
                  email: user.identity,
                  componentId: instance.tileId,
                  quality: "direct",
              ...
              partialReason: comparisonQueryAttribution.complete && comparisonActivity.available ? "" : "Comparison-window attribution is incomplete."
            });
        ...
        const comparisonSpends = comparisonWindows ? await Promise.all([
        ...
        const metricSnapshot = (spend) => {
          ...
          const completeComponents = profile?.components.every(
            (component) => userUnit === "USD" ? component.usd.amount !== null : component.dbu.amount !== null
          ) ?? false;
          return {
            amount: reading?.amount ?? null,
            comparable: spend?.state === "ready" && reading?.amount !== null && reading?.quality !== "partial" && completeComponents && reconciliation?.difference === 0
          };
        };
        ...
        const selectedCurrentReading = userUnit === "USD" ? selectedCurrentProfile?.total.usd : selectedCurrentProfile?.total.dbu;
        const selectedReconciliation = userUnit === "USD" ? spendWithGenie.reconciliation.usd : spendWithGenie.reconciliation.dbu;
        const profileMetrics = spendUser && selectedCurrent.amount !== null ? buildUserSpendMetrics({
          unit: userUnit,
          current: {
            ...selectedCurrent,
            comparable: selectedCurrentReading?.amount !== null && selectedCurrentReading?.amount !== void 0 && selectedCurrentReading.quality !== "partial",
        ...
        const selectedSpendByUser = spendUser ? {
          ...spendWithGenie,
          users: spendWithGenie.users.filter((profile) => profile.email.toLowerCase() === spendUser).map((profile) => profileMetrics ? { ...profile, metrics: profileMetrics } : profile)
        } : userBrowse ? { ...spendWithGenie, users: [] } : spendWithGenie;
        const perQuestion = questionRunsRead.available ? buildQuestionAttribution(questionRunsRead.runs, tiles, QUESTION_COST_LIMIT, queryAttribution) : {
          ...empty.perQuestion,
