...
function buildGenieAccountingStatement(workspaceId, range) {
...
function userResult(identity, value) {
...
function classifyGenieAccounting(rows, throughDay) {
  ...
  const perUser = [...users].map(([identity, value]) => userResult(identity, value)).sort(
  ...
  return {
    ...
    pricingState: !anyRows ? "none" : !anyCharged ? "priced" : paidUsdComplete ? "priced" : paidUsd > 0 ? "partial" : "unpriced",
    ...
    app.get("/api/ops/cost", async (req, res) => {
      ...
      const userMonitoringFor = (spend) => {
        ...
        return buildUserMonitoringPage({
          spend: rosterRead.available ? spend : { ...spend, state: "partial", reason: [spend.reason, rosterRead.reason].filter(Boolean).join(" ") },
      ...
      try {
        ...
        const spendByUser = cachedUserSpend(spendCacheKey, clock()) ?? buildSpendByUser({
          ...
          direct: geniePeriod?.users.map((user) => ({
            email: user.identity,
