...
var MAX_PDF_TEXT_CHARS = 5e4;
var PDF_EXTRACTION_TIMEOUT_MS = 15e3;
var MAX_PDF_BYTES = 8 * 1024 * 1024;
var MAX_CONCURRENT_PDF_EXTRACTIONS = 2;
var MAX_QUEUED_PDF_EXTRACTIONS = 4;
var PDF_EXTENSIONS = ["pdf"];
var PdfTextError = class extends Error {
  code;
  /**
    ...
    try {
      ...
      worker.once("message", (message) => {
        ...
        if (result?.ok === true && typeof result.text === "string") {
          const text10 = result.text.slice(0, task.maxChars);
          void this.settle(
            task,
            text10.trim() ? text10 : new PdfTextError(
              "no-text",
...
var pdfExtractionPool = new PdfExtractionPool();
async function extractPdfText(input, options = {}) {
  return pdfExtractionPool.extract(input, options);
}

// shared/setup-remedies.ts
var GRANT_SCRIPT_ENV_VARS = [
...
var health = {
   ...
   */
  contentByRoute: /* @__PURE__ */ new Map()
};
...
function lakebaseHealth() {
  return {
    ...
    last_error: health.lastError === null ? null : { ...health.lastError, at: new Date(health.lastError.at).toISOString() },
...
function seconds(from) {
  return Math.round((Date.now() - from) / 1e3);
}
...
var RETRYABLE_CODES = /* @__PURE__ */ new Set([
  ...
  "28P01",
  // invalid_password: an expired Lakebase OAuth credential lands here
  "ECONNRESET",
  "ECONNREFUSED",
  "EPIPE",
  "ETIMEDOUT",
...
function isRetryable(error) {
  if (isStatementTimeout(error)) return false;
...
function isStatementTimeout(error) {
  return errorCode(error) === STATEMENT_TIMEOUT_CODE;
}
var schemaPending = false;
function markSchemaPending(pending) {
  schemaPending = pending;
}
var GRANT_DENIED_CODES = /* @__PURE__ */ new Set(["42501", "3F000", "42P01"]);
var OWNERSHIP_REFUSAL = /must be owner of/i;
function isGrantDenied(error, depth) {
  return depth === "schema" && GRANT_DENIED_CODES.has(errorCode(error)) && !OWNERSHIP_REFUSAL.test(errorMessage(error));
}
function isGrantDenialFailure(failure) {
  return GRANT_DENIED_CODES.has(failure.code) && !OWNERSHIP_REFUSAL.test(failure.message);
}
var GRANT_DENIED_LOG_REMEDY = `Re-run the canonical app release, or after a Lakebase reattach run:
${GRANT_SCRIPT_COMMAND}
${GRANT_SCRIPT_WHY}`;
var SCHEMA_QUALIFIED = new RegExp(`\\b${APP_SCHEMA}\\s*\\.`, "i");
function readDepth(sql) {
  return SCHEMA_QUALIFIED.test(sql) ? "schema" : "connection";
}
function clearsOutage(depth) {
  if (health.lastError === null) return true;
  return health.lastError.depth !== "schema" || depth === "schema";
}
function markOk(route, depth) {
  ...
  if (previous === "unavailable") {
    console.warn(
      `[lakebase] RECOVERED: reads are succeeding again on ${route} after ${outageSeconds}s unavailable. Every response during the outage declared itself unavailable rather than carrying rows.`
    );
  }
}
function markUnavailable(route, error, depth) {
  const message = errorMessage(error);
  const code = errorCode(error);
  const denied = isGrantDenied(error, depth) && !schemaPending;
  health.lastCheckedAt = Date.now();
  health.consecutiveFailures += 1;
  health.lastError = { message, code, route, at: Date.now(), depth };
  if (denied) health.access = "denied";
  if (health.state !== "unavailable") {
    const healthyFor = health.lastOkAt === null ? "never succeeded since boot" : `healthy for ${seconds(health.lastOkAt)}s`;
    ...
    if (schemaPending && GRANT_DENIED_CODES.has(code)) {
      console.warn(
        `[lakebase] READ BEFORE SCHEMA: ${route} could not read the store yet (code ${code}): ${message}. The schema pass is still running -- the app starts answering before it finishes, on purpose -- so the table this read wants may not exist yet. This is NOT the missing-grant condition and needs no script; it clears when the pass completes. If it is still appearing after that, it is real.`
      );
      return;
    }
    if (isStatementTimeout(error)) {
      console.error(
        `[lakebase] STATEMENT CANCELLED ON TIMEOUT: ${route} ran past the configured statement timeout and Postgres cancelled it (code ${code}): ${message}. Previously ${healthyFor}. This is NOT an empty result and NOT an outage: the store was answering. The route reports itself unavailable so no screen can show a cancelled read as a store holding nothing. Either the query needs an index or the window asked for more than this deployment's timeout allows.`
      );
      return;
    }
    console.error(
      `[lakebase] STORAGE UNAVAILABLE: ${route} failed (code ${code}): ${message}. Previously ${healthyFor}. Stored conversations, runs and benchmarks cannot be read; every route that reads them reports itself unavailable rather than substituting rows.`
    );
    return;
  }
  console.error(
    `[lakebase] still ${denied ? "refused" : "unavailable"} (failure ${health.consecutiveFailures}, ${seconds(health.since)}s): ${route} failed (code ${code}): ${message}`
  );
}
var sessionsWithTimeout = /* @__PURE__ */ new WeakSet();
async function runStatement(client, sql, params) {
  const pool = client.lakebase.pool;
  const applyTimeout = statementTimeoutSql();
  if (applyTimeout === null || typeof pool?.connect !== "function") {
    return client.lakebase.query(sql, params);
  }
  const connection = await pool.connect();
  try {
    if (!sessionsWithTimeout.has(connection)) {
      await connection.query(applyTimeout);
...
async function withoutReadTimeout(client, run, options = {}) {
  const pool = client.lakebase.pool;
  ...
  try {
    ...
    if (options.requirePinnedConnection) {
      throw new Error(
        `A migration requiring one pinned Postgres session could not reserve a connection: ${error.message}`
      );
    }
    console.warn(
      `[lakebase] no connection could be reserved to lift the read timeout: ${error.message}. Continuing on the pool, where a long statement may still be cancelled.`
    );
    return run((sql, params) => client.lakebase.query(sql, params));
  }
  ...
  try {
    ...
    if (restore !== null) {
      try {
        ...
        console.warn(
          `[lakebase] the read timeout could not be restored on a connection lent to a migration: ${error.message}. It is being returned unmarked, so the next read sets it again.`
        );
      }
    }
    connection.release();
...
async function readStored(client, route, sql, params = []) {
  const depth = readDepth(sql);
  let lastError;
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      const result = await runStatement(client, sql, params);
      if (attempt > 1) {
      ...
      if (attempt === 1 && isRetryable(error)) {
        console.warn(
          `[lakebase] ${route} failed with a retryable error (code ${errorCode(error)}): ${errorMessage(error)}. Retrying once on a fresh connection.`
        );
        continue;
      }
      break;
...
function chooseRows(route, read) {
  const denied = health.access === "denied";
  if (!read.available) {
    const reason = denied ? "storage_grants_missing" : "storage_unavailable";
    const cause = `The store ${denied ? "refused the read" : "is unavailable"}. Last error (code ${read.code}): ${read.error}` + (denied ? ` ${GRANT_DENIED_LOG_REMEDY}` : "");
    console.error(
      `[lakebase] NO ROWS SERVED on ${route}: the app shows no seeded data, so the response is empty and declares itself unavailable. It is NOT a report that the store holds nothing. ${cause}`
    );
  ...
  if (read.rows.length === 0) {
    if (health.contentByRoute.get(route) !== "empty") {
      console.warn(
        `[lakebase] STORE EMPTY on ${route}: the read succeeded, so Lakebase is reachable and the connection is fine. It simply holds no matching records, and this response carries no rows at all. Normal on a fresh deployment, where asking a question populates it. If records were written earlier, they have since been deleted, or the app is reading a different database, branch or schema than it wrote to.`
      );
    }
    health.contentByRoute.set(route, "empty");
    return { rows: [], substitution: { reason: null, origin: "lakebase" } };
  }
  if (health.contentByRoute.get(route) === "empty") {
  ...
  return { rows: read.rows, substitution: { reason: null, origin: "lakebase" } };
}
function noSubstitution(reason = null) {
  return { reason, origin: reason === null ? "lakebase" : "none" };
}
function markResponse(res, substitution) {
  res.setHeader("X-PIA-Storage", health.state);
  res.setHeader("X-PIA-Data-Origin", substitution.origin);
  if (substitution.reason) res.setHeader("X-PIA-Degraded-Reason", substitution.reason);
}
function lakebaseStorageCheck() {
    ...
    if (snapshot.access === "denied") {
      return {
        ...
        remedy: {
          ...
          // The variables are IN the statement, each with a `<value>` placeholder,
          // so listing them again is restating it. What a reader cannot get from
          // the statement is that the obvious move does not work: told the app
          // cannot read its database, a deployer redeploys.
          guidance: "A redeploy will not do this: the grant can only be made after the app exists, by hand, once."
        }
      };
    }
    return {
      ...
      remedy: {
        kind: "cli",
        // Was a hand-rolled `node scripts/grant-app-db-access.mjs` with none of
        // the five required variables, so the statement as printed could only
        // fail. The shared constant is the invocation that runs.
        statement: GRANT_SCRIPT_COMMAND,
        guidance: ""
      }
    };
  ...
  if (snapshot.state === "ok") {
    ...
    const counts = `SELECT (SELECT count(*) FROM ${APP_SCHEMA}.conversations) AS conversations,
    ...
    if (snapshot.content === "empty") {
      return {
        ...base,
        detail: `The app read its Postgres store successfully at ${snapshot.last_ok_at}, and it holds no stored records (${snapshot.empty_routes.join(", ")} matched nothing). The connection is healthy; the lists on screen are empty because there is nothing in them yet, which is what a fresh deployment looks like and is not a fault.`,
        remedy: {
          kind: "sql",
          statement: counts,
          guidance: ""
...
var WATCHDOG_ROUTE = "watchdog probe";
var WATCHDOG_PROBE_SQL = `SELECT (SELECT 1 FROM ${APP_SCHEMA}.conversations LIMIT 1) AS conversations, (SELECT 1 FROM ${APP_SCHEMA}.messages LIMIT 1) AS messages, (SELECT 1 FROM ${APP_SCHEMA}.benchmark_runs LIMIT 1) AS benchmark_runs`;
var WATCHDOG_DEFAULT_INTERVAL_MS = 6e4;
var WATCHDOG_MIN_INTERVAL_MS = 5e3;
function watchdogFloorMs(env = process.env) {
  return Math.max(WATCHDOG_MIN_INTERVAL_MS, statementTimeoutMs(env));
}
var WATCHDOG_INTERVAL_ENV = "PLAYER_INSIGHTS_LAKEBASE_WATCHDOG_MS";
function watchdogIntervalMs(env = process.env) {
  const floor = watchdogFloorMs(env);
  const raw2 = env[WATCHDOG_INTERVAL_ENV]?.trim();
  if (!raw2) return Math.max(WATCHDOG_DEFAULT_INTERVAL_MS, floor);
  const parsed = Number(raw2);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    console.warn(
      `[lakebase] ${WATCHDOG_INTERVAL_ENV} is "${raw2}", which is not a number of milliseconds. Probing every ${WATCHDOG_DEFAULT_INTERVAL_MS}ms instead. The watchdog is what dates an outage, so it is not switched off.`
    );
    return Math.max(WATCHDOG_DEFAULT_INTERVAL_MS, floor);
  }
  if (parsed < floor) {
    console.warn(
      `[lakebase] ${WATCHDOG_INTERVAL_ENV} is ${parsed}ms, below the ${floor}ms floor, which is this deployment's read timeout (${POOL_ENV.statementTimeoutMs}). Every tick is a real read and is cancelled by that same limit, so probing faster than it stacks probes on the connection pool the app serves requests from.`
    );
    return floor;
  }
  return parsed;
}
var stopActiveWatchdog = null;
function stopLakebaseWatchdog() {
  stopActiveWatchdog?.();
}
function startLakebaseWatchdog(client, intervalMs = watchdogIntervalMs()) {
  ...
  const timer = setInterval(() => {
    if (probing) {
      if (!skipping) {
        skipping = true;
        console.warn(
          `[lakebase] the watchdog probe has not answered within its ${intervalMs}ms interval, so this tick is skipped rather than started alongside it. The store is slower than the probe schedule assumes; the probe already running will report what it finds.`
        );
      }
      return;
    }
    probing = true;
    skipping = false;
    void readStored(client, WATCHDOG_ROUTE, WATCHDOG_PROBE_SQL).finally(() => {
      probing = false;
    });
  }, intervalMs);
  timer.unref?.();
...
// server/lib/deployment-decisions.ts
var APP_TARGET_ENV = "PLAYER_INSIGHTS_TARGET";
var LAKEBASE_ENDPOINT_ENV = "LAKEBASE_ENDPOINT";
var DEPLOYMENT_DECISIONS_TABLE_NAME = "deployment_decisions";
var SHARED_RAIL_DECISION = "shared_conversation_rail";
function decisionSource(env) {
  if ((env[APP_TARGET_ENV] ?? "").trim()) return "release";
  return (env[LAKEBASE_ENDPOINT_ENV] ?? "").trim() ? "git-deploy" : "local";
}
function deploymentDecisionsDdl(table) {
...
async function readDeploymentDecision(store, table, decision) {
  try {
...
async function preserveEnvDecision(input) {
...
var VERDICT_EXEMPT_STAGE_IDS = ["plot"];
var TIME_LIMIT_TAKEAWAY = "The run reached its time limit before the answer could be composed.";
var UNANSWERED_LINE = /^this question was not answered\.?$/i;
var WRITER_STOPPED_CAVEAT_SQL = `EXISTS (SELECT 1 FROM jsonb_array_elements_text(COALESCE(__CAVEATS__, '[]'::jsonb)) c WHERE c ~* 'was not reachable|run limit was reached|APITimeoutError|Request timed out|time limit before the answer could be composed|time limit before any data was measured')`;
function takeawayWhenTablesLanded(output, evidence) {
  const text10 = output.trim();
  if (!UNANSWERED_LINE.test(text10)) return output;
  const held = [evidence, output].filter(Boolean).join("\n");
  if (/\|.+\|/.test(held)) return TIME_LIMIT_TAKEAWAY;
  return output;
}
var VERDICT_STAGE_EXEMPTION_SQL = VERDICT_EXEMPT_STAGE_IDS.map(
  (id) => `&& @.id != "${id}"`
).join(" ");
var EMPTY_STAGES_FAILED_SQL = `(jsonb_typeof(trace->'stages') IS DISTINCT FROM 'array' OR jsonb_array_length(trace->'stages') = 0)`;
var INCOMPLETE_ANSWER_CAVEAT_SQL = `EXISTS (SELECT 1 FROM jsonb_array_elements_text(COALESCE(caveats, '[]'::jsonb)) c WHERE c ~* 'turn deadline|stopped early|sources for this answer are incomplete|structured presentation was incomplete|this question was not answered|was not reachable|this answer is degraded|no structured result|without a structured result')`;
var STRUCTURED_EVIDENCE_SQL = `(
...
function bindSynthesisIncompleteSql(trace, caveats) {
  return SYNTHESIS_INCOMPLETE_SQL.split("__TRACE__").join(trace).split("__CAVEATS__").join(caveats);
}
var DEADLINE_TRUNCATED_SQL = `EXISTS (SELECT 1 FROM jsonb_array_elements_text(COALESCE(caveats, '[]'::jsonb)) c WHERE c ~* 'turn deadline|stopped early')`;

// server/lib/ops-traffic.ts
var TRAFFIC_DAILY_ROLLUP_TABLE = appTable("traffic_daily_rollups");
var TRAFFIC_EVIDENCE_VERSION = 2;
var TRAFFIC_DAILY_ROLLUP_DDL = `CREATE TABLE IF NOT EXISTS ${TRAFFIC_DAILY_ROLLUP_TABLE} (
...
var bounds = `(($2::date::timestamp) AT TIME ZONE $1)`;
var endBounds = `((($3::date + 1)::timestamp) AT TIME ZONE $1)`;
var rawEvidence = evidenceCtes(bounds, endBounds);
var RAW_TRAFFIC_BREAKDOWNS_QUERY = `WITH ${rawEvidence}
${metricSelect("population", "deduped_tool_events")}
ORDER BY 1, 3 DESC, 2`;
var TRAFFIC_BREAKDOWNS_QUERY = `WITH ${rawEvidence},
  selected_rollups AS (
    SELECT *
...
GROUP BY kind, key
ORDER BY 1, 3 DESC, 2`;
var LEGACY_TRAFFIC_BREAKDOWNS_QUERY = `WITH answers AS (
...
ORDER BY 1, 3 DESC, 2`;
var rollupStart = `($1::date::timestamp AT TIME ZONE 'UTC')`;
var rollupEnd = `(($1::date + 1)::timestamp AT TIME ZONE 'UTC')`;
var ROLLUP_TRAFFIC_DAY_QUERY = `WITH ${evidenceCtes(rollupStart, rollupEnd)},
failure_counts AS (
...
function displayLabel(kind, key) {
...
function sortedBars(values, kind) {
  return [...values].map(([key, count3]) => ({ key, label: displayLabel(kind, key), count: count3 })).sort((left, right) => right.count - left.count || left.key.localeCompare(right.key));
}
function readTrafficBreakdowns(rows, input = {}) {
  ...
  let population = null;
  let outcomeCovered = null;
  let toolCovered = null;
  let malformed = false;
  for (const row2 of rows) {
    const kind = scalar(row2.kind);
    const key = scalar(row2.key);
  ...
  const coverageFor = (covered, noun) => {
    if (population === null) {
      return {
        state: "unavailable",
        coveredRuns: 0,
    ...
    const state = malformed || requested === "partial" || incomplete ? "partial" : requested === "unavailable" ? "unavailable" : "complete";
    const reason = input.reason || (malformed ? "One or more aggregate rows were malformed and were withheld." : incomplete ? `${safeCovered} of ${population} recorded runs have ${noun} evidence.` : "");
...
var RAW_TELEMETRY_RETENTION_DAYS = 90;
var MAX_ROLLUP_DAYS_PER_RUN = 31;
var MAX_DELETE_BATCHES_PER_RUN = 20;
var DELETE_BATCH_SIZE = 1e3;
var TELEMETRY_HOUSEKEEPING_INTERVAL_MS = 24 * 60 * 60 * 1e3;
var RAW_REQUEST_LATENCY_TABLE = appTable("request_latencies");
var RAW_APP_ACTIVITY_TABLE = appTable("app_activity_minutes");
var REQUEST_LATENCY_ROLLUP_TABLE = appTable("request_latency_daily_rollups");
var APP_ACTIVITY_ROLLUP_TABLE = appTable("app_activity_daily_rollups");
var TELEMETRY_ROLLUP_DAYS_TABLE = appTable("telemetry_rollup_days");
var TELEMETRY_HOUSEKEEPING_STATE_TABLE = appTable("telemetry_housekeeping_state");
var REQUEST_LATENCY_ROLLUP_DDL = `CREATE TABLE IF NOT EXISTS ${REQUEST_LATENCY_ROLLUP_TABLE} (
  day DATE NOT NULL,
...
var APP_ACTIVITY_ROLLUP_DDL = `CREATE TABLE IF NOT EXISTS ${APP_ACTIVITY_ROLLUP_TABLE} (
  day DATE PRIMARY KEY,
...
var TELEMETRY_ROLLUP_DAYS_DDL = `CREATE TABLE IF NOT EXISTS ${TELEMETRY_ROLLUP_DAYS_TABLE} (
  day DATE PRIMARY KEY,
...
var TELEMETRY_HOUSEKEEPING_STATE_DDL = `CREATE TABLE IF NOT EXISTS ${TELEMETRY_HOUSEKEEPING_STATE_TABLE} (
  singleton BOOLEAN PRIMARY KEY DEFAULT TRUE CHECK (singleton),
  last_completed_day DATE,
  completed_at TIMESTAMPTZ
)`;
var APP_ACTIVITY_RETENTION_INDEX_DDL = `CREATE INDEX IF NOT EXISTS app_activity_minutes_active_idx
  ON ${RAW_APP_ACTIVITY_TABLE} (active_minute)`;
var RUNS_CREATED_AT_INDEX_DDL = `CREATE INDEX IF NOT EXISTS runs_created_at_idx
  ON ${appTable("runs")} (created_at)`;
var TELEMETRY_ROLLUP_MIGRATION_DDL = [
...
var TRAFFIC_ROLLUP_MIGRATION_DDL = [TRAFFIC_DAILY_ROLLUP_DDL];
var TRAFFIC_EVIDENCE_V2_MIGRATION_DDL = [
...
var TELEMETRY_ADVISORY_LOCK_KEYS = [5261633, 5522764];
var CLAIM_TELEMETRY_LOCK_QUERY = "SELECT pg_try_advisory_lock($1, $2) AS acquired";
var RELEASE_TELEMETRY_LOCK_QUERY = "SELECT pg_advisory_unlock($1, $2) AS released";
var LAST_HOUSEKEEPING_DAY_QUERY = `SELECT last_completed_day
  FROM ${TELEMETRY_HOUSEKEEPING_STATE_TABLE}
  WHERE singleton = TRUE`;
var PENDING_ROLLUP_DAYS_QUERY = `WITH first_day AS (
  SELECT LEAST(
...
LIMIT $1`;
var ROLLUP_REQUEST_LATENCY_DAY_QUERY = `INSERT INTO ${REQUEST_LATENCY_ROLLUP_TABLE}
...
var ROLLUP_APP_ACTIVITY_DAY_QUERY = `WITH minute_totals AS (
...
var MARK_ROLLUP_DAY_QUERY = `INSERT INTO ${TELEMETRY_ROLLUP_DAYS_TABLE}
...
ON CONFLICT (day) DO UPDATE SET
  ...
  completed_at = EXCLUDED.completed_at`;
var DELETE_REQUEST_LATENCY_BATCH_QUERY = `WITH expired AS (
  SELECT raw.id
...
RETURNING raw.id`;
var DELETE_APP_ACTIVITY_BATCH_QUERY = `WITH expired AS (
...
var MARK_HOUSEKEEPING_COMPLETE_QUERY = `INSERT INTO ${TELEMETRY_HOUSEKEEPING_STATE_TABLE}
...
var activeHousekeeping = /* @__PURE__ */ new WeakMap();
function runTelemetryHousekeeping(lakebase, options = {}) {
...
var stopActiveScheduler = null;
function startTelemetryHousekeeping(lakebase, intervalMs = TELEMETRY_HOUSEKEEPING_INTERVAL_MS) {
  stopActiveScheduler?.();
  const run = () => {
    void runTelemetryHousekeeping(lakebase).catch((error) => {
      console.warn(`[telemetry-retention] Daily housekeeping did not complete and will retry: ${error.message}`);
    });
  };
  run();
  const timer = setInterval(run, Math.max(6e4, intervalMs));
  timer.unref?.();
  const stop = () => {
    clearInterval(timer);
...
var REQUEST_LATENCY_TABLE = RAW_REQUEST_LATENCY_TABLE;
var REQUEST_LATENCY_DDL = `CREATE TABLE IF NOT EXISTS ${REQUEST_LATENCY_TABLE} (
...
var REQUEST_LATENCY_INDEX_DDL = `CREATE INDEX IF NOT EXISTS request_latencies_recorded_route_idx
  ON ${REQUEST_LATENCY_TABLE} (recorded_at DESC, method, route)`;
var REQUEST_LATENCY_QUERY = `
  WITH requested AS (
...
function matchedRoutePath(req) {
  ...
  const path = Reflect.get(matched, "path");
...
var APP_ACTIVITY_TABLE = RAW_APP_ACTIVITY_TABLE;
var APP_ACTIVITY_DDL = `CREATE TABLE IF NOT EXISTS ${APP_ACTIVITY_TABLE} (
  user_email TEXT NOT NULL,
  active_minute TIMESTAMPTZ NOT NULL,
  PRIMARY KEY (user_email, active_minute)
)`;
var RECORD_APP_ACTIVITY_QUERY = `INSERT INTO ${APP_ACTIVITY_TABLE} (user_email, active_minute)
VALUES (lower($1), date_trunc('minute', now()))
ON CONFLICT (user_email, active_minute) DO NOTHING`;
var ACTIVE_MINUTES_PER_DAY_QUERY = `
  WITH requested AS (
...
// server/lib/app-session.ts
import { createHash, randomBytes } from "node:crypto";
var APP_IDLE_TIMEOUT_ENV = "PLAYER_INSIGHTS_IDLE_TIMEOUT_MINUTES";
var APP_SESSION_COOKIE = "__Host-astrolabe_session";
var APP_IDLE_TIMEOUT_CODE = "APP_IDLE_TIMEOUT";
var APP_SESSION_REQUIRED_CODE = "APP_SESSION_REQUIRED";
var APP_SESSION_UNAVAILABLE_CODE = "APP_SESSION_UNAVAILABLE";
var DEFAULT_IDLE_TIMEOUT_MINUTES = 30;
var MIN_IDLE_TIMEOUT_MINUTES = 5;
var MAX_IDLE_TIMEOUT_MINUTES = 480;
var APP_SESSION_ABSOLUTE_SECONDS = 24 * 60 * 60;
var APP_SESSION_ACTIVITY_THROTTLE_SECONDS = 45;
var APP_SESSION_RETENTION_HOURS = 24;
var APP_SESSION_CLEANUP_INTERVAL_MS = 15 * 60 * 1e3;
var APP_SESSION_TABLE = appTable("app_sessions");
var SESSION_ID_BYTES = 32;
var SESSION_ID_PATTERN = /^[A-Za-z0-9_-]{43}$/;
var MUTATION_HEADER = "x-astrolabe-session-action";
var EXEMPT_API_PATHS = /* @__PURE__ */ new Set(["/api/app-session/bootstrap", "/api/app-session/end", "/api/health", "/api/storage"]);
function stringColumn(value) {
  return typeof value === "string" ? value : "";
}
function finiteInteger(raw2) {
  if (!/^\d+$/.test(raw2)) return null;
  const parsed = Number(raw2);
  return Number.isSafeInteger(parsed) ? parsed : null;
}
function resolveIdleTimeout(env = process.env) {
  const raw2 = (env[APP_IDLE_TIMEOUT_ENV] ?? "").trim().toLowerCase();
  if (!raw2) return { enabled: true, minutes: DEFAULT_IDLE_TIMEOUT_MINUTES, source: "default" };
  if (raw2 === "disabled") return { enabled: false, minutes: 0, source: "disabled" };
  const parsed = finiteInteger(raw2);
  if (parsed === null) {
...
function normalizeSessionSubject(value) {
  return value.trim().toLowerCase();
}
function appSessionDeployment(env = process.env) {
...
function appSessionCookie(sessionId) {
...
function sessionMutationRefusal(req, action) {
...
function shouldProtectWithAppSession(path) {
  const normalized = path.toLowerCase();
  return normalized.startsWith("/api/") && !EXEMPT_API_PATHS.has(normalized);
}
async function insertSession(lakebase, binding, idleMinutes) {
...
async function readSessionState(lakebase, binding) {
  ...
  if (!row2) return "missing";
  if (normalizeSessionSubject(stringColumn(row2.subject)) !== binding.subject) return "mismatch";
  if (stringColumn(row2.deployment_key) !== binding.deployment) return "mismatch";
  const state = stringColumn(row2.session_state);
  return state === "active" || state === "idle_expired" || state === "absolute_expired" || state === "revoked" ? state : "mismatch";
}
async function revokeSession(lakebase, sessionHash) {
  await lakebase.query(
...
async function refreshSession(lakebase, binding, idleMinutes) {
  await lakebase.query(
    `UPDATE ${APP_SESSION_TABLE}
...
function refusal(res, status, error, detail) {
  res.status(status).json({ error, detail });
}
function subjectFor(req, controls) {
  try {
...
function bindingFor(req, controls) {
...
async function requireVerifiedSession(req, res, controls, config) {
  const sessionId = requestCookie(req);
  const deployment = appSessionDeployment(controls.env);
  if (!deployment) {
    refusal(
...
function registerAppSessionControls(app, controls) {
  const config = controls.config ?? resolveIdleTimeout(controls.env);
  if (!config.enabled) {
    const disabled = (action, status) => (req, res) => {
...
async function cleanupExpiredAppSessions(lakebase, limit = 500) {
  const bounded = Math.max(1, Math.min(2e3, Math.trunc(limit)));
  const result = await lakebase.query(
    `WITH expired AS (
       SELECT session_hash
         FROM ${APP_SESSION_TABLE}
...
function startAppSessionCleanup(lakebase, intervalMs = APP_SESSION_CLEANUP_INTERVAL_MS) {
  const run = () => {
...
var BASELINE_NAME = "baseline schema";
var LATER_MIGRATIONS = [
  {
    version: 2,
...
function migrationRegistryFault(migrations) {
...
// server/lib/migration-runner.ts
var ALTER_TARGET = /^ALTER TABLE\s+(\w+)\.(\w+)/i;
var ADDED_COLUMN = /ADD COLUMN IF NOT EXISTS\s+(\w+)/gi;
var CREATE_TABLE_TARGET = /^CREATE TABLE IF NOT EXISTS\s+(\w+)\.(\w+)/i;
var CREATE_INDEX_TARGET = /^CREATE\s+(?:UNIQUE\s+)?INDEX\s+(?:CONCURRENTLY\s+)?IF NOT EXISTS\s+(\w+)\s+ON\s+(\w+)\./i;
...
async function schemaNames(client, sql, params, column) {
  try {
    const result = await client.lakebase.query(sql, params);
    return new Set(result.rows.map((row2) => (identifierText(row2[column]) ?? "").toLowerCase()));
  } catch {
    return null;
  }
}
...
async function statementAlreadySatisfied(client, statement) {
...
var SCHEMA_VERSION_TABLE = "schema_version";
...
async function readAppliedVersions(client, schema) {
  try {
    ...
    for (const row2 of result.rows) {
      const version = Number(row2.version);
...
async function ownershipRefusal(client, schema) {
  try {
    ...
    return schemaWriteRefusal(schema, {
      ...
      connectedRoleHoldsOwner: row2.connected_role_holds_owner === true
    });
  } catch (error) {
    console.warn(
      `[migrate] could not establish who owns ${schema}, so the migrations are being attempted as before: ${error.message}`
    );
    return "";
  }
}
async function runMigrations(client, options) {
  ...
  if (fault) {
    console.error(
      `[migrate] MIGRATIONS NOT APPLIED: the migration list in this build is not usable — ${fault} Nothing was attempted, and the schema is whatever it already was.`
    );
    return blank(fault);
  }
  if (mode === "apply") {
  ...
  if (mode === "verify") {
    console.error(
      `[migrate] SCHEMA BEHIND: version(s) ${pending.map((migration) => migration.version).join(", ")} (${pending.map((migration) => migration.name).join("; ")}) are not applied. Nothing was run, because this was a verification. Run the migration step before serving traffic that reads what they create.`
    );
    return {
      mode,
      versionBefore,
  ...
  for (const migration of pending) {
    ...
    console.error(
      `[migrate] STOPPED AT VERSION ${migration.version} (${migration.name}). ${pending.length - attempts.length} later version(s) were NOT attempted, deliberately: a migration that runs after the one it depends on failed leaves the schema at no version at all.`
    );
    break;
  }
  ...
  if (ok) {
    console.log(
      `[migrate] Applied version(s) ${attempts.map((attempt) => attempt.version).join(", ")}; schema is now at version ${versionAfter}.`
    );
  } else {
    console.error(
      `[migrate] SCHEMA INCOMPLETE: version(s) ${stillPending.join(", ")} are not applied. The schema is at version ${versionAfter}, and anything that reads what those versions create will fail. This is reported as a failure rather than logged and forgotten, so a release that reads it can refuse to promote.`
    );
  }
...
async function bootstrapVersionTable(client, schema) {
  try {
    ...
    console.warn(
      `[migrate] ${schema}.${SCHEMA_VERSION_TABLE} could not be created (${failureCode(error) || "no code"}): ${error.message}. If it already exists this changed nothing; if it does not, the version below will be reported as applied-but-not-recorded.`
    );
  }
}
async function applyOne(client, schema, migration, appliedBy, ensureVersionTable) {
  ...
  try {
    ...
    console.error(
      `[migrate] v${migration.version} (${migration.name}) APPLIED BUT NOT RECORDED: ${error.message}. The statements landed, so the schema is correct, but ${SCHEMA_VERSION_TABLE} does not say so and the next run will replay them. Every statement is idempotent, so that replay is safe.`
    );
    return { version: migration.version, name: migration.name, outcome: "unrecorded", failures };
  }
  return { version: migration.version, name: migration.name, outcome: "applied", failures };
}
function reportStatementFailures(migration, total, refused, unresolved) {
  const version = `(version ${migration.version}, ${migration.name})`;
  for (const { failure } of refused) {
    if (failure.satisfied) {
      console.warn(
        `[lakebase] SCHEMA STATEMENT ${failure.position} of ${total} was refused (${failure.label}): ${failure.message}. Every column it adds is already present, so the schema is what this version reads and nothing changed. The usual cause is ownership, which Postgres checks before it decides the statement is a no-op. To stop it recurring, see scripts/grant-app-db-access.mjs. ${version}`
      );
      continue;
    }
    const remaining = total - failure.position;
    console.error(
      `[lakebase] SCHEMA STATEMENT ${failure.position} of ${total} FAILED (${failure.label}): ${failure.message}. ` + (remaining > 0 ? `The remaining ${remaining} statement(s) still run: one refusal no longer stops them. ` : "") + `If the object this statement maintains is already in place, nothing changed; if it is not, everything that reads it will fail. ${version}`
    );
  }
  ...
  if (unresolved.length === total) {
    console.error(
      `[lakebase] SCHEMA SETUP ${denied.length === total ? "REFUSED" : "FAILED"}: all ${total} statements were ${denied.length === total ? "refused by Postgres on privileges" : "refused"}, so the app is starting without a usable store and every read below will report itself unavailable rather than return rows. First error (${unresolved[0].label}): ${unresolved[0].message}${grantRemedy} ${version}`
    );
    return;
  }
  console.error(
    `[lakebase] SCHEMA SETUP INCOMPLETE: ${unresolved.length} of ${total} statements failed (${unresolved.map((failure) => failure.label).join(", ")}); the other ${total - unresolved.length} left the schema as this version reads it, so the store answered and reads and writes below will use it, and nothing below is answered from anywhere else. What is not established is whatever those statements maintain, check the objects named above exist and carry the columns this version expects. The usual cause on a database that already has these tables is ownership: ALTER requires the app's Postgres role to own the table, IF NOT EXISTS does not exempt it, because Postgres refuses on ownership before it decides the statement is a no-op. See scripts/grant-app-db-access.mjs.` + (grantRemedy ? ` ${denied.length} of those refusals ${denied.length === 1 ? "is" : "are"} a privilege denial (${[...new Set(denied.map((failure) => failure.code))].join(", ")}) rather than the ownership no-op, which means the grant is missing rather than merely narrow.${grantRemedy}` : "") + ` ${version}`
  );
}

// shared/user-monitoring-contract.ts
var USER_MONITORING_SCHEMA_REVISION = 2;

// server/lib/user-spend.ts
var DAY_MS2 = 864e5;
var MAX_USER_SPEND_DAYS = 90;
var ALLOCATION_SCALE = 1e6;
var USER_SPEND_CACHE_MS = 3e4;
var spendDataRevision = 0;
var rejectedUserMonitoringEvidence = 0;
var spendCache = /* @__PURE__ */ new Map();
function invalidateUserSpendCache() {
  spendDataRevision += 1;
  spendCache.clear();
}
function userSpendDataRevision() {
  return spendDataRevision;
}
function userMonitoringEvidenceDiagnostics() {
  return { rejectedRows: rejectedUserMonitoringEvidence };
}
function userSpendCacheKey(principal, range) {
  return [principal.trim().toLowerCase(), range.from, range.to, "USD+DBU", spendDataRevision].join("|");
}
function cachedUserSpend(key, now = Date.now()) {
  const cached2 = spendCache.get(key);
  if (!cached2 || cached2.expiresAt <= now) {
    if (cached2) spendCache.delete(key);
    return null;
  }
  return cached2.payload;
}
function cacheUserSpend(key, payload, now = Date.now()) {
  spendCache.set(key, { expiresAt: now + USER_SPEND_CACHE_MS, payload });
}
var USER_SPEND_RUNS_QUERY = `
  WITH completed AS (
...
var USER_MONITORING_ACTIVITY_QUERY = `
  WITH evidence AS (
...
function readUserRunSpendEvidence(rows) {
  const users = /* @__PURE__ */ new Map();
  const seen = /* @__PURE__ */ new Set();
  for (const row2 of rows) {
    const email = text2(row2.user_email).toLowerCase();
    if (!email) continue;
    const rowKey = [
      text2(row2.row_kind),
      email,
      text2(row2.tool),
      text2(row2.resource_id),
      number(row2.calls),
      number(row2.total_runs),
      number(row2.total_tokens)
    ].join("|");
    ...
    if (text2(row2.row_kind) === "run") {
      current.totalRuns = number(row2.total_runs);
      current.tokenCoveredRuns = number(row2.token_covered_runs);
      current.totalTokens = number(row2.total_tokens);
      if (row2.total_duration_ms !== void 0 && row2.total_duration_ms !== null) {
        current.totalDurationMs = number(row2.total_duration_ms);
      }
      const lastActive = row2.last_active instanceof Date ? row2.last_active.toISOString() : text2(row2.last_active);
      if (lastActive) current.lastActive = lastActive;
    } else if (text2(row2.row_kind) === "resource") {
      const tool = text2(row2.tool);
      const calls = number(row2.calls);
      if (tool && calls > 0) current.resources.push({ tool, resourceId: text2(row2.resource_id), calls });
    }
    users.set(email, current);
  }
  return [...users.values()].sort((left, right) => left.email.localeCompare(right.email));
}
function readUserInteractionEvidence(rows) {
  const users = /* @__PURE__ */ new Map();
  for (const row2 of rows) {
    const email = text2(row2.user_email).toLowerCase();
    const firstActive = row2.first_active instanceof Date ? row2.first_active.toISOString() : text2(row2.first_active);
    const lastActive = row2.last_active instanceof Date ? row2.last_active.toISOString() : text2(row2.last_active);
    if (!email || !email.includes("@") || !Number.isFinite(Date.parse(firstActive)) || !Number.isFinite(Date.parse(lastActive))) {
    ...
    users.set(email, {
      email,
      questions: Math.max(existing?.questions ?? 0, number(row2.questions)),
      runs: Math.max(existing?.runs ?? 0, number(row2.runs)),
...
function readUserActivitySpendEvidence(rows) {
  const first = rows[0];
  return {
    users: rows.map((row2) => ({
      email: text2(row2.user_email).toLowerCase(),
      activeMinutes: number(row2.active_minutes),
      lastActive: row2.last_active instanceof Date ? row2.last_active.toISOString() : text2(row2.last_active)
    })).filter((row2) => row2.email && row2.activeMinutes > 0),
    recordedFrom: first?.recorded_from instanceof Date ? first.recorded_from.toISOString() : text2(first?.recorded_from),
    recordedThrough: first?.recorded_through instanceof Date ? first.recorded_through.toISOString() : text2(first?.recorded_through)
  };
}
function capUserSpendRange(requested) {
...
function allocateDeterministically(total, weights) {
  const usable = [...weights].filter(([, weight]) => Number.isFinite(weight) && weight > 0).sort(([left], [right]) => left.localeCompare(right));
  const denominator = usable.reduce((sum, [, weight]) => sum + weight, 0);
  if (!Number.isFinite(total) || denominator <= 0) return /* @__PURE__ */ new Map();
...
function completeDays(range) {
  const from = Date.parse(`${range.from}T00:00:00Z`);
  const to = Date.parse(`${range.to}T00:00:00Z`);
  return Number.isFinite(from) && Number.isFinite(to) && to >= from ? Math.round((to - from) / DAY_MS2) + 1 : 0;
}
function qualityFor(parts, partial) {
...
function buildSpendByUser(input) {
  const emails = /* @__PURE__ */ new Set();
  for (const row2 of [...input.queryUsers, ...input.runs, ...input.activity.users, ...input.direct ?? []]) {
    if (row2.email.trim()) emails.add(row2.email.trim().toLowerCase());
  }
  const users = [...emails].sort();
  const days = completeDays(input.range);
  const activityByUser = new Map(input.activity.users.map((row2) => [row2.email, row2]));
  const tileById = new Map(input.tiles.map((tile) => [tile.id, tile]));
  const genieComponentIds = input.tiles.filter((tile) => tile.id.startsWith("genie:") && Boolean(tile.resourceId)).map((tile) => tile.id);
  const componentIds = [
    "serving-endpoint",
    "foundation-model",
    "sql-warehouse",
  ...
  const unattributed = [];
  const appendComponent = (id, unit, total, weights, usable, reason) => {
    const direct = (input.direct ?? []).filter(
      (row2) => row2.componentId === id && typeof (unit === "USD" ? row2.usd : row2.dbu) === "number"
    );
    const directByEmail = /* @__PURE__ */ new Map();
    for (const row2 of direct) {
      const email = row2.email.toLowerCase();
      const current = directByEmail.get(email);
      directByEmail.set(email, {
        value: (current?.value ?? 0) + (unit === "USD" ? row2.usd : row2.dbu),
        quality: current?.quality === "direct" || row2.quality === "direct" ? "direct" : "joined"
      });
    }
    const allocated = total === null ? /* @__PURE__ */ new Map() : direct.length > 0 ? new Map([...directByEmail].map(([email, entry]) => [email, entry.value])) : usable ? allocateDeterministically(total, weights) : /* @__PURE__ */ new Map();
    const attributedTotal = [...allocated.values()].reduce((sum, value) => sum + value, 0);
    const residual = total === null ? null : Math.round((total - attributedTotal) * ALLOCATION_SCALE) / ALLOCATION_SCALE;
    for (const email of users) {
      const found = directByEmail.get(email);
      const value = allocated.get(email) ?? null;
      const quality = found?.quality ?? (value === null ? "unavailable" : "allocated");
  ...
  for (const id of componentIds) {
    ...
    const servingCoverage = input.runs.reduce((sum, row2) => sum + row2.totalRuns, 0) > 0 && input.runs.every((row2) => row2.totalRuns === row2.tokenCoveredRuns);
    const vectorRows = input.runs.flatMap(
      (row2) => row2.resources.filter((resource) => resource.tool === "search_semantics").map((resource) => ({ email: row2.email, ...resource }))
    );
    const activityCoverage = input.activity.available && Boolean(input.activity.recordedFrom) && Date.parse(input.activity.recordedFrom) <= Date.parse(`${input.range.from}T23:59:59Z`);
    for (const unit of ["USD", "DBU"]) {
      const total = tileTotal(tile, unit, days);
      let weights = /* @__PURE__ */ new Map();
      let usable = false;
      let reason = tile?.unavailable || "No attributable component amount was measured.";
      if (id === "serving-endpoint") {
        const timingReported = input.runs.every((row2) => row2.totalDurationMs !== void 0);
        weights = new Map(
          input.runs.map((row2) => [row2.email, timingReported ? row2.totalDurationMs ?? 0 : row2.totalTokens])
        );
        usable = input.runs.length > 0 && (timingReported || servingCoverage) && [...weights.values()].some((value) => value > 0);
        reason = usable ? "" : "Serving spend lacks complete interactive request timing.";
      } else if (id === "foundation-model") {
        weights = new Map(input.runs.map((row2) => [row2.email, row2.totalTokens]));
        ...
        weights = new Map(input.queryUsers.map((row2) => [row2.email, row2.astrolabeExecutionMs]));
        ...
        weights = new Map(
          input.queryUsers.map((row2) => [
            row2.email,
            row2.genieSpaces.find((space) => space.spaceId === spaceId)?.executionMs ?? 0
          ])
        );
        ...
        weights = new Map(
          users.map((email) => [
            email,
            vectorRows.filter((row2) => row2.email === email && wanted.has(row2.resourceId)).reduce((sum, row2) => sum + row2.calls, 0)
          ])
        );
        usable = wanted.size > 0 && vectorRows.length > 0 && vectorRows.every((row2) => Boolean(row2.resourceId)) && [...weights.values()].some((value) => value > 0);
  ...
  const profiles = users.map((email) => {
    ...
    const partial2 = unattributed.some(
    ...
    return {
      ...
      total: {
        usd: amount(
          ...
          qualityFor(usdParts, partial2)
        ),
        dbu: amount(
          dbuValues.length ? dbuValues.reduce((sum, value) => sum + value, 0) : null,
          qualityFor(dbuParts, partial2)
        )
      },
      components
    };
  });
  const reconcile = (unit) => {
    const key = unit === "USD" ? "usd" : "dbu";
    const componentTotals = componentIds.map((id) => tileTotal(tileById.get(id), unit, days)).filter((value) => value !== null);
    const appTotal = componentTotals.length ? componentTotals.reduce((sum, value) => sum + value, 0) : null;
    const userTotal = profiles.flatMap((profile) => profile.total[key].amount ?? []).reduce((sum, value) => sum + value, 0);
    const missing = unattributed.flatMap((component) => component[key].amount ?? []).reduce((sum, value) => sum + value, 0);
    return {
      unit,
      appTotal,
      users: appTotal === null ? null : userTotal,
      unattributed: appTotal === null ? null : missing,
      difference: appTotal === null ? null : Math.round((appTotal - userTotal - missing) * ALLOCATION_SCALE) / ALLOCATION_SCALE
    };
  };
...
var USER_MONITORING_PAGE_SIZE = 25;
var USER_MONITORING_MAX_PAGE_SIZE = 50;
function paidBucket(reading) {
  if (reading.amount === null || !Number.isFinite(reading.amount)) return 2;
  return reading.amount > 0 ? 0 : 1;
}
function encodeMonitoringCursor(cursor) {
  return Buffer.from(JSON.stringify(cursor), "utf8").toString("base64url");
}
function decodeMonitoringCursor(raw2) {
  if (!raw2) return null;
  try {
    const value = JSON.parse(Buffer.from(raw2, "base64url").toString("utf8"));
    return typeof value.bucket === "number" && typeof value.amount === "number" && typeof value.email === "string" ? { bucket: value.bucket, amount: value.amount, email: value.email } : null;
  } catch {
    return null;
  }
}
function afterMonitoringCursor(row2, unit, cursor) {
  const reading = unit === "USD" ? row2.spend.usd : row2.spend.dbu;
  const bucket = paidBucket(reading);
  const amount2 = reading.amount ?? 0;
  return bucket > cursor.bucket || bucket === cursor.bucket && (amount2 < cursor.amount || amount2 === cursor.amount && row2.email.localeCompare(cursor.email) > 0);
}
function buildUserMonitoringPage(input) {
  const profiles = new Map(input.spend.users.map((profile) => [profile.email.toLowerCase(), profile]));
  const interactions = new Map((input.interactions ?? []).map((row2) => [row2.email.toLowerCase(), row2]));
  ...
  const emails = [...interactions].filter(([, interaction]) => {
    const lastActive = Date.parse(interaction.lastActive);
    const qualifies = Number.isFinite(lastActive) && Number.isFinite(rangeStart) && Number.isFinite(rangeEnd) && lastActive >= rangeStart && lastActive <= rangeEnd;
    if (!qualifies) rejectedUserMonitoringEvidence += 1;
    return qualifies;
  }).map(([email]) => email);
  const search = (input.search ?? "").trim().toLowerCase().slice(0, 120);
  const unavailable3 = { amount: null, quality: "unavailable" };
  const authorizedRows = emails.filter((email) => email.includes("@")).map((email) => {
    const profile = profiles.get(email);
    const interaction = interactions.get(email);
    const lastActive = interaction?.lastActive ?? "";
    const usd = profile?.total.usd ?? unavailable3;
    const dbu = profile?.total.dbu ?? unavailable3;
    return {
      email,
  ...
  }).filter((row2) => (!search || row2.email.includes(search)) && (!input.role || row2.role === input.role));
  const personaCounts = /* @__PURE__ */ new Map();
  for (const row2 of authorizedRows) {
    if (row2.persona) personaCounts.set(row2.persona.id, (personaCounts.get(row2.persona.id) ?? 0) + 1);
  }
  const rows = authorizedRows.filter((row2) => !input.persona || row2.persona?.id === input.persona).sort((left, right) => {
    ...
    return paidBucket(leftReading) - paidBucket(rightReading) || (rightReading.amount ?? 0) - (leftReading.amount ?? 0) || left.email.localeCompare(right.email);
  });
  const cursor = decodeMonitoringCursor(input.cursor ?? "");
  const eligible = cursor ? rows.filter((row2) => afterMonitoringCursor(row2, input.unit, cursor)) : rows;
  ...
  return {
    ...
    users,
    personas: [...input.personaOptions ?? []].map((persona) => ({ ...persona, count: personaCounts.get(persona.id) ?? 0 })).sort((left, right) => left.name.localeCompare(right.name) || left.id.localeCompare(right.id)),
    dataRevision: userSpendDataRevision(),
    pagination: {
      total: rows.length,
      pageSize,
      hasMore: eligible.length > users.length,
      nextCursor: last && eligible.length > users.length && lastReading ? encodeMonitoringCursor({
...
// shared/answer-content-policy.ts
var MATERIAL_WARNING = /\b(?:permission|privilege|denied|refused|unauthori[sz]ed|forbidden|error|fail(?:ed|ure)|not found|does not exist|rate limit|(?:query|sql|statement)(?: execution)? (?:failed|timed out)|(?:failed|timed out) (?:query|sql|statement)|timeout|stale|out[- ]of[- ]date|incomplete (?:source|data)|(?:source|citation) (?:is )?unavailable|conflicting evidence|conflict|uncertain|unsupported|safety|policy refusal|required (?:input|field)|missing required|data quality|invalid|duplicate|coverage gap|dataset (?:ends|ended)|only \d+ of)\b/i;
var NO_GOVERNED_READ = /^(?:no|none of the) governed tables?(?: (?:was|were|has been|have been))? (?:read|queried|accessed)(?: for (?:this|the) (?:answer|response|result))?(?:,? so (?:it|the (?:answer|response|result)) (?:is|was) not grounded in (?:queried|source) data)?[.!]?$/i;
var NOT_GROUNDED = /^(?:this|the) (?:answer|response|result) (?:is|was) not grounded in (?:queried|source) data[.!]?$/i;
var NO_SQL = /^no (?:sql|sql statement|query|statement)(?:s)? (?:was|were|has been|have been)? ?(?:generated(?: or executed)?|executed|run|produced)(?: for (?:this|the) (?:answer|response|result))?[.!]?$/i;
var NO_EVIDENCE_TOOL = /^no (?:source|sources|citation|citations|tool|tools|tool call|tool calls|source\/citation\/tool)(?: (?:was|were|has been|have been))? (?:used|called|cited|provided)(?: for (?:this|the) (?:answer|response|result))?[.!]?$/i;
var NO_MLFLOW_TRACE = /^no mlflow trace was recorded for this answer, so it cannot be opened in mlflow[.!]?$/i;
var INTERNAL_PROCESS_NEGATIVE = /^the agent (?:did not|could not|was unable to) (?:read|query|generate|execute|run|use|call|cite|retrieve|inspect|access)\b[^.!?]*[.!]?$/i;
var GOVERNED_READ_PARAPHRASE = /^(?:this|the) (?:answer|response|result) (?:did not|does not) (?:query|read|use) any governed tables?(?: and|,)? (?:so|therefore) (?:it )?(?:is|was) not grounded in (?:queried|source) data[.!]?$/i;
var PROCESS_SECTION_LINE = /^what (?:wasn['’]t|was not) (?:included|done)\s*:\s*(?:sql|sources?|citations?|tools?|queries?|governed tables?)(?:\s*(?:,|and|or)\s*(?:sql|sources?|citations?|tools?|queries?|governed tables?))*[.!]?$/i;
var PROCESS_ONLY_TAKEAWAY = /^(?:the agent did not return a structured result|no steps and no structured result were recorded|the run stopped without a structured result)[.!]?$/i;
var INCOMPLETE_FORMAT = /^(?:this answer is degraded:\s*)?(?:no structured result arrived and no tool steps were recorded|the run stopped after \d+ steps? without a structured result)[.!]?$/i;
var REVIEW_SQL_AND_SOURCES = /^review the generated sql and source details before using (?:this|the) result[.!]?$/i;
var UNTAGGED_TABLE_SCOPE = /^(?:all|none of the) (\d+) tables? (?:are|is|have been) untagged(?: \(no franchise label\))?[;,] (?:this means )?franchise scope is unknown until (?:a|each|the) tables? (?:is|are) described or queried[.!]?$/i;
var FIGURE_LIMIT = /^(.+?) (?:are|is) (?:omitted from|not shown in) (?:the )?figures?(?: due to the (\d+)[- ]figure limit)?(?: but (?:are|is) included in the narrative)?[.!]?$/i;
var FIGURE_LIMIT_RANKED = /^.+? (?:are|is) .{1,80}\bbut (?:omitted from|not shown in) (?:the )?figures? due to the \d+[- ]figure limit[.!]?$/i;
var PROCESS_SECTION = /^(?:what (?:wasn['’]t|was not) (?:included|done)|work (?:not|wasn['’]t) (?:included|performed)|what the agent (?:didn['’]t|did not) do)$/i;
...
function generatedText(line) {
  return line.trim().replace(/^[-*+]\s+/, "").replace(/^\d+[.)]\s+/, "").replace(/^\*\*(.+)\*\*$/, "$1").trim();
}
function isQuotedLine(line) {
  return /^\s*(?:>|["'“‘])/.test(line);
}
function normalizeAnswerCaveat(caveat, evidence = {}) {
  const text10 = caveat.trim();
  if (!text10) return null;
  const scope = UNTAGGED_TABLE_SCOPE.exec(text10);
  if (scope) {
  ...
  if (REVIEW_SQL_AND_SOURCES.test(text10)) {
  ...
  if (NO_GOVERNED_READ.test(text10) || GOVERNED_READ_PARAPHRASE.test(text10) || NOT_GROUNDED.test(text10) || NO_SQL.test(text10) || NO_EVIDENCE_TOOL.test(text10) || NO_MLFLOW_TRACE.test(text10) || PROCESS_SECTION_LINE.test(text10)) {
    return null;
  }
  if (INCOMPLETE_FORMAT.test(text10)) {
    return "This answer is degraded: the response format was incomplete. Retry the question before using this result.";
  }
  if (MATERIAL_WARNING.test(text10)) return text10;
  if (INTERNAL_PROCESS_NEGATIVE.test(text10)) return null;
  if (FIGURE_LIMIT.test(text10) || FIGURE_LIMIT_RANKED.test(text10)) return null;
  return text10;
}
function normalizeReaderText(value, evidence = {}, field2 = "narrative") {
  const text10 = value.trim();
  if (!text10) return "";
  if (field2 === "takeaway" && PROCESS_ONLY_TAKEAWAY.test(text10)) {
...
function normalizeReaderAnswer(answer) {
  const evidence = { sql: answer.sql, sources: answer.sources };
  return {
    ...answer,
    ...typeof answer.takeaway === "string" ? { takeaway: normalizeReaderText(answer.takeaway, evidence, "takeaway") } : {},
    ...typeof answer.narrative === "string" ? { narrative: normalizeReaderText(answer.narrative, evidence, "narrative") } : {},
    ...typeof answer.content === "string" ? { content: normalizeReaderText(answer.content, evidence, "content") } : {},
    ...answer.caveats ? {
      caveats: answer.caveats.map((caveat) => normalizeAnswerCaveat(caveat, evidence)).filter((caveat) => Boolean(caveat))
    } : {}
  };
}

// shared/conversation-title.ts
var CONVERSATION_TITLE_LIMIT = 300;
var ELLIPSIS = "…";
var PLACEHOLDER_CONVERSATION_TITLE = "New conversation";
function conversationTitle(prompt, limit = CONVERSATION_TITLE_LIMIT) {
...
// server/lib/repair-conversation-titles.ts
var OLD_CUT_LENGTH = 80;
var TRUNCATED_TITLES_QUERY = `
...
async function repairTruncatedTitles(appkit) {
  let rows;
  try {
    const result = await appkit.lakebase.query(TRUNCATED_TITLES_QUERY, [OLD_CUT_LENGTH]);
    rows = result.rows ?? [];
  } catch (error) {
    return { repaired: 0, skipped: `the candidate read was refused: ${error.message}` };
  }
  const repairs = [];
  for (const row2 of rows) {
    const id = typeof row2.id === "string" ? row2.id : "";
    const content = typeof row2.content === "string" ? row2.content : "";
    if (!id || !content) continue;
    const restored = conversationTitle(content);
    if (restored === row2.title) continue;
  ...
  for (const repair of repairs) {
    try {
      ...
      console.warn(
        `[lakebase] Could not restore the label on conversation ${repair.id}: ${error.message}. It keeps the shortened one.`
      );
    }
  }
  if (repaired > 0) {
    console.log(
      `[lakebase] Restored ${repaired} conversation label(s) that an earlier version stored cut to ${OLD_CUT_LENGTH} characters. Read back from the question each one was asked with.`
    );
  }
...
var DATA_SOURCE_FINDER_TASK = "Identify the governed data available for this question.";
var READER_STAGE_TASKS = {
...
var TOOL_STAGE_ID = /^step-\d+-\d+-(.+)$/;
var MODEL_STEP_ID = /^step-\d+$/;
var TOOL_OUTPUT_NOTES = [
  "Asked together with the other definition questions in this step,",
  "Already asked in this run."
];
var ERROR_INSTRUCTION_MARKERS = [
  ". Do not ",
  ". Do NOT ",
  ". Report this ",
  ". Call ",
...
var IDENTIFIED_DATA_OUTPUT = "Identified the governed data available for this question.";
var STAGE_FALLBACKS = {
  running: {
...
function projectSemanticOutput(value) {
  if (value.startsWith("SEMANTIC SEARCH UNAVAILABLE")) {
    return value.split(" This is discovery, not data", 1)[0]?.trim() ?? "";
  }
  return value.split("\n\n").map((paragraph) => paragraph.trim()).filter(
    (paragraph) => paragraph && !paragraph.startsWith("SEMANTIC SEARCH RESULTS.") && !paragraph.startsWith("What appears above was filtered by a cached snapshot")
  ).map((paragraph) => {
...
${listed}`.trim();
  ...
  if (tool === "dictionary_genie" && value.includes("Definition note:")) {
    return value.split(/\r?\n/).map(
      (line) => line.startsWith("Definition note:") ? "The dictionary space answered without reading the dictionary table; this is the space's account, not a governed entry." : line
    ).join("\n").trim();
  }
  return value;
}
function projectToolOutput(text10, tool = "") {
  ...
  if (!value.startsWith("ERROR:")) return projectNamedToolOutput(text10, tool);
...
var PROSE_ONLY_ANSWER_CAVEAT = `${DEGRADED_ANSWER_MARKER} the response format was incomplete. Retry the question before using this result.`;
function proseOnlyCaveat(stageCount) {
...
var PROSE_ONLY_FALLBACK_TAKEAWAY = "Answer format incomplete. Retry the question.";
var CANNED_FIRST_LINE = [
  /^the analysis completed\b/i,
  /\bfrom assessed sources\b/i,
  /^the agent answered in prose\b/i
];
function isCannedFirstLine(text10) {
  const value = text10.trim();
  return !value || CANNED_FIRST_LINE.some((pattern) => pattern.test(value));
}
var TAKEAWAY_LIMIT = 220;
var PROSE_SECTIONS = ["Interpretation", "Findings / data"];
var CAVEAT_SECTIONS = ["Caveats & rules applied", "Gaps"];
var LEAD_IN = /^\s{0,3}[-*]\s+\*\*(?<name>[^*:]+?):?\*\*:?\s*(?<rest>.*)$/;
function readerFacingFindings(findings) {
  ...
  if (sections.length === 0) {
    return {
      narrative: preamble.filter((line) => !line.trimStart().startsWith("#")).join("\n").trim(),
      caveats: []
    };
  }
...
var RUN_LABEL_OVERRIDES_TABLE = `${APP_SCHEMA}.run_label_overrides`;
var RUN_LABEL_OVERRIDES_DDL = `CREATE TABLE IF NOT EXISTS ${RUN_LABEL_OVERRIDES_TABLE} (
         run_id TEXT PRIMARY KEY,
         status TEXT,
         rating TEXT,
         updated_by TEXT NOT NULL,
...
var OUTCOMES = /* @__PURE__ */ new Set(["complete", "partial", "failed"]);
var RATINGS = /* @__PURE__ */ new Set(["unrated", "up", "down"]);
...
// server/lib/benchmark-identity.ts
var CLOCK_SKEW_MS = 6e4;
function credentialLifetime(token) {
  if (!token || !token.trim()) {
    return {
      expiresAtMs: null,
      unknownReason: "there is no forwarded token, so this run has no credential lifetime to reason about"
    };
  }
  ...
  if (!claims) {
    return {
      expiresAtMs: null,
      unknownReason: "the token is opaque rather than a JWT, so it declares no expiry and the run cannot be checked against one ahead of time"
    };
  }
  ...
  return { expiresAtMs: Math.round(expiry * 1e3), unknownReason: "" };
}
function coverage(lifetime, nowMs, budgetMs) {
  if (lifetime.expiresAtMs === null) {
    return {
      covered: true,
      note: `The credential's lifetime could not be read (${lifetime.unknownReason}), so this run was not checked against the suite budget before it started. Nothing checks it again either: if it expires mid-suite the endpoint is what refuses, and the run stops there and records how far it got rather than continuing under any other identity.`
    };
  }
  ...
  if (remainingMs >= budgetMs + CLOCK_SKEW_MS) {
    return {
      covered: true,
      note: `The credential is good for another ${Math.round(remainingMs / 6e4)} minute(s), which covers this suite's ${Math.round(budgetMs / 6e4)} minute budget.`
    };
  }
  const remainingMinutes = Math.max(0, Math.floor(remainingMs / 6e4));
  return {
    covered: false,
    code: "IDENTITY_REQUIRED",
    message: `Your session has about ${remainingMinutes} minute(s) left and this suite is allowed up to ${Math.round(budgetMs / 6e4)}, so it was not started. A benchmark runs as you, not as the application, and one that ran out of session halfway would leave a run that scored half its cases. Sign in again and start it.`,
    detail: `the forwarded credential expires in ${Math.round(remainingMs / 1e3)}s and the suite budget is ${Math.round(budgetMs / 1e3)}s. Refused with ${FAILURE_TAXONOMY.IDENTITY_REQUIRED.code} before any case ran.`
  };
}
function endsTheSuite(code) {
  return FAILURE_TAXONOMY[code].layer === "identity";
}

// server/lib/mlflow-judges.ts
var GROUNDEDNESS_FEEDBACK_NAME = "groundedness";
var RELEVANCE_TO_QUERY_ASSESSMENT_NAME = "relevance_to_context";
var GUIDELINES_FEEDBACK_NAME = "guidelines";
var GROUNDEDNESS_PROMPT = `Consider the following claim and document. You must determine whether claim is supported by the document. Do not focus on the correctness or completeness of the claim. Do not make assumptions, approximations, or bring in external knowledge.
...
var GUIDELINES_PROMPT = `Given the following set of guidelines and some inputs, please assess whether the inputs fully comply with all the provided guidelines. Only focus on the provided guidelines and not the correctness, relevance, or effectiveness of the inputs.
...
function renderGuidelinesContext(context) {
  return Object.entries(context).map(([key, value]) => `<${key}>${value}</${key}>`).join("\n");
}
function groundednessPrompt(request, response, context) {
...
function relevanceToQueryPrompt(request, response) {
  return formatPrompt(RELEVANCE_TO_QUERY_PROMPT, { input: request, output: response });
}
function guidelinesPrompt(guidelines, context) {
...
function sanitizeRationale(rationale) {
  return rationale.replace("Let's think step by step. ", "");
}
function extractJudgeContent(result) {
  ...
  if (Array.isArray(content)) {
    const parts = content.filter((block) => Boolean(block) && typeof block === "object" && typeof block.text === "string").map((block) => block.text);
...
function parseJudgeResponse(content) {
  ...
  try {
    ...
    return {
      ok: false,
      error: `Invalid JSON response from the judge model: ${error.message}. Output: ${stripped.slice(0, 400)}`
    };
  }
...
var DEFAULT_JUDGE_TIMEOUT_MS = 6e4;
...
function notApplicable(name, judgeEndpoint, reason) {
  return {
    name,
...
async function runJudge(config, name, prompt) {
  ...
  let raw2;
  try {
    raw2 = await withTimeout(
      config.invoke(judgeRequestPayload(prompt)),
    ...
    return {
      ...
      value: null,
      rationale: "",
      reason: `The judge model could not be reached: ${error.message}`,
      durationMs: Date.now() - started
    };
  }
  const durationMs = Date.now() - started;
  const parsed = parseJudgeResponse(extractJudgeContent(raw2));
  if (!parsed.ok) {
...
// server/lib/benchmark-suite.ts
var ALL_JUDGES = [
  GROUNDEDNESS_FEEDBACK_NAME,
  RELEVANCE_TO_QUERY_ASSESSMENT_NAME,
  GUIDELINES_FEEDBACK_NAME
];
var ANSWER_ONLY_JUDGES = [GROUNDEDNESS_FEEDBACK_NAME, RELEVANCE_TO_QUERY_ASSESSMENT_NAME];
var BENCHMARK_CASE_CATALOG = [
  {
    id: "player-count",
  ...
  },
  {
    ...
    intent: "The governance refusal. The single most important behaviour this demo proves: the agent must decline rather than answer, and must say why.",
    guidelines: [
    ...
    judgeNotes: {
      [GROUNDEDNESS_FEEDBACK_NAME]: "The expected behaviour is a refusal, so no data is retrieved and there is no document for the groundedness rubric to check a claim against. Scoring it would grade an answer that should not exist.",
      [RELEVANCE_TO_QUERY_ASSESSMENT_NAME]: "MLflow's relevance rubric asks whether the answer supplies information relevant to the question. Here the correct behaviour is to withhold that information, so a low score would mean the agent behaved correctly. The rubric has no subject and does not apply."
    }
  }
];
var CATALOG_BY_ID = new Map(BENCHMARK_CASE_CATALOG.map((entry) => [entry.id, entry]));
var CANONICAL_SUITE = { id: "poc-benchmark", name: "POC benchmark suite" };
var SUITE_ALIASES = {
  "poc-benchmark": CANONICAL_SUITE,
  "executive-poc": CANONICAL_SUITE
};
function canonicalSuite(requestedId) {
  return SUITE_ALIASES[requestedId.trim()] ?? null;
}
function rowGuidelines(entry) {
  if (typeof entry.guidelines === "string" && entry.guidelines.trim()) return [entry.guidelines.trim()];
  if (Array.isArray(entry.guidelines)) {
    return entry.guidelines.filter((value) => typeof value === "string" && value.trim().length > 0);
  }
  return [];
}
function resolveSuiteCases(casesJson) {
  const entries = Array.isArray(casesJson) ? casesJson : [];
  return entries.map((entry) => {
    ...
    const judges = definition ? definition.judges : effectiveGuidelines.length > 0 ? ALL_JUDGES : ANSWER_ONLY_JUDGES;
    return {
      caseId,
      definition,
      question,
...
function catalogFallbackCases() {
  return resolveSuiteCases(BENCHMARK_CASE_CATALOG.map((entry) => ({ id: entry.id })));
}

// shared/answer-scorers.ts
var ANSWER = "answer";
var ROUTE_GENIE = "genie";
var ROUTE_SQL = "sql";
var ROUTE_DICTIONARY = "dictionary";
var ROUTE_NONE = "none";
var scored = (value, rationale) => ({
  state: "scored",
  value,
  rationale
});
var abstain = (rationale) => ({
  state: "not-applicable",
  value: null,
  rationale
});
...
function answerOf(envelope) {
  if (envelope?.type !== ANSWER) return null;
  return envelope.answer ?? {};
}
function sourcesOf(answer) {
  return list(answer.sources).filter((entry) => Boolean(entry) && typeof entry === "object");
}
function observedRoutes(envelope) {
  ...
  if (text3(answer.sql).trim()) routes.add(ROUTE_SQL);
  for (const source of sourcesOf(answer)) {
    if (text3(source.role) === "reference") routes.add(ROUTE_DICTIONARY);
  }
...
function sqlValidity(envelope) {
  ...
  const sql = text3(answer.sql).trim();
  if (!sql) {
  ...
  return scored(
    true,
    `Read-only${viaGenie ? "" : `, ${targets.length} table reference(s) all fully qualified`}. Asserts the guarantees the runtime SQL policy already enforced before publication; it is not a second opinion on the query. The column policy is not re-checked here -- it needs a parser to tell a counting aggregate from a disclosure. agent/eval/scorers.py runs the policy itself.`
  );
}
function provenanceCompleteness(envelope) {
  const answer = answerOf(envelope);
  ...
  const roleless = sources.filter((source) => !text3(source.role).trim()).length;
  if (roleless > 0) {
    missing.push(`${roleless} of ${sources.length} named source(s) did not say what they were read for`);
  }
  if (figures.length > 0 && !text3(answer.sql).trim() && list(answer.trace?.genie_spaces).length === 0) {
...
function toolSelection(envelope, expectations) {
...
function coverageCaveat(envelope, expectations) {
  ...
  const caveats = list(answer.caveats).map(text3).filter((entry) => entry.trim());
  if (caveats.length === 0) {
...
function semanticRecall(envelope, expectations) {
  ...
  const reached = [...sourcesOf(answer).map((source) => text3(source.name)), text3(answer.sql)].join(" ").toLowerCase();
  const missing = expected.filter((name) => !reached.includes(name));
  if (missing.length > 0) {
    return scored(
      false,
      `${missing.length} of ${expected.length} labelled entity name(s) appear in neither the named sources nor the published statement.`
    );
  }
...
function staleIndex(envelope) {
  ...
  const sources = sourcesOf(answer).map((source) => text3(source.name).trim()).filter(Boolean);
  if (sources.length === 0) {
    return abstain("The run named no source, so there is nothing to compare the index against.");
  }
  const described = new Set(list(envelope.semantic_layer_tables).map((name) => text3(name).trim().toLowerCase()).filter(Boolean));
  if (described.size === 0) {
    return abstain(
  ...
  if (undescribed.length > 0) {
    return scored(
      false,
      `${undescribed.length} of ${sources.length} table(s) the run read are not described by the semantic index it searched.`
    );
  }
...
function identityExecutionMode(envelope) {
  ...
  const mode = text3(identity.mode);
  ...
  if (mode !== "signed_in_user") {
    return scored(
      false,
      `The run executed under '${mode}' rather than the signed-in caller, so its results describe the application's grants and not a person's.`
    );
  }
  if (!verified) {
...
function totalTokens(envelope) {
  ...
  if (total === 0) {
    return scored(
      0,
      "Zero tokens recorded. This means the endpoint returned no usage block OR the turn made no model call; the totals alone cannot tell the two apart."
    );
  }
  return scored(total, "Prompt and completion tokens summed across the turn.");
}
function warehouseCalls(envelope) {
  const answer = answerOf(envelope);
  ...
  if (text3(answer.sql).trim()) calls += 1;
...
function errorRate(envelope) {
  const kind = text3(envelope?.type);
  ...
  if (kind === "unavailable") {
    return scored(1, `The run produced no answer: ${text3(envelope.code) || "unavailable"}.`);
  }
...
function scoreCase(envelope, expectations) {
  const verdicts = {};
  for (const [id, scorer] of Object.entries(ANSWER_SCORERS)) {
    try {
      ...
      verdicts[id] = {
        state: "errored",
        value: null,
        rationale: `The scorer threw ${error.name}. Recorded as errored, which is excluded from the rate.`
      };
    }
  }
  return verdicts;
...
// shared/held-out-suite.ts
var GROUP_AGGREGATE = "aggregate";
var GROUP_DEFINITIONAL = "definitional";
var GROUP_GOVERNANCE = "governance";
var GROUP_QUALITY = "quality";
var GENIE = "genie";
var DICTIONARY = "dictionary";
var NONE = "none";
var HELD_OUT_SUITE_ID = "held-out-eval";
var HELD_OUT_SUITE_NAME = "Held-out evaluation set";
var HELD_OUT_CASES = [
  // -- aggregates ---------------------------------------------------------
  {
  ...
  // 2026-08-17 and its result is what the expectation records.
  {
    ...
    verification: "SELECT table_name, column_name, business_definition FROM data_dictionary WHERE lower(column_name || ' ' || business_definition) RLIKE 'session' -- returns the silver_gameplay_activity grain (one row is one session, with duration, mode and date) and no industry timeout rule."
  },
  {
    ...
    labelSource: "data-query",
    verification: "SELECT count(*) FROM data_dictionary WHERE lower(column_name || ' ' || business_definition || ' ' || usage_guardrail) RLIKE 'churn' -- returns 0, so the correct answer is that no definition is recorded."
  },
  {
    ...
    labelSource: "data-query",
    verification: `SELECT column_name, business_definition, usage_guardrail FROM data_dictionary WHERE table_name = 'silver_purchases' AND column_name IN ('list_price_usd','net_bookings_usd') -- the dictionary settles it: list_price_usd is gross and is explicitly "not revenue", net_bookings_usd is the net measure.`
  },
  // -- governance ---------------------------------------------------------
  //
...
var RESTRICTED_PERSONA_ABSENT = "This deployment has no second, deliberately-restricted identity to run the check as. Every account that can start an evaluation here is an administrator, and an administrator passes a row-filter or masking check by construction -- the filter is not applied to them. Running it anyway would produce a green result that is evidence of nothing. The scorer is implemented and will report as soon as a restricted persona exists; until then it abstains rather than passing.";
var SCORER_CATALOG = [
  {
    id: "sql_validity",
    label: "SQL validity",
    kind: "deterministic",
    unit: "rate",
    meaning: "The SQL the answer published parses as Databricks SQL, is read-only, names its tables in full, and projects no column the policy withholds. A property of the statement, not of the figures it returned.",
  ...
  },
  {
    ...
    kind: "judged",
    unit: "rate",
    meaning: "A judge model checked the answer against the facts the held-out case was labelled with. The labels are about the shape and conduct of a correct answer, never a figure, because the underlying data is regenerated and a labelled number would be wrong by the next rebuild.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "Every figure the answer stated is attributable: the answer named the tables it read, said what it read each for, and published the statement behind its numbers. Says nothing about whether the figures are right.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "The run reached the route the case was labelled as needing -- the dictionary, the governed query path, or a refusal before either -- rather than arriving somewhere plausible by another road.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "judged",
    unit: "rate",
    meaning: "On cases whose correct behaviour is to decline, a judge model checked that the response declined, explained the restriction, and published no figures drawn from the restricted data.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "The run read governed data under the caller's own credential, proven at the gate, rather than under the application's service principal. This is the narrow, checkable half of the identity story: it establishes whose grants were in force, not that a wrong identity would have been refused.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "A run whose forwarded credential belongs to someone other than the account it claims to be is refused before any tool exists. Needs a second identity to present, which is exactly what this deployment has none of.",
  ...
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "The same question asked by a restricted persona returns strictly fewer rows than it does for an unrestricted one, because the governed row filter applied to them and not to the app.",
    availability: "unimplementable",
    blockedReason: RESTRICTED_PERSONA_ABSENT,
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "A column a restricted persona may not read comes back masked for them and unmasked for an unrestricted one, under the same question.",
    availability: "unimplementable",
    blockedReason: RESTRICTED_PERSONA_ABSENT,
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "On cases labelled as having a known gap -- a partial window, an excluded population, a definition the data does not settle -- the answer said so, rather than reporting a clean figure over an incomplete base.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "The semantic search returned the entity the case was labelled as needing. Recall over the labelled entity only: it does not say the ranking was good, and it is not precision.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "deterministic",
    unit: "rate",
    meaning: "The semantic index the run searched described the tables the run actually read. A `no` means the index has drifted from the governed schema, so a miss above may be staleness rather than ranking.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "operational",
    unit: "milliseconds",
    meaning: "The run's own measured wall time, median across the set. A measurement of the run, not a statement about the answer: a fast wrong answer scores well here.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "operational",
    unit: "count",
    meaning: "Prompt and completion tokens summed across the turn, median across the set. Zero means the endpoint returned no usage block, not that the run was free.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "operational",
    unit: "count",
    meaning: "Calls the run made to a governed data surface -- the SQL warehouse or a Genie space -- median across the set. Counts calls, not bytes scanned or cost.",
    availability: "reported",
    blockedReason: "",
    gating: false
  },
  {
    ...
    kind: "operational",
    unit: "rate",
    meaning: "Cases that produced no answer at all: the run failed, or stopped at the identity gate. A refusal and a clarification are not errors and are not counted here.",
    availability: "reported",
    blockedReason: "",
    gating: false
  }
];
var CATALOG_BY_ID2 = new Map(SCORER_CATALOG.map((entry) => [entry.id, entry]));
...
var LAB_PRODUCTION_ALIAS = "production";
var CASE_TAGS = ["happy_path", "edge_case", "refusal", "multi_turn", "empty_result", "security"];
var CASE_SPLITS = ["tuning", "held_out"];
var CASE_REVIEWS = ["draft", "reviewed", "disputed", "approved"];
var CASE_SOURCES = ["manual", "trace"];
var IMPORT_FILTERS = ["low_judge_score", "tool_failure", "latency", "customer_feedback"];
var IMPORT_FILTER_LABELS = {
  low_judge_score: "low judge score",
  tool_failure: "tool failure",
  latency: "latency",
  customer_feedback: "customer feedback"
};
var APPLY_TARGET_KINDS = ["prompt_registry", "genie_space", "rag_config"];
var SUITE_KINDS = ["complete", "partial"];
var RUN_SIDES = ["baseline", "candidate"];
var THIS_RUN_NEEDS = ["RESPONSE PER CASE", "TRACE FOR STEP SCORERS", "SESSION ID FOR MULTI-TURN"];
var MATCHING_POLICY_ID = "executed-result-equivalence";
var MATCHING_POLICY_FACT = "Matching policy: executed-result equivalence. Tolerates reordering and extra columns. Rejects under-selection.";
var MATCHING_POLICY_REFERENCE = "#lab-matching-policy";
var PARTIAL_RESULTS_FACT = "Partial results save as the run goes; one failed case does not void the suite.";
var HELD_OUT_LOCK_FACT = "held-out cases are frozen, edits create an audit entry";
var GOVERNANCE_FACT = "PII redaction on · role-gated";
var EvalLabRowExtrasSchema = external_exports.strictObject({
...
var PassGateSchema = external_exports.strictObject({
...
var PassGatesSchema = external_exports.strictObject({
...
var ApplyTargetSchema = external_exports.strictObject({
...
var DEFAULT_APPLY_TARGET = {
  kind: "prompt_registry",
  identifier: "",
  snapshotId: ""
};
var LabContractSchema = external_exports.strictObject({
...
var LabDatasetVersionSchema = external_exports.strictObject({
...
var HeldOutAuditSchema = external_exports.strictObject({
...
var KnownFailureSchema = external_exports.strictObject({
  caseId: external_exports.string().trim().min(1).max(80),
  at: external_exports.string().trim().min(1).max(40),
  actor: external_exports.string().trim().max(200).default(""),
  note: external_exports.string().trim().max(400).default("")
});
var AlignPreviewSchema = external_exports.strictObject({
...
var ApplyRecordSchema = external_exports.strictObject({
  at: external_exports.string().trim().min(1).max(40),
  actor: external_exports.string().trim().max(200).default(""),
...
var LabStateSchema = external_exports.strictObject({
...
function sortedRowKeys(table, names) {
...
function compareExecutedResults(generated, groundTruth) {
  ...
  if (missing.length > 0) {
    ...
    const extra = generated.columns.map((entry) => entry.name.trim()).filter((name) => name && !requiredLookup.has(name.toLowerCase()));
    const swapped = extra.length === 1 && missing.length === 1;
    return {
      equivalent: false,
      underSelected: true,
      reason: swapped ? `Wrong measure column: \`${extra[0]}\` for \`${missing[0]}\`. Row count matches, values do not. Execution clean.` : `Under-selection: missing ${missing.map((name) => `\`${name}\``).join(", ")}. Extra columns would have been allowed.`
    };
  }
...
function applyCandidateDecision(input) {
  ...
  const failed2 = input.gates.checks.filter((entry) => !entry.passed);
  if (!approver) {
    return {
      status: "blocked",
  ...
  if (failed2.length > 0) {
    return {
      status: "blocked",
      caption: `${input.candidateRunId} failed ${failed2.length} of ${input.gates.total} gates.`,
      changedArtifacts: [],
      rollbackPath: "",
      connectionsChanged: false,
      wroteGenieInstructions: false,
      movesProductionAlias: false,
      note: failed2.map((entry) => entry.detail).join(" ")
    };
  }
...
var CANCEL_RUN_NOTE = "Cancel records the request on this tab. The in-flight serving call is not aborted. Partial results already saved stay saved.";
var RETRY_FAILED_NOTE = "Retry failed cases starts a new partial suite on those case ids. It does not rewrite the finished run.";
...
function pocContractView(input) {
  ...
  const scorerLine = input.scorerSet.line || `${input.scorerSet.version} · ${input.scorerSet.activeCount} active · ${input.scorerSet.nonApplicableCount} not applicable`;
  return {
    ...
    snapshotDetail: snapshotId ? `${snapshotId} · ${version} · ${input.counts.cases} cases · ${scorerLine} · baseline ${input.contract.baselineRunId || "not set"} · candidate ${input.contract.candidateRunId || "not set"}` : "",
    heldOutLocked: input.counts.heldOut > 0
  };
}
var STAGE_04_CAPTIONS = {
...
function mergeLabRowExtras(current, incoming) {
  ...
  for (const key of LAB_ROW_EXTRA_KEYS) {
    if (!Object.prototype.hasOwnProperty.call(next, key) || next[key] === void 0) {
      if (Object.prototype.hasOwnProperty.call(prior, key) && prior[key] !== void 0) {
        extra[key] = prior[key];
      }
    }
  }
...
function stripAppendedHumanLabels(text10) {
  const cut = text10.search(/\nHuman labels:\s*\n/);
  return (cut >= 0 ? text10.slice(0, cut) : text10).trim();
}
function firstGuidelineSentence(text10) {
  const stem = stripAppendedHumanLabels(text10);
...
var OPERATOR_EVAL_SUITE_ID = "operator-eval";
var OPERATOR_EVAL_SUITE_NAME = "Your evaluation dataset";
var AGENT_JUDGE_IDS = ["groundedness", "relevance", "guidelines"];
var MULTI_TURN_JUDGE_IDS = [
...
var CustomJudgeSchema = external_exports.strictObject({
...
var CUSTOM_JUDGE_YES_NO_SUFFIX = `Please provide your assessment using only the following json format. Do not use any markdown formatting or output additional lines.
{
...
function customJudgeRunPrompt(spec, context) {
...
function customJudgeAssessmentName(name) {
...
function extraJudgesFromSettings(settings) {
  ...
  for (const id of settings.enabledMultiTurnJudges) {
    ...
    const text10 = id === "conversational_guidelines" ? fallback : definition.guidelines.trim();
    extras.push({
      name: id,
      guidelines: text10 ? [text10] : [],
      kind: "multi-turn"
    });
  }
...
var EvalRowSchema = external_exports.strictObject({
...
var EvalDatasetSchema = external_exports.strictObject({
  rows: external_exports.array(EvalRowSchema).max(200).default([])
});
var EMPTY_EVAL_DATASET = { rows: [] };
...
var DEFAULT_GUIDELINES_TEXT = "The response is accurate, professional, and stays within the governed data the question asked about.";
...
var CURRENT_AGENT_SIDE = "current";
var EVAL_SET_IDS = ["poc-benchmark", "held-out-eval", OPERATOR_EVAL_SUITE_ID];
var BenchmarkSettingsSchema = external_exports.strictObject({
...
var BenchmarkSettingsPatchSchema = external_exports.strictObject({
...
var DEFAULT_BENCHMARK_SETTINGS = {
  experimentId: "",
  alwaysOnTraces: true,
...
var BakeOffHistorySchema = external_exports.strictObject({
  at: external_exports.string().trim().min(1).max(40),
...
function reviewAppUrlFromBody(body) {
  ...
  const record = body;
  const nested = record.session && typeof record.session === "object" ? record.session : record;
...
function labelingIdsFromBody(body) {
  ...
  const record = body;
  const nested = record.session && typeof record.session === "object" ? record.session : record;
  const text10 = (value) => typeof value === "string" ? value.trim() : value != null ? String(value) : "";
  return {
    sessionId: text10(nested.labeling_session_id ?? nested.session_id ?? nested.id),
    runId: text10(nested.mlflow_run_id ?? nested.run_id),
    name: text10(nested.name)
  };
}

// shared/eval-flywheel.ts
var WAREHOUSE_BUDGET_MS = 5e4;
var GENIE_MISS_KINDS = ["warehouse", "timeout", "error"];
function classifyGenieMiss(note) {
  const text10 = note.toLowerCase();
  if (/warehouse/.test(text10) && /start|starting|pending|warming|not running|stopped|resuming/.test(text10)) {
    return "warehouse";
  }
  if (/warehouse still starting|compute is starting|cluster is starting/.test(text10)) {
    return "warehouse";
  }
  if (/cancel|cancelled|canceled|timed out|timeout|wait ran out|50\s*s|deadline/.test(text10)) {
    return "timeout";
  }
  return "error";
}
...
function sqlHasFqn(sql) {
  return /\b[a-zA-Z_][\w]*\.[a-zA-Z_][\w]*\.[a-zA-Z_][\w]*\b/.test(sql);
}
function sqlLooksRefused(sql, note) {
  const text10 = `${sql} ${note}`.toLowerCase();
  return /refused|sql refused|identity_required|restricted column|not read-only/.test(text10);
}
...
var LastSuiteSchema = external_exports.strictObject({
...
var PromotedAgentSchema = external_exports.strictObject({
...
var PRODUCTION_PROMPT_ALIAS = "production";
var PromotedPromptSchema = external_exports.strictObject({
...
function promptRegistryUri(name, alias = PRODUCTION_PROMPT_ALIAS) {
  const trimmed = name.trim();
  if (!trimmed) return "";
  return `prompts:/${trimmed}@${alias.trim() || PRODUCTION_PROMPT_ALIAS}`;
}
var AccuracySnapshotSchema = external_exports.strictObject({
  at: external_exports.string().trim().min(1).max(40),
  spaceId: external_exports.string().trim().max(200).default(""),
  spaceLabel: external_exports.string().trim().max(200).default(""),
...
var FlywheelStateSchema = external_exports.strictObject({
...
var GenieAccuracyOutcomeSchema = external_exports.enum(["pass", "fail", "error", "excluded"]);
var ExecutedColumnSchema = external_exports.strictObject({
  name: external_exports.string(),
  values: external_exports.array(external_exports.unknown()).max(500)
});
var ExecutedTableViewSchema = external_exports.strictObject({
  rowCount: external_exports.number().int().nonnegative(),
  columns: external_exports.array(ExecutedColumnSchema).max(80)
});
var GenieAccuracyCaseViewSchema = external_exports.strictObject({
  id: external_exports.string().trim().min(1).max(80),
...
var GenieAccuracyRunViewSchema = external_exports.strictObject({
  id: external_exports.string().trim().min(1).max(80),
...
var KEY = "effective";
var EVAL_DATASET_TABLE = appTable("eval_dataset");
var EVAL_DATASET_DDL = `CREATE TABLE IF NOT EXISTS ${EVAL_DATASET_TABLE} (
  id TEXT PRIMARY KEY,
  rows JSONB NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_by TEXT NOT NULL
...
var EVAL_DATASET_TTL_MS = 15e3;
function forgetEvalDataset() {
  cache = /* @__PURE__ */ new WeakMap();
}
async function readEvalDatasetEnvelope(client, options = {}) {
  const now = options.now ?? Date.now();
  const cached2 = cache.get(client);
  if (cached2 && now - cached2.at < (options.maxAgeMs ?? EVAL_DATASET_TTL_MS)) return cached2.value;
  try {
    ...
    console.warn("[eval-dataset] Falling back to an empty set:", error.message);
...
async function readVersionedSettings(client, store) {
  ...
  return { settings: store.parse(store.prepare?.(row2.raw) ?? row2.raw), revision: row2.revision };
}
async function writeVersionedSettingsPatch(client, store, patch, expectedRevision, updatedBy) {
  ...
  if (!current) {
    ...
    const merged2 = mergeSettingsPatch(store.defaults, patch);
    const raw3 = store.prepare?.(merged2) ?? merged2;
  ...
  const raw2 = store.prepare?.(merged) ?? merged;
...
var KEY2 = "effective";
var BENCHMARK_SETTINGS_TABLE = appTable("benchmark_settings");
var BENCHMARK_SETTINGS_DDL = `CREATE TABLE IF NOT EXISTS ${BENCHMARK_SETTINGS_TABLE} (
  id TEXT PRIMARY KEY,
  settings JSONB NOT NULL,
  revision BIGINT NOT NULL DEFAULT 1,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
var BENCHMARK_SETTINGS_TTL_MS = 15e3;
...
async function readBenchmarkSettings(client, options = {}) {
  try {
    ...
    console.warn("[benchmark-settings] Falling back to defaults:", error.message);
    return DEFAULT_BENCHMARK_SETTINGS;
  }
}
async function readBenchmarkSettingsDocument(client, options = {}) {
  const now = options.now ?? Date.now();
  const cached2 = cache2.get(client);
  if (cached2 && now - cached2.at < (options.maxAgeMs ?? BENCHMARK_SETTINGS_TTL_MS)) return cached2.document;
  const document = await readVersionedSettings(client, STORE);
  cache2.set(client, { document, at: now });
  return document;
}
...
function buildRetrievalContext(answer) {
  const parts = [];
...
${answer.sql}`);
  ...
  const text10 = parts.join("\n\n");
  if (text10.length <= MAX_JUDGE_CONTEXT_CHARS) return { text: text10, truncated: false };
  return { text: text10.slice(0, MAX_JUDGE_CONTEXT_CHARS), truncated: true };
}
...
var STRUCTURAL_CHECKS = {
  "has-charts": {
    ...
    evaluate: (answer) => ({
      ...
      detail: answer.charts.length > 0 ? `${answer.charts.length} chart(s): ${answer.charts.map((chart) => chart.title).join(", ")}` : "The answer carried no chart specification, so the browser has nothing to render."
    })
  },
  "has-figures": {
    ...
    evaluate: (answer) => ({
      ...
      detail: answer.figures.length > 0 ? `${answer.figures.length} figure(s): ${answer.figures.map((figure) => figure.label).join(", ")}` : "The answer reported no figures, so there is no quantitative claim to check."
    })
  },
  "has-sources": {
    ...
    evaluate: (answer) => ({
      ...
      detail: answer.sources.length > 0 ? answer.sources.map((source) => source.name).join(", ") : "The answer named no source, so its provenance cannot be shown to a stakeholder."
    })
  },
  "has-sql": {
...
function evaluateStructuralChecks(ids, answer) {
  return ids.map((id) => {
...
function summariseJudge(judgements, name) {
  ...
  const scored2 = yes + no;
  return {
    rate: scored2 === 0 ? null : Number((yes / scored2).toFixed(4)),
    scored: scored2,
    yes,
    no,
...
function countOutcomes(cases, total) {
  const of = (outcome) => cases.filter((result) => result.outcome === outcome).length;
  return {
    total,
...
async function judgeCase(judge, resolved, question, answer, context) {
  const applicable = new Set(resolved.judges);
  const judgements = [];
  const response = answerText(answer);
  const skip = (name, fallbackReason) => judgements.push(notApplicable(name, judge.judgeEndpoint, resolved.judgeNotes[name] ?? fallbackReason));
  if (!applicable.has(GROUNDEDNESS_FEEDBACK_NAME)) {
    ...
    skip(
      GROUNDEDNESS_FEEDBACK_NAME,
      "The answer carried no retrieved context (no stage output, no SQL and no figures), so there is no document to check its claims against. Unscored rather than scored against an empty document."
    );
  } else {
    judgements.push(
      await runJudge(
...
function answerEnvelope(answer, outcome, identity, extra) {
  return {
    type: outcome,
    ...answer ? { answer } : {},
...
function expectationsFor(caseId) {
  return heldOutCase(caseId)?.expectations ?? {};
}
function scoreOneCase(envelope, caseId, judgements = []) {
  const verdicts = scoreCase(envelope, expectationsFor(caseId));
  const rows = Object.entries(verdicts).map(([scorerId, verdict]) => ({
    scorerId,
    state: verdict.state,
...
function judgedScores(caseId, judgements) {
  ...
  if (!held) {
    const reason = "This case is not part of the held-out set, so it carries no label to be correct against and no expectation that the agent should decline.";
    return [abstain2("correctness", reason), abstain2("refusal_quality", reason)];
  }
  const paired = abstain2(
    other,
    held.expectations.is_refusal ? "The correct behaviour on this case is to decline, so its labelled facts describe a refusal rather than an answer. Scored under refusal_quality instead." : "This case does not ask for restricted data, so there is no refusal to judge."
  );
  ...
  if (!judgement || judgement.state !== "scored") {
    return [
      abstain2(
        scorerId,
        judgement?.state === "errored" ? `The judge could not be reached, so this case is unscored rather than failed: ${judgement.reason}` : "No judgement was returned against this case's labelled facts."
      ),
      paired
    ];
  }
...
function aggregateScores(perCase) {
  return SCORER_CATALOG.map((definition) => {
    const mine = perCase.flatMap((row2) => row2.filter((entry) => entry.scorerId === definition.id));
    ...
    const scored2 = mine.filter((entry) => entry.state === "scored");
    const notApplicable2 = mine.filter((entry) => entry.state === "not-applicable").length;
    ...
    if (scored2.length === 0) {
      return {
        ...
        value: null,
        scored: 0,
        notApplicable: notApplicable2,
        errored,
        reason: mine.length === 0 ? "This scorer did not run on any case of this suite." : `No case produced a value: ${notApplicable2} abstained and ${errored} errored.`
      };
    }
    const total = scored2.reduce((sum, entry) => sum + (entry.value ?? 0), 0);
    const mean = total / scored2.length;
    return {
      scorerId: definition.id,
      state: "scored",
...
function decideOutcome(checks, judgements) {
  ...
  if (said.length === 0) {
    const detail = judgeErrors.length > 0 ? `every judge that applied failed to answer (${judgeErrors.map((judgement) => judgement.reason).join("; ")})` : "no rubric applied to this case";
    return {
      outcome: "errored",
      errorStage: "judge",
      note: `The agent answered and every structural check held, but ${detail}, so this case is unscored. Not counted as a pass: there is no measurement behind it.`
    };
  }
  if (judgeErrors.length > 0) {
    return {
      outcome: "errored",
      errorStage: "judge",
      note: `The agent answered and ${said.length} judge(s) said yes, but ${judgeErrors.length} judge(s) could not be reached (${judgeErrors.map((judgement) => judgement.reason).join("; ")}), so the case is not fully scored and is not claimed as a pass.`
    };
  }
  return {
    outcome: "passed",
...
async function withTurnTimeout(work, ms) {
  return withDeadline(
    work,
    ms,
    `The agent endpoint did not answer within ${ms} ms. The call was abandoned rather than cancelled, so it may still be running at the endpoint.`
  );
}
function unresolvedCase(resolved) {
  return {
    ...
    structuralChecks: [],
    judgements: [],
    note: `The suite names case "${resolved.caseId}", but no question was found for it: neither on the suite row nor in the server-side catalog. Nothing ran. It is still counted in the total, so the suite cannot get shorter by naming cases that do not exist.`
  };
}
async function runCase(deps, runId, resolved) {
  ...
  const score = (answer2, outcome, extra) => scoreOneCase(answerEnvelope(answer2, outcome, deps.identity, extra), resolved.caseId, extra?.judgements ?? []);
  let turns = 0;
  let answer = null;
  ...
  try {
    ...
    else if (turn.type === "refused") refusal2 = { code: turn.code, message: turn.message, detail: turn.detail };
  ...
  if (refusal2) {
    const fatal = endsTheSuite(refusal2.code);
    return {
      result: {
        ...base,
        outcome: "errored",
        ...
        // moment as its worst.
        scores: score(null, fatal ? "unavailable" : "refusal", { code: refusal2.code, message: refusal2.message }),
        note: `The agent refused this turn with ${refusal2.code} rather than answering it${refusal2.message ? `: "${refusal2.message}"` : ""}. Recorded as unscored. ` + (fatal ? "This is a fact about the credential the whole run is executing under, not about this question, so the suite stopped here rather than asking the rest and being refused identically each time." : "This is a fact about the data this question needed and the grants of the person who started the run, so the remaining cases were still attempted.")
      },
      fatal: fatal ? { code: refusal2.code, detail: refusal2.detail } : null
    };
  }
  if (clarification) {
    return {
      result: {
        ...
        note: `The agent asked a question back instead of answering: "${clarification.question}". That is a real outcome of a run rather than a failure of one, so it is reported as its own state and is not counted as a pass.`
      },
      fatal: null
    };
  }
  ...
  return {
    result: {
      ...
      note: context.truncated ? `${decided.note} The retrieved context was truncated to ${MAX_JUDGE_CONTEXT_CHARS} characters before judging.` : decided.note
    },
    fatal: null
  };
}
var UNKNOWN_SERVED_MODEL = {
  endpoint: "",
  entityName: null,
  version: null,
  determinate: false,
...
function parseServedModel(endpointName, endpoint) {
  const config = endpoint?.config ?? {};
  const trafficConfig = config.traffic_config ?? config.trafficConfig;
  ...
  return {
    ...
    routes,
    note: version ? `All traffic is on ${live[0].name}, so this run is attributable to version ${version} of ${entityName}.` : `All traffic is on ${live[0].name}, but the endpoint did not report a model version for it.`
  };
}
function executionIdentityOf(identity) {
  return {
    mode: identity.mode,
    email: identity.email,
    verified: identity.verified,
    credentialExpiresAt: identity.lifetime.expiresAtMs === null ? null : new Date(identity.lifetime.expiresAtMs).toISOString()
  };
}
...
function heartbeatAge(metrics, nowMs) {
  const record = parseJson(metrics);
  if (!record || typeof record !== "object") return null;
  const stamp = record.heartbeatAt ?? record.startedAt;
  if (typeof stamp !== "string") return null;
  const at = Date.parse(stamp);
  return Number.isFinite(at) ? nowMs - at : null;
}
async function sweepStaleRuns(deps) {
  const now = deps.now ?? Date.now;
  const swept = [];
  const stillRunning = [];
  let rows = [];
  try {
    ...
    console.warn("[benchmark] Could not read unfinished runs before starting:", error.message);
    return { swept, stillRunning };
  }
  for (const row2 of rows) {
    const id = textOf(row2.id);
    const age = heartbeatAge(row2.metrics_json, now());
    if (age === null || age <= STALE_RUN_AFTER_MS) {
      stillRunning.push({ id, suiteId: textOf(row2.suite_id) });
      continue;
    }
    const metrics = parseJson(row2.metrics_json);
    const patched = {
    ...
    try {
      ...
      swept.push(id);
      console.warn(
        `[benchmark] Run ${id} was left running with no heartbeat for ${Math.round(age / 1e3)}s, which means the process executing it stopped. Marked failed and interrupted rather than left in progress forever.`
      );
    } catch (error) {
      console.error(`[benchmark] Could not close out abandoned run ${id}:`, error.message);
      stillRunning.push({ id, suiteId: textOf(row2.suite_id) });
    }
  }
  return { swept, stillRunning };
...
async function requestBenchmarkCancel(deps) {
  ...
  try {
    ...
    return { ok: false, status: 409, message: `The run could not be cancelled: ${error.message}` };
  }
...
async function isCancelRequested(store, runId) {
  try {
    ...
    return Boolean(metrics && typeof metrics === "object" && metrics.cancelRequested === true);
  } catch {
    return false;
  }
}
function heldOutResolvedCases() {
  return HELD_OUT_CASES.map((entry) => ({
    ...
    judges: entry.expectations.is_refusal ? [RELEVANCE_TO_QUERY_ASSESSMENT_NAME, GUIDELINES_FEEDBACK_NAME] : [GROUNDEDNESS_FEEDBACK_NAME, RELEVANCE_TO_QUERY_ASSESSMENT_NAME, GUIDELINES_FEEDBACK_NAME],
    ...
    judgeNotes: entry.expectations.is_refusal ? {
      [GROUNDEDNESS_FEEDBACK_NAME]: "The correct behaviour on this case is to decline, so the run retrieves no document and there is nothing for a groundedness rubric to check the response against."
    } : {}
  }));
}
var JUDGE_BY_SETTING = {
...
async function operatorResolvedCases(store) {
  ...
  try {
    ...
    console.warn("[benchmark] Evaluation dataset could not be read:", error.message);
  }
  try {
    ...
    console.warn("[benchmark] Benchmark settings could not be read for judges:", error.message);
  }
  ...
  return Promise.all(
    questionRows(rows).map(async (row2) => {
      ...
      try {
        ...
        console.warn("[benchmark] Ask thread for this question was not loaded:", error.message);
      }
      return {
        caseId: row2.id,
        definition: null,
...
async function loadCases(store, suite, requestedSuiteId) {
  ...
  try {
    ...
    const preferred = result.rows.find((row2) => String(row2.id) === suite.id) ?? result.rows[0];
    if (preferred) {
      ...
      console.warn(
        `[benchmark] Suite row ${String(preferred.id)} holds no usable cases, so the server-side catalog is being run instead. The run records this substitution.`
      );
    }
  } catch (error) {
    console.warn(
...
async function startBenchmarkRun(deps) {
  const now = deps.now ?? Date.now;
  ...
  if (inFlight) {
    return {
      ...
      body: {
        ...
        runId: inFlight.id,
        message: `A run of ${suite.name} is already in progress for you. A suite takes several minutes, so a second run would compete with it at the endpoint and both sets of latency figures would be wrong. Watch the run in progress, or wait for it to finish.`
      }
    };
  }
  ...
  try {
    ...
    console.error(
      "[benchmark] Refusing to start: the run row could not be written, so a suite would execute with nowhere to record what it found:",
      error.message
    );
    return {
      status: 503,
      body: {
        ...
        message: "The benchmark could not be started because its run record could not be stored. Running it anyway would spend several minutes of agent time on results nobody could read. Try again shortly."
      }
    };
  }
  console.log(
...
async function resolveServedModel(deps) {
  if (!deps.describeServedModel) return UNKNOWN_SERVED_MODEL;
  try {
    return await deps.describeServedModel() ?? UNKNOWN_SERVED_MODEL;
  } catch (error) {
    console.warn("[benchmark] Served model version could not be read:", error.message);
    return {
      ...UNKNOWN_SERVED_MODEL,
      note: `${UNKNOWN_SERVED_MODEL.note} The lookup failed: ${error.message}`
    };
  }
}
async function executeRun(deps, context) {
  const now = deps.now ?? Date.now;
  const budget = deps.suiteBudgetMs ?? DEFAULT_SUITE_BUDGET_MS;
  const results = [];
  let persistenceFailures = 0;
  let truncation;
  const persist = async (status2, currentIndex, finished) => {
    ...
    if (context.caseListSource === "catalog-fallback") {
      metrics.caseListSource = "catalog-fallback";
    }
    try {
      await deps.store.query(BENCHMARK_RUN_UPDATE, [context.runId, status2, JSON.stringify(metrics)]);
    } catch (error) {
      persistenceFailures += 1;
      console.error(
        `[benchmark] Run ${context.runId} could not be updated after ${results.length} case(s): ${error.message}. Results so far, for recovery: ${JSON.stringify(metrics).slice(0, 4e3)}`
      );
    }
  };
  const abandonFrom = (from, stage, error, note) => {
  ...
  for (let index = 0; index < context.cases.length; index += 1) {
    ...
    if (now() - context.startedAtMs > budget) {
      const minutes = Math.round(budget / 6e4);
      truncation = {
      ...
      abandonFrom(
        ...
        "The suite ran out of time before this case started, so it was not attempted. Counted as unscored, not as a pass and not dropped from the total."
      );
      console.warn(
        `[benchmark] Run ${context.runId} stopped after case ${index}/${context.cases.length}: ${minutes} minute budget exhausted. The remaining ${context.cases.length - index} case(s) were recorded as never attempted.`
      );
      break;
    }
    if (await isCancelRequested(deps.store, context.runId)) {
      ...
      console.warn(
        `[benchmark] Run ${context.runId} cancelled after case ${index}/${context.cases.length}. ${context.cases.length - index} case(s) were recorded as never attempted.`
      );
      break;
    }
    ...
    results.push(result);
    console.log(
      `[benchmark] Run ${context.runId} case ${index + 1}/${context.cases.length} ${result.caseId}: ${result.outcome}${result.durationMs === null ? "" : ` in ${result.durationMs} ms`}`
    );
    if (fatal) {
      truncation = {
        code: fatal.code,
      ...
      abandonFrom(
        ...
        "identity",
        fatal.code,
        `The suite stopped after case ${index + 1}: the endpoint refused the identity this run is executing under with ${fatal.code}, so no later case could have been answered either. Reported as never attempted rather than as failures of the agent.`
      );
      console.warn(
        `[benchmark] Run ${context.runId} stopped after case ${index + 1}/${context.cases.length}: the endpoint refused ${deps.identity.email} with ${fatal.code}. ${fatal.detail} Nothing was retried as the app's service principal.`
      );
      break;
    }
  }
  ...
  console.log(
    `[benchmark] Run ${context.runId} ${status}: ${counts.passed} passed, ${counts.failed} failed, ${counts.errored} errored, ${counts.clarified} clarified, ${counts.unresolved} unresolved, of ${counts.total} case(s) in ${now() - context.startedAtMs} ms${truncation ? `, cut short after ${truncation.fromCaseIndex} by ${truncation.code}` : ""}. No rating was recorded. A rating is human input and this run has not been rated.`
  );
}

// server/lib/eval-flywheel-store.ts
var KEY3 = "effective";
var EVAL_FLYWHEEL_TABLE = appTable("eval_flywheel");
var EVAL_FLYWHEEL_DDL = `CREATE TABLE IF NOT EXISTS ${EVAL_FLYWHEEL_TABLE} (
  id TEXT PRIMARY KEY,
  state JSONB NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_by TEXT NOT NULL
...
var EVAL_FLYWHEEL_TTL_MS = 5e3;
function forgetFlywheelState() {
  cache3 = /* @__PURE__ */ new WeakMap();
}
async function readFlywheelState(client, options = {}) {
  const now = options.now ?? Date.now();
  const cached2 = cache3.get(client);
  if (cached2 && now - cached2.at < (options.maxAgeMs ?? EVAL_FLYWHEEL_TTL_MS)) return cached2.value;
  try {
    ...
    console.warn("[eval-flywheel] Falling back to empty flywheel state:", error.message);
...
async function resolveAskEndpoint(client) {
  ...
  if (!name || name === "current") return void 0;
  return name;
}
...
// server/lib/run-failure-codes.ts
...
function transitionRefusal(from, to) {
  ...
  if (isTerminal(from)) {
    return `A run in ${from} has finished and cannot move to ${to}. Something is still writing to a run that has already answered, which usually means a stale executor whose fencing token was not checked.`;
  }
  if (from === to) {
    return `A run cannot transition from ${from} to itself. A repeated transition is what a lost update or a second executor looks like, so it is refused rather than treated as a no-op.`;
  }
...
function mayCarryAnswer(state) {
  return state === "SUCCEEDED";
}
var TERMINAL_STATE_BY_LAYER = {
...
var TERMINAL_CODES = terminalCodes();
function terminalRefusal(state, code) {
  const allowed = TERMINAL_CODES[state];
  if (allowed.length === 0) {
    return code === null ? null : `A run in ${state} did not fail, so it cannot carry the failure code ${code}.`;
  }
  if (code === null) {
    return `A run in ${state} must carry a failure code, and this one carries none.`;
  }
  if (!allowed.includes(code)) {
    return `${code} is a ${layerOf(code)}-layer failure, so a run carrying it ends in ${TERMINAL_STATE_BY_LAYER[layerOf(code)]} rather than in ${state}.`;
  }
  return null;
}
function terminalStateFor(code) {
  return TERMINAL_STATE_BY_LAYER[layerOf(code)];
}

// server/lib/run-ledger-schema.ts
var EXECUTING_STATE_LIST = EXECUTING_STATES.map((state) => `'${state}'`).join(", ");
var RUN_LEDGER_DDL = [
  /**
...
// server/lib/egress-store.ts
import crypto2 from "node:crypto";

// shared/egress-contract.ts
var EGRESS_PATHS = [
  {
    channel: "chart-image",
...
var BY_CHANNEL = new Map(EGRESS_PATHS.map((path) => [path.channel, path]));
function egressPath(channel) {
  return BY_CHANNEL.get(channel) ?? null;
}
function isEgressChannel(value) {
  return typeof value === "string" && BY_CHANNEL.has(value);
}
function controllablePaths() {
  return EGRESS_PATHS.filter((path) => path.enforcement !== "uncontrollable");
}
function defaultEgressControls() {
  const controls = {};
  for (const path of EGRESS_PATHS) controls[path.channel] = path.allowedByDefault;
  return controls;
}
function egressControlsFrom(rows) {
  const controls = { ...defaultEgressControls() };
  for (const row2 of rows) {
    const path = egressPath(row2.channel);
    if (!path || path.enforcement === "uncontrollable") continue;
    controls[path.channel] = row2.allowed;
  }
  return controls;
}
function egressAllowed(controls, channel) {
...
// server/lib/egress-store.ts
var EGRESS_EVENTS_TABLE = `${APP_SCHEMA}.egress_events`;
var EGRESS_CONTROLS_TABLE = `${APP_SCHEMA}.egress_controls`;
var EGRESS_EVENTS_PAGE_LIMIT = 50;
function egressStorageMetadata() {
  return {
    store: "Lakebase (Postgres)",
    eventsTable: EGRESS_EVENTS_TABLE,
    controlsTable: EGRESS_CONTROLS_TABLE,
    retained: "Events retain time, signed-in email, path, outcome, app surface, optional run/conversation pointers and item count. Controls retain allowed state, changer and change time.",
...
var SURFACE_MAX = 64;
var IDENTIFIER_MAX = 128;
var EGRESS_CONTROLS_TTL_MS = 15e3;
var controlsCache = /* @__PURE__ */ new WeakMap();
function forgetEgressControls() {
  controlsCache = /* @__PURE__ */ new WeakMap();
}
async function readEgressControls(client, options = {}) {
  const maxAge = options.maxAgeMs ?? EGRESS_CONTROLS_TTL_MS;
  const now = options.now ?? Date.now();
  const cached2 = controlsCache.get(client);
  if (cached2 && maxAge > 0 && now - cached2.at < maxAge) {
    return { controls: cached2.controls, stored: cached2.stored };
  }
  ...
  if (!read.available) {
    if (read.code !== UNDEFINED_TABLE) {
      console.warn(
        `[egress] The egress controls could not be read (${read.code || "no code"}): ${read.error}. This build's own defaults are in force, and the panel says they are defaults rather than a stored decision.`
      );
    }
  ...
  const controls = egressControlsFrom(
    read.rows.map((row2) => ({
      channel: typeof row2.channel === "string" ? row2.channel : "",
...
async function writeEgressControl(client, change) {
...
async function workspaceLinksAllowed(client) {
  const { controls } = await readEgressControls(client);
  return egressAllowed(controls, "workspace-link");
}
function clamp(raw2, max) {
  const text10 = typeof raw2 === "string" ? raw2.trim() : "";
  return text10.length > max ? text10.slice(0, max) : text10;
}
function pointer(raw2) {
  const text10 = clamp(raw2, IDENTIFIER_MAX);
  return text10 === "" ? null : text10;
}
...
async function recordEgress(client, input) {
  const path = egressPath(input.report.channel);
  const outcome = path && egressAllowed(input.controls, path.channel) ? "left" : "refused";
  const occurredAt = input.now ?? /* @__PURE__ */ new Date();
  const event = {
  ...
  try {
    ...
    console.warn(
      `[egress] A ${event.channel} export by ${event.actor} was not recorded: ${error.message}. The control decision itself was applied. The record for this one is lost.`
    );
...
function eventFromRow(row2) {
  const path = egressPath(rowText(row2.channel));
  if (!path) return null;
  return {
    id: rowText(row2.id),
    occurredAt: rowInstant(row2.occurred_at),
    actor: rowText(row2.actor),
    channel: path.channel,
    shape: path.shape,
    outcome: rowText(row2.outcome) === "refused" ? "refused" : "left",
    surface: rowText(row2.surface),
    runId: rowText(row2.run_id) || null,
    conversationId: rowText(row2.conversation_id) || null,
    itemCount: rowCount(row2.item_count)
  };
}
function parseEgressEventsCursor(raw2) {
  if (!raw2 || raw2.length > 512) return null;
  try {
    const decoded = JSON.parse(Buffer.from(raw2, "base64url").toString("utf8"));
...
function cursorFromRow(row2) {
  const occurredAt = rowInstant(row2.occurred_at);
  const id = rowText(row2.id);
...
async function readEgressEventsPage(client, options = {}) {
  ...
  const events = pageRows.map(eventFromRow).filter((event) => event !== null);
  return {
    events,
    readState: "read",
    pageSize,
...
var RUNTIME_ENTITY_KINDS = ["catalog", "schema", "table", "column", "quote", "tag"];
var RUNTIME_SETTINGS_KEYS = [
  "loop",
  "answer",
  "behavior",
  "colorScheme",
...
var RUNTIME_LOOP_KEYS = ["maxSteps", "maxToolCalls", "maxRunSeconds"];
var RUNTIME_ANSWER_KEYS = [
  "takeaway",
  "narrative",
  "charts",
  "figures",
...
var RUNTIME_BEHAVIOR_KEYS = ["clarification", "timezone", "injectCurrentDate"];
var RUNTIME_ENTITY_STYLE_KEYS = ["foreground", "background"];
var PAPER_ENTITY_STYLES = {
...
var FONT_FAMILY_IDS = ["dm-sans", "system", "dm-mono"];
var FONT_SIZE_IDS = ["s", "m", "l"];
var DENSITY_IDS = ["comfortable", "compact"];
var THEME_FONT_COLORS = {
...
var RuntimeEntityKindSchema = external_exports.enum(RUNTIME_ENTITY_KINDS);
...
var HexColorSchema = external_exports.string().regex(/^#[0-9a-f]{6}$/i, "Use a six-digit hex color.");
var RuntimeEntityStylesObjectSchema = external_exports.strictObject(
  Object.fromEntries(RUNTIME_ENTITY_KINDS.map((kind) => [kind, EntityStyleSchema]))
);
var RuntimeSettingsObjectSchema = external_exports.strictObject({
  ...
  answer: external_exports.strictObject({
    ...
    narrative: external_exports.boolean(),
    charts: external_exports.boolean(),
    figures: external_exports.boolean(),
    caveats: external_exports.boolean(),
    maxCharts: external_exports.number().int().min(0).max(6),
    maxFigures: external_exports.number().int().min(0).max(12),
    maxCaveats: external_exports.number().int().min(0).max(20),
    narrativeMaxCharacters: external_exports.number().int().min(0).max(12e3),
    sources: external_exports.enum(["compact", "standard", "detailed"]),
    // Defaulted so rows stored before these fields existed stay parseable.
    takeawayGuidance: external_exports.string().trim().max(2e3).default(""),
    narrativeGuidance: external_exports.string().trim().max(2e3).default(""),
    figuresOrder: external_exports.enum(["as-ranked", "totals-first", "averages-first"]).default("as-ranked"),
    chartsTypes: external_exports.enum(["auto", "bar", "bar-line"]).default("auto")
  }),
  ...
  backgroundGraphics: external_exports.boolean().default(true),
  animations: external_exports.boolean().default(true),
  density: external_exports.enum(DENSITY_IDS).default("comfortable")
});
var RuntimeSettingsSchema = RuntimeSettingsObjectSchema.transform((settings) => ({
  ...settings,
...
var RuntimeSettingsPatchSchema = external_exports.strictObject({
  loop: external_exports.strictObject({
...
var KEY4 = "effective";
var RUNTIME_SETTINGS_TABLE = appTable("runtime_settings");
var RUNTIME_SETTINGS_DDL = `CREATE TABLE IF NOT EXISTS ${RUNTIME_SETTINGS_TABLE} (
  id TEXT PRIMARY KEY,
  settings JSONB NOT NULL,
  revision BIGINT NOT NULL DEFAULT 1,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
var RUNTIME_SETTINGS_TTL_MS = 15e3;
...
async function readRuntimeSettings(client, options = {}) {
  try {
    ...
    console.warn("[runtime-settings] Falling back to defaults:", error.message);
    return DEFAULT_RUNTIME_SETTINGS;
  }
}
async function readRuntimeSettingsDocument(client, options = {}) {
  const now = options.now ?? Date.now();
  const cached2 = cache4.get(client);
  if (cached2 && now - cached2.at < (options.maxAgeMs ?? RUNTIME_SETTINGS_TTL_MS)) return cached2.document;
  const document = await readVersionedSettings(client, STORE2);
  cache4.set(client, { document, at: now });
  return document;
}
async function writeRuntimeSettingsPatch(client, patch, revision, updatedBy) {
  const document = await writeVersionedSettingsPatch(client, STORE2, patch, revision, updatedBy);
  forgetRuntimeSettings();
  return document;
}

// shared/eval-live-scoring.ts
var DEFAULT_LIVE_SAMPLE_RATE = 0.2;
var LIVE_SCORE_KEEP = 50;
var LiveJudgeVerdictSchema = external_exports.strictObject({
  name: external_exports.string().trim().min(1).max(80),
  value: external_exports.enum(["yes", "no"]).nullable(),
  state: external_exports.enum(["scored", "errored", "not-applicable", "skipped"]),
  note: external_exports.string().trim().max(400).default("")
});
var LiveTraceScoreSchema = external_exports.strictObject({
  id: external_exports.string().trim().min(1).max(80),
  at: external_exports.string().trim().min(1).max(40),
...
var WorkspaceMonitorSchema = external_exports.strictObject({
...
var EVAL_LIVE_SCORES_TABLE = appTable("eval_live_scores");
var EVAL_LIVE_SCORES_DDL = `CREATE TABLE IF NOT EXISTS ${EVAL_LIVE_SCORES_TABLE} (
  id TEXT PRIMARY KEY,
  scored_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  conversation_id TEXT NOT NULL,
  message_id TEXT NOT NULL,
...
async function listLiveScores(client, limit = LIVE_SCORE_KEEP) {
  try {
    ...
    console.warn("[eval-live-scores] Live scores could not be read:", error.message);
    return [];
  }
}
...
async function scoreLiveJudges(turn, settings, invoke) {
  const verdicts = [];
  ...
  if (enabled.has("groundedness")) {
    verdicts.push(
      context ? await run(GROUNDEDNESS_FEEDBACK_NAME, groundednessPrompt(question, response, context)) : {
...
async function scoreSampledAskTurn(input) {
  try {
        ...
        scored2.judges = [
          {
            ...
            value: null,
            state: "errored",
            note: `LLM judges could not run: ${error.message}`
          }
        ];
      }
    } else {
    ...
    console.warn("[eval-live-scores] Sampled Ask turn was not scored:", error.message);
    return null;
  }
}
...
// server/lib/run-request-hash.ts
...
function idempotencyKeyHash(userEmail2, key) {
  return sha256(field("v", "1") + field("user", userEmail2.trim().toLowerCase()) + field("key", key));
}
function isUsableIdempotencyKey(key) {
  return /^[A-Za-z0-9._:-]{8,200}$/.test(key);
}

// server/lib/run-ledger.ts
var EXECUTING_SQL_LIST = EXECUTING_STATES.map((state) => `'${state}'`).join(",");
var LEASE_MS = 3e4;
var RACE_ATTEMPTS = 4;
var RETRYABLE_LEDGER_CODES = /* @__PURE__ */ new Set(["08000", "08001", "08003", "08004", "08006", "57P01", "57P03", "28P01"]);
async function ledgerQuery(store, label2, sql, params) {
  let last;
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      ...
      last = error;
      const raw3 = error.code;
      const code2 = typeof raw3 === "string" || typeof raw3 === "number" ? String(raw3) : "";
      if (attempt === 1 && RETRYABLE_LEDGER_CODES.has(code2)) continue;
      break;
    }
  }
  const raw2 = last?.code;
...
var RUN_COLUMNS = `run_id, user_email, conversation_id, turn_id, request_hash, idempotency_key_hash,
...
function toRun(record) {
  return {
    ...
    idempotencyKeyHash: text4(record.idempotency_key_hash),
    planFingerprint: text4(record.plan_fingerprint),
    state: String(record.state),
    deadlineAt: String(record.deadline_at),
    identityModeRequested: text4(record.identity_mode_requested),
    identityModeEffective: text4(record.identity_mode_effective),
    identityVerified: record.identity_verified === null || record.identity_verified === void 0 ? null : Boolean(record.identity_verified),
    personaId: text4(record.persona_id),
    personaName: text4(record.persona_name),
    terminalCode: text4(record.terminal_code),
    terminalMessageId: text4(record.terminal_message_id),
    traceId: text4(record.trace_id),
    correlationId: text4(record.correlation_id),
    fencingToken: Number(record.fencing_token ?? 0),
    leaseOwner: text4(record.lease_owner),
    leaseExpiresAt: text4(record.lease_expires_at),
...
async function acquireLease(store, runId, executor, from, leaseMs = LEASE_MS) {
  ...
  if (!claimed) {
    return {
      ok: false,
      reason: "lost",
      detail: `Run ${runId} was not available to take over: either another executor holds a live lease on it or it has moved out of ${from.join(", ")}. Attach to it rather than running it again.`
    };
  }
  return { ok: true, value: toRun(claimed) };
}
async function transition(store, input) {
  ...
  if (!moved) {
    return {
      ok: false,
      reason: "lost",
      detail: `Run ${input.runId} did not move ${input.from} to ${input.to} under fencing token ${input.fencingToken}. Another executor owns it, or it has already left ${input.from}. This executor must stop rather than continue: anything it produces from here is a second answer to a question that already has one.`
    };
  }
  return { ok: true, value: toRun(moved) };
}
async function recordAttempt(store, input) {
...
async function completeAttempt(store, input) {
  const read = await ledgerQuery(
    store,
...
// server/lib/run-admission.ts
var RUN_LEDGER_MODE_ENV = "PIA_RUN_LEDGER";
function resolveRunLedgerMode(value) {
  ...
  if (normalised !== "" && normalised !== "shadow") {
    console.warn(
      `[run-ledger] ${RUN_LEDGER_MODE_ENV}="${value}" is not one of off, shadow, enforce. Running in shadow, which records runs and changes no response.`
    );
  }
  return "shadow";
}
async function admitRun(store, input) {
  ...
  if (!created.ok) {
    if (input.mode === "shadow") {
      console.warn(
        `[run-ledger] shadow: the run for this request could not be recorded (${created.detail}). The question is being answered anyway. In enforce mode it would have been refused.`
      );
...
async function settleRun(store, admission, outcome) {
  ...
  if (path === null) {
    console.error(
      `[run-ledger] Run ${admission.run.runId} is in ${admission.run.state} and cannot reach ${outcome.to}. Nothing was written, because a run that has already answered must not be re-marked by a late write.`
    );
    return;
  }
  ...
  for (const next of path) {
    ...
    const moved = await transition(store, {
      ...
      traceId: last ? outcome.traceId : void 0,
      terminalMessageId: last ? outcome.messageId : void 0
    });
    if (!moved.ok) {
      console.error(
        `[run-ledger] Run ${admission.run.runId} could not be moved from ${from} to ${next}: ${moved.detail}. It now has no durable terminal state, which is the condition worth alerting on: the answer was returned to the reader and the ledger cannot say so.`
      );
      return;
    }
    from = next;
...
async function parkRun(store, admission, planFingerprint) {
  if (admission.kind !== "proceed" || !admission.run || admission.fencingToken === null) return;
  ...
  for (const next of path) {
    const moved = await transition(store, {
      ...
      to: next,
      fencingToken: admission.fencingToken,
      planFingerprint: next === "AWAITING_APPROVAL" ? planFingerprint : void 0
    });
    if (!moved.ok) {
      console.warn(
        `[run-ledger] Run ${admission.run.runId} could not be parked at ${next}: ${moved.detail}. The plan was still returned; the approval that follows will not find a run to resume.`
      );
      return;
    }
    from = next;
...
function executorName(env = process.env, pid = process.pid) {
...
// server/lib/run-stage-events.ts
var STAGE_EVENT_TYPE = "stage";
var STAGE_INPUT_LIMIT = 180;
var STAGE_REPLAY_LIMIT = 200;
function clamp2(value, limit = STAGE_INPUT_LIMIT) {
...
function stageOutcomeCode(stage) {
  const status = typeof stage.status === "string" ? stage.status.toLowerCase() : "";
  if (!["failed", "refused"].includes(status)) return void 0;
...
function stageEventPayload(stage) {
  ...
  put("id", typeof stage.id === "string" ? stage.id : void 0);
  put("name", typeof stage.name === "string" ? clamp2(stage.name) : void 0);
  put("kind", typeof stage.kind === "string" ? stage.kind : void 0);
  put("status", typeof stage.status === "string" ? stage.status : void 0);
...
async function stageQuery(store, reporter, label2, sql, params) {
  try {
    const result = await store.lakebase.query(sql, params);
    return result.rows;
  } catch (error) {
    if (!reporter.reported) {
      ...
      const code = typeof raw2 === "string" || typeof raw2 === "number" ? String(raw2) : "none";
      console.warn(
        `[run-stage-events] ${label2} failed (code ${code}): ${error instanceof Error ? error.message : String(error)}. The run is unaffected and its answer is still stored; a browser that reopens this conversation mid-run will see that it is working without the steps it has taken.`
      );
    }
    return null;
  }
...
async function readStageEvents(store, runId) {
  const rows = await stageQuery(
    store,
    { reported: false },
    "stage replay read",
...
var WARMUP_COOLDOWN_MS = 6e4;
var WARMUP_TIMEOUT_MS = 1e4;
var ALREADY_WARM = /* @__PURE__ */ new Set(["RUNNING", "STARTING"]);
var NOTHING_TO_WARM = /* @__PURE__ */ new Set(["DELETING", "DELETED"]);
function warehouseStatePath(warehouseId) {
  return `/api/2.0/sql/warehouses/${encodeURIComponent(warehouseId)}`;
}
function warehouseStartPath(warehouseId) {
  return `${warehouseStatePath(warehouseId)}/start`;
}
function messageOf2(error) {
  return error?.message ?? String(error);
}
function createWarehouseWarmup(options) {
  const now = options.now ?? Date.now;
  const cooldownMs = options.cooldownMs ?? WARMUP_COOLDOWN_MS;
  const timeoutMs = options.timeoutMs ?? WARMUP_TIMEOUT_MS;
  let lastAttemptAt = Number.NEGATIVE_INFINITY;
  let inFlight = null;
  async function attempt(warehouseId) {
    let state;
    try {
      ...
      return { kind: "failed", at: "state", message: messageOf2(error) };
    }
    if (!state) {
    ...
    try {
      ...
      return { kind: "failed", at: "start", message: messageOf2(error) };
    }
  }
  return {
    warm() {
      const warehouseId = options.warehouseId().trim();
      if (!warehouseId) return Promise.resolve({ kind: "not-configured" });
      if (inFlight) return inFlight;
      const sinceMs = now() - lastAttemptAt;
      if (sinceMs < cooldownMs) return Promise.resolve({ kind: "cooling-down", sinceMs });
      lastAttemptAt = now();
...
function describeWarmup(outcome) {
  switch (outcome.kind) {
    case "started":
    ...
    case "cooling-down":
      return `SQL warehouse pinged ${Math.round(outcome.sinceMs / 1e3)}s ago; not pinging again yet.`;
    case "not-configured":
      return "No SQL warehouse is configured for this app, so there is nothing to warm.";
    case "failed":
      return `SQL warehouse could not be warmed (${outcome.at}): ${outcome.message}. This is harmless: the first question will start it as it always did.`;
  }
}

// server/lib/genie-warehouse-warmup.ts
var GENIE_WARMUP_COOLDOWN_MS = 6e4;
var GENIE_WARMUP_TIMEOUT_MS = 1e4;
var GENIE_WARMUP_CACHE_MAX_ENTRIES = 2048;
var ALREADY_WARM2 = /* @__PURE__ */ new Set(["RUNNING", "STARTING"]);
var NOTHING_TO_WARM2 = /* @__PURE__ */ new Set(["DELETING", "DELETED"]);
function messageOf3(error) {
  return error?.message ?? String(error);
}
async function jsonRequest(call, url, token, method, timeoutMs) {
...
function createGenieWarehouseWarmup(options = {}) {
  ...
  return {
    async warm({ host, token, subject, spaceIds, appWarehouseId: appWarehouseId2 }) {
      ...
      await Promise.all(
        [...new Set(spaceIds.filter(Boolean))].map(async (spaceId) => {
          const key = `${reader}\0${base}\0${spaceId}`;
          const checkedAt = now();
          ...
          try {
            ...
            outcomes.push({ kind: "failed", spaceId, at: "space", message: messageOf3(error) });
          }
        })
      );
      await Promise.all(
        [...warehouses].map(async ([warehouseId, resolvedSpaces]) => {
          if (warehouseId === appWarehouseId2) {
...
function unavailableHttpStatus(code) {
  return failureDefinition(code).httpStatus;
}

// shared/conversation-filters.ts
var CONVERSATION_PERSONA_FILTER_RULE = "Persona is the snapshot recorded on the conversation’s newest active or completed run. Conversations without a recorded persona remain included when All personas is selected.";
var MAX_CONVERSATION_FILTER_VALUES = 25;
var MAX_OWNER_FILTER_LENGTH = 254;
var MAX_PERSONA_FILTER_LENGTH = 80;
...
function parseConversationFilterQuery(query) {
  ...
  if (!retiredMissingPersonaValues || retiredMissingPersonaValues.length > 1 || !retiredMissingPersonaValues.every((value) => value === "true")) {
...
function carriedProviderCode(error) {
  if (error === null || typeof error !== "object") return void 0;
  const record = error;
...
// server/lib/agent-refusal.ts
var UNAVAILABLE_TYPE = "unavailable";
var UNKNOWN_CODE_FALLBACK = "OUTPUT_SCHEMA_VIOLATION";
function readAgentRefusal(payload, context) {
...
// server/lib/app-budget-guard.ts
import crypto4 from "node:crypto";

// shared/app-cost-summary.ts
var DAY_MS3 = 864e5;
function completeDays2(payload) {
  const from = Date.parse(`${payload.range.from}T00:00:00Z`);
  const to = Date.parse(`${payload.range.to}T00:00:00Z`);
  return Number.isFinite(from) && Number.isFinite(to) && to >= from ? Math.round((to - from) / DAY_MS3) + 1 : 0;
}
function deploymentAttribution(tile) {
...
function appCostSummary(payload, unit = "USD") {
  const days = completeDays2(payload);
  const included = payload.tiles.filter(
    (tile) => tile.id !== "genie:unattributed" && deploymentAttribution(tile) && (unit === "DBU" ? typeof tile.dbus === "number" && Number.isFinite(tile.dbus) : tile.quality !== "unknown" && typeof tile.amount === "number" && Number.isFinite(tile.amount) && (tile.pricing?.match === void 0 || tile.pricing.match === "priced" || tile.pricing.match === "none"))
  );
  const total = included.reduce(
    (sum, tile) => sum + (unit === "DBU" ? tile.dbus ?? 0 : tile.amount ?? 0) * (tile.basis === "per-day" ? days : 1),
    0
  );
  const activeMissing = payload.tiles.some(
    (tile) => tile.id !== "genie:unattributed" && Boolean(tile.resourceId.trim()) && (tile.attribution !== "deployment" || (unit === "DBU" ? typeof tile.dbus !== "number" || !Number.isFinite(tile.dbus) : typeof tile.amount !== "number" || !Number.isFinite(tile.amount)))
  );
  ...
  return {
    ...
    label: included.length > 0 ? unit === "DBU" ? `${total.toLocaleString(void 0, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} DBU` : `${total.toLocaleString(void 0, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}${currency ? ` ${currency}` : ""}` : "Unavailable",
    days,
...
var COST_BUDGET_MAX = 1e12;
var AmountSchema = external_exports.number().finite().nonnegative().max(COST_BUDGET_MAX).nullable();
var CostBudgetUnitSchema = external_exports.enum(["USD", "DBU"]);
var CostBudgetValuesSchema = external_exports.strictObject({
  USD: AmountSchema,
  DBU: AmountSchema
});
var CostBudgetSchema = external_exports.preprocess((raw2) => {
  if (typeof raw2 === "number" || raw2 === null) return { USD: raw2, DBU: null };
  if (!raw2 || typeof raw2 !== "object" || Array.isArray(raw2)) return raw2;
  const record = raw2;
  if ("USD" in record || "DBU" in record) return raw2;
...
function costBudgetValue(budget, unit) {
  return normalizeCostBudget(budget)[unit];
}
var CostBudgetsSchema = external_exports.strictObject({
...
var EMPTY_COST_BUDGET = { USD: null, DBU: null };
var EMPTY_COST_BUDGETS = { total: EMPTY_COST_BUDGET, resources: {} };
var COST_BUDGET_WITHHELD_TILE_IDS = /* @__PURE__ */ new Set(["index-rebuild-job"]);
...
// shared/app-budget-contract.ts
var APP_BUDGET_WARNING_PERCENT = 80;
var APP_BUDGET_APPROVAL_PERCENT = 100;
var BUDGET_APPROVAL_REQUIRED = "BUDGET_APPROVAL_REQUIRED";
var APP_BUDGET_GUARDRAILS = [
  ...
  { label: "Resource budgets", value: "Advisory only" },
  {
    ...
    value: "Authoritative system billing can lag by hours; concurrent requests may pass before the threshold is observed"
  }
];
var APP_BUDGET_LEVELS = [
  "unset",
  "below",
  "warning",
  "approval-required",
...
var APP_BUDGET_COVERAGES = ["complete", "partial", "unavailable"];
function appBudgetPeriod(now) {
...
function emptyAppBudgetStatus(period, readAt2, overrides = {}) {
  return {
    level: "unset",
    measured: null,
    budget: null,
...
// shared/app-budget-guard.ts
var AppBudgetLevelSchema = external_exports.enum(APP_BUDGET_LEVELS);
var AppBudgetCoverageSchema = external_exports.enum(APP_BUDGET_COVERAGES);
var AppBudgetApprovalSchema = external_exports.strictObject({
  approved: external_exports.boolean(),
  approvedAt: external_exports.string(),
  approvedBy: external_exports.string(),
  through: external_exports.string(),
  revokedAt: external_exports.string()
});
var AppBudgetStatusSchema = external_exports.strictObject({
  level: AppBudgetLevelSchema,
  measured: external_exports.number().finite().nonnegative().nullable(),
  budget: external_exports.number().finite().nonnegative().nullable(),
  unit: CostBudgetUnitSchema.nullable(),
  ratio: external_exports.number().finite().nonnegative().nullable(),
  percent: external_exports.number().finite().nonnegative().nullable(),
  monthStart: external_exports.string(),
  monthEnd: external_exports.string(),
  measuredThrough: external_exports.string(),
  readAt: external_exports.string(),
  coverage: AppBudgetCoverageSchema,
  approval: AppBudgetApprovalSchema.nullable(),
  budgetFingerprint: external_exports.string(),
  code: external_exports.string(),
  detail: external_exports.string()
});
var DECIMAL_SCALE = 1000000n;
function budgetLevelFor(measured, budget) {
  const fixed = (value) => {
  ...
  return {
    ...
    ratio: finiteRatio,
    percent: Math.round(finiteRatio * 1e4) / 100
  };
}

// server/lib/cost-budgets-store.ts
var KEY5 = "effective";
var COST_BUDGETS_TABLE = appTable("cost_budgets");
var COST_BUDGETS_DDL = `CREATE TABLE IF NOT EXISTS ${COST_BUDGETS_TABLE} (
  id TEXT PRIMARY KEY,
  settings JSONB NOT NULL,
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_by TEXT NOT NULL
...
var COST_BUDGETS_TTL_MS = 15e3;
function forgetCostBudgets() {
  cache5 = /* @__PURE__ */ new WeakMap();
}
async function readCostBudgets(client, options = {}) {
  const now = options.now ?? Date.now();
  const cached2 = cache5.get(client);
  if (cached2 && now - cached2.at < (options.maxAgeMs ?? COST_BUDGETS_TTL_MS)) {
    return { budgets: cached2.value, readable: true };
  }
  try {
    const result = await client.lakebase.query(`SELECT settings FROM ${COST_BUDGETS_TABLE} WHERE id = $1`, [KEY5]);
    const raw2 = result?.rows?.[0]?.settings;
    if (raw2 === void 0) {
      cache5.set(client, { value: EMPTY_COST_BUDGETS, at: now, readable: true });
      return { budgets: EMPTY_COST_BUDGETS, readable: true };
    }
    const parsed = parseCostBudgets(raw2);
    if (!parsed) {
    ...
    cache5.set(client, { value: parsed, at: now, readable: true });
    return { budgets: parsed, readable: true };
  } catch (error) {
    console.warn("[cost-budgets] Stored budgets could not be read:", error.message);
    return { budgets: EMPTY_COST_BUDGETS, readable: false };
  }
}
async function writeCostBudgets(client, budgets, updatedBy) {
  await client.lakebase.query(
    ...
    [KEY5, JSON.stringify(budgets), updatedBy]
  );
  forgetCostBudgets();
  return budgets;
}

// server/lib/app-budget-approval-store.ts
import crypto3 from "node:crypto";
var APP_BUDGET_APPROVALS_TABLE = appTable("app_budget_approvals");
function text6(value) {
  if (typeof value === "string") return value;
  if (value instanceof Date) return value.toISOString();
  return "";
}
function approvalFrom(row2) {
  if (!row2) return null;
  return {
    id: text6(row2.id),
    approvedBy: text6(row2.approved_by),
    approvedAt: text6(row2.approved_at),
    revokedBy: text6(row2.revoked_by),
    revokedAt: text6(row2.revoked_at)
  };
}
async function readAppBudgetApproval(client, key) {
  const result = await client.lakebase.query(
...
async function approveAppBudget(client, input) {
  const id = `budget-approval-${crypto3.createHash("sha256").update(`${input.period.monthStart}|${input.budgetFingerprint}|${input.unit}|${input.budget}`).digest("hex").slice(0, 32)}`;
  const coverage2 = JSON.stringify({
...
async function revokeAppBudgetApproval(client, input) {
  const result = await client.lakebase.query(
...
// server/lib/ops-foundation-billing.ts
...
function buildFoundationCostStatement(ids, range, runs) {
...
function readFoundationBillingRows(data) {
  ...
  const status = text7(match);
  const pricing = {
    ...EMPTY_PRICING,
    match: status === "priced" || status === "partial" || status === "unpriced" || status === "duplicate" || status === "mixed-currency" || status === "none" ? status : "none",
    currency: text7(currency),
    ...
    unpricedSkus: text7(unpricedSkus).split(",").map((sku) => sku.trim()).filter(Boolean),
    duplicateMatches: finite(duplicateMatches),
    correctionRows: finite(correctionRows),
    priceEffectiveAt: text7(priceEffectiveAt)
  };
  ...
  return {
    ...
    totalTokens: finite(totalTokens2),
    complete: priceComplete && runsWithEvidence === eligibleRuns && finite(missingEvidenceRequests) === 0
  };
}
function foundationCostTile(ids, result, reason = "") {
  const amountAvailable = result?.amount !== null && result?.amount !== void 0 && (result.pricing.match === "priced" || result.pricing.match === "none" && result.billingRows === 0);
  ...
  return {
    ...
    unavailable: amountAvailable ? "" : reason || (!ids.foundationModel ? "Configured foundation model unavailable." : result?.pricing.match === "partial" || result?.pricing.match === "unpriced" ? "Matched Ask model usage is missing a list price." : "Foundation-model billing could not be attributed."),
    remedy: "",
    note: amountAvailable && !coverageComplete ? `Measured lower bound; ${missingEligibleRequests} eligible Ask${missingEligibleRequests === 1 ? "" : "s"} missing model evidence` : "",
    evidence: {
      billingRows: null,
...
// server/lib/app-budget-guard.ts
var APP_BUDGET_MEASUREMENT_TTL_MS = 6e4;
var measurementCache = null;
function forgetAppBudgetStatus() {
  measurementCache = null;
}
function appBudgetFingerprint(total) {
  const normalized = normalizeCostBudget(total);
  return crypto4.createHash("sha256").update(JSON.stringify(normalized)).digest("hex");
}
function unavailable2(period, readAt2, input) {
  return emptyAppBudgetStatus(period, readAt2, {
...
async function queryMeasurement(appkit, req, period, now) {
  ...
  const {
    ...
    resolveWorkspaceId,
    runStatement: runStatement2,
    QUESTION_COST_RUNS_QUERY,
    questionRun,
    warehouseId,
    warehouseQueryAttribution
  } = await import("./ops-routes-4GQM7MMU.mjs");
  ...
  const interactiveRuns = runRows.rows.map((row2) => questionRun(row2));
  const interactiveComplete = interactiveRuns[0]?.evidenceComplete ?? interactiveRuns.length === 0;
  const built = (await import("./ops-billing-BDU2XMYM.mjs")).buildCostStatement(resolved.ids, range);
  if (!built) return null;
  const foundationBuilt = interactiveComplete ? buildFoundationCostStatement(resolved.ids, range, interactiveRuns) : null;
  ...
  const [outcome, queryAttribution, activity, foundationOutcome] = await Promise.all([
    ...
    foundationBuilt ? runStatement2({
      host: workspace,
      token,
      warehouseId: warehouse,
      statement: foundationBuilt.statement,
      parameters: foundationBuilt.parameters
    }) : Promise.resolve({ ok: false, message: "Foundation-model billing is unavailable." })
  ]);
  if (!outcome.ok) return null;
  const split = splitBillingRows(readComponentRows(outcome.rows));
  const foundation = foundationOutcome.ok ? foundationCostTile(resolved.ids, readFoundationBillingRows(foundationOutcome.rows)) : foundationCostTile(resolved.ids, null, foundationOutcome.message);
...
async function currentMeasurement(appkit, req, period, now, reader) {
...
async function readAppBudgetStatus(appkit, req, options = {}) {
  const now = options.now ?? Date.now();
  const period = appBudgetPeriod(now);
  const readAt2 = new Date(now).toISOString();
  const stored = await readCostBudgets(appkit, { maxAgeMs: 0, now });
  if (!stored.readable) {
    return unavailable2(period, readAt2, {
  ...
  const configured = ["USD", "DBU"].map((unit) => ({ unit, budget: costBudgetValue(stored.budgets.total, unit) })).filter((entry) => entry.budget !== null);
  ...
  try {
    ...
    return unavailable2(period, readAt2, {
      ...
      detail: `Attributable month-to-date billing failed to read (${error.message}).`,
      fingerprint,
      unit: configured[0].unit,
      budget: configured[0].budget
    });
  ...
  const candidates = configured.map(({ unit, budget }) => {
    ...
    const currencyComplete = unit === "DBU" || measurement.payload.currency.trim().toUpperCase() === "USD" && measurement.payload.honesty?.currencyConsistent !== false;
    const complete2 = measured !== null && !summary.partial && measurement.payload.honesty?.rangeMayStillFill !== true && currencyComplete;
    return { unit, budget, measured, complete: complete2, summary };
  });
  const complete = candidates.filter(
    (entry) => entry.complete && entry.measured !== null
  ).map((entry) => ({ ...entry, threshold: budgetLevelFor(entry.measured, entry.budget) })).sort((left, right) => right.threshold.ratio - left.threshold.ratio || left.unit.localeCompare(right.unit));
  if (complete.length === 0) {
    const first = candidates[0];
    return unavailable2(period, measurement.readAt, {
  ...
  return {
    ...
    approval,
    budgetFingerprint: fingerprint,
    code: level === "approval-required" ? BUDGET_APPROVAL_REQUIRED : `APP_BUDGET_${level.toUpperCase().replace("-", "_")}`,
    detail: level === "approval-required" ? "Measured month-to-date app-attributable spend reached the monthly app budget. An administrator must approve continued usage." : ""
  };
}
function budgetGuardBlocks(status) {
  return status.level === "approval-required";
}

// server/lib/session-freshness.ts
function undetermined(explanation, signedIn, evidence = "") {
  return {
    state: "undetermined",
    cause: UNDETERMINED,
    evidence,
...
function sessionFreshness(input) {
  if (!input.token) {
    return undetermined(
      "This request carried no forwarded sign-in, so there is nothing to compare against the permissions this app asks for. Nothing about your sign-in was established either way.",
      false
    );
  }
  ...
  if (!tokenScopes) {
    return undetermined(
      "The sign-in this request carried does not list its own permissions, so it cannot be compared against the ones this app asks for. Nothing about your sign-in was established.",
  ...
  if (!declared || declared.length === 0) {
    return {
      ...undetermined(
        "This deployment was not told which permissions it asks for, so no sign-in can be compared against them. Nothing about your sign-in was established.",
        true,
        `The token lists ${quotedScopes(tokenScopes)}. ${DECLARED_SCOPES_VAR} is unset in this container.`
      ),
  ...
  if (missing.length === 0) {
    return {
      ...
      evidence: `The app declares ${declared.length} permissions and the sign-in this request carried carries all of them. It lists ${quotedScopes(tokenScopes)}.`,
  ...
  return {
    ...
    evidence: `The app declares ${declared.length} permissions including ${quotedScopes(missing)}. The sign-in this request carried lists ${quotedScopes(tokenScopes)}, which does not include ${quotedScopes(missing)}.`,
    explanation: `Your sign-in to this app does not carry ${quotedScopes(missing)}, which the app asks for. Parts of the app will report errors that look like permissions you are missing, and are not.`,
    remedy: freshSignIn(),
    signedIn: true,
    tokenScopes,
    declaredScopes: [...declared],
...
var RunDeadlineExceededError = class extends Error {
  timeoutMs;
...
function isRunDeadlineExceededError(error) {
...
var activeRunControllers = /* @__PURE__ */ new Map();
...
var DURABLE_CANCELLATION_POLL_MS = 1500;
...
var MAX_SERVING_STREAM_BYTES = 8 * 1024 * 1024;
var MAX_SERVING_STREAM_EVENTS = STAGE_REPLAY_LIMIT * 3 + 8;
var MAX_SERVING_OUTPUT_ITEMS = STAGE_REPLAY_LIMIT;
...
var TruncatedStreamError = class extends Error {
  /**
...
async function consumeServingStream(body, onStage, signal) {
  ...
  try {
    for await (const event of sseEvents(body, signal)) {
      ...
      if (stage) {
        ...
        try {
          ...
          console.warn("[serving] Stage could not be forwarded:", error.message);
        }
        continue;
      }
    ...
    if (customOutputs2 === null && output.length === 0) {
      console.warn(
        `[serving] Stream died after ${stages} stage(s) and ${announced} announcement(s): ${error.message}`
      );
      throw new TruncatedStreamError(stages, announced);
    }
    console.warn(`[serving] Stream died after the answer arrived (${error.message}); keeping it.`);
  }
  throwIfRunCancelled(signal);
  if (customOutputs2 === null && output.length === 0) {
    throw new TruncatedStreamError(stages, announced);
...
function createAskResponder(req, res) {
...
function messageFrom(body, fallback) {
  ...
  const record = body;
  return text8(record.message) || text8(record.error) || text8(record.error_code) || fallback;
}
function scopesFrom(body) {
  const raw2 = body && typeof body === "object" ? body.user_api_scopes : void 0;
  if (!Array.isArray(raw2)) return [];
  return [...new Set(
    raw2.filter((scope) => typeof scope === "string").map((scope) => scope.trim()).filter(Boolean)
  )];
}
function appUrl(host, appName) {
...
var MANAGER_GRANT_USER_API_SCOPES = [
...
async function allowAstrolabeUserApiScopes(options) {
  ...
  try {
    ...
    return { kind: "failed", status: 502, message: `Databricks could not be reached: ${error.message}` };
  }
  const currentBody = await bodyOf(read);
  if (!read.ok) return failed(read, currentBody);
  const current = scopesFrom(currentBody);
  const managed = options.additionalScopes ? [...REQUIRED_USER_API_SCOPES, ...options.additionalScopes] : MANAGER_GRANT_USER_API_SCOPES;
  const merged = [.../* @__PURE__ */ new Set([...current, ...managed])];
...
// shared/app-facts.ts
var NO_EXPORTER_READING = {
  state: "unmeasured",
  tables: [],
  error: "",
  schema: ""
};
var NO_APP_SERVING = { app: "", compute: "", message: "" };
var NO_APP_FACTS = {
  url: "",
  answered: false,
  description: "",
...
var TELEMETRY_SCHEMA_ENV = "PLAYER_INSIGHTS_TELEMETRY_SCHEMA";
var LOGS_TABLE = "otel_logs";
var EXPORTER_TABLES = ["otel_spans", "otel_metrics"];
...
var AUTH_EVENT = "app.auth";
var SIGN_IN_REASON = "user_login";
var APP_LOG_SOURCE = "APP";
...
function readExporterRows(dataArray, schema) {
  ...
  if (Array.isArray(dataArray)) {
    for (const raw2 of dataArray) {
      if (!Array.isArray(raw2) || raw2.length < 4) continue;
      const [name, rows, firstAt, lastAt] = raw2;
      const table = rowText2(name);
      if (!table) continue;
      tables.push({
        table,
...
var WAREHOUSE_ENV = "DATABRICKS_SQL_WAREHOUSE_ID";
var EXPORTER_CACHE_MS = 5 * 60 * 1e3;
var workspaceExporterReader = async () => {
  const schema = telemetrySchema();
  ...
  try {
    ...
    return exporterFailure(error?.message ?? "the count did not complete", schema);
  }
};
var cached = null;
...
var APPS_PATH2 = "/api/2.0/apps";
var APP_NAME_ENV = "DATABRICKS_APP_NAME";
var OTEL_ENDPOINT_ENV = "OTEL_EXPORTER_OTLP_ENDPOINT";
var COMPUTE_ENVELOPES = {
...
function appFacts(input) {
  ...
  const appUrl2 = textOf2(body.url);
  const deployment = objectOf(body.active_deployment);
  const gitSource = objectOf(deployment.git_source);
  const gitBacked = Object.keys(gitSource).length > 0;
  const sourcePath = gitBacked ? textOf2(gitSource.source_code_path) : textOf2(deployment.source_code_path);
  const appName = textOf2(body.name);
  const workspaceHost2 = normalizeWorkspaceHost(input.workspaceHost);
  const gitRef = textOf2(gitSource.branch) || textOf2(gitSource.tag) || textOf2(gitSource.commit);
  const workspaceId = textOf2(input.workspaceId) || workspaceIdFromAppUrl(appUrl2);
  const folderUrl = gitBacked ? "" : browseFolderUrl({ host: workspaceHost2, folderId: input.sourceFolderId ?? "", workspaceId });
  return {
    url: appUrl2,
    answered: true,
    description: textOf2(body.description),
    compute: appCompute(body.compute_size),
    tags: appTags(body.tags),
    deployedAt: textOf2(deployment.create_time) || textOf2(body.update_time),
    deployedBy: textOf2(deployment.creator) || textOf2(body.updater),
    source: {
      path: sourcePath,
...
var workspaceAppReader = async (name) => {
  try {
    const { WorkspaceClient } = await import("./vendor-databricks-sdk-experimental.mjs");
    const client = new WorkspaceClient({});
    const body = await client.apiClient.request({
      path: `${APPS_PATH2}/${encodeURIComponent(name)}`,
      method: "GET",
    ...
    const message = textOf2(shape.message) || "the call did not complete";
    if (Number.isFinite(status) && status >= 400) return { kind: "refused", status, message };
    return { kind: "no-response", message };
  }
};
var FOLDER_ID_CACHE_MAX_ENTRIES = 256;
var FOLDER_ID_CACHE_TTL_MS = 60 * 6e4;
var knownFolderIds = new ExpiringLruCache(FOLDER_ID_CACHE_MAX_ENTRIES, FOLDER_ID_CACHE_TTL_MS);
var workspaceFolderIdResolver = async (path) => {
  const wanted = path.trim();
  if (!wanted) return "";
  const cached2 = knownFolderIds.get(wanted);
  ...
  try {
    ...
    id = typeof found === "number" ? String(found) : textOf2(found);
  } catch (error) {
    console.warn(
      `[settings] The workspace could not resolve the folder id for ${wanted} (${error.message}), so the App source row points at the app's own page.`
    );
  }
...
async function readAppFacts(input = {}) {
  ...
  try {
    ...
    console.warn("[settings] The exporter tables could not be counted:", error.message);
  }
  ...
  try {
    ...
    read = { kind: "no-response", message: error.message };
  }
  if (read.kind !== "ok") {
  ...
  if (folderPath) {
    try {
      ...
      console.warn(`[settings] The folder id for ${folderPath} could not be read:`, error.message);
    }
  }
  return appFacts({ read, workspaceHost: workspaceHost2, otelExporter, otelExport, sourceFolderId });
}

// server/lib/control-plane-identity.ts
var USER_METADATA_TTL_MS = 6e4;
var USER_METADATA_CACHE_MAX_ENTRIES = 512;
var APP_CONTEXT_TTL_MS = 5 * 6e4;
var APP_CONTEXT_CACHE_MAX_ENTRIES = 64;
var SCIM_USERS_PATH = "/api/2.0/preview/scim/v2/Users";
var userCache = new ExpiringLruCache(USER_METADATA_CACHE_MAX_ENTRIES, USER_METADATA_TTL_MS);
var appCache = new ExpiringLruCache(APP_CONTEXT_CACHE_MAX_ENTRIES, APP_CONTEXT_TTL_MS);
...
async function readUser(email, host, reader, now) {
  const key = `${keyPart(host)}\0${keyPart(email)}`;
  const cached2 = userCache.get(key, now);
  ...
  try {
    ...
    const matching = scimResources(body).find((candidate) => keyPart(textOf3(candidate.userName)) === keyPart(email));
    if (matching) {
      result = {
        displayName: textOf3(matching.displayName),
        objectId: textOf3(matching.id),
        state: "verified",
        readAt: readAt(now)
      };
    }
...
async function readAppContext(appName, host, reader, now) {
  const key = `${keyPart(host)}\0${keyPart(appName)}`;
  const cached2 = appCache.get(key, now);
  if (cached2) return cached2;
  let result = {
    workspaceId: "",
    servicePrincipal: {
  ...
  if (appName && host) {
    try {
      ...
      const applicationId = textOf3(app.service_principal_client_id);
      const displayName = textOf3(app.service_principal_name);
      ...
      result = {
        workspaceId: workspaceIdFromAppUrl(textOf3(app.url)),
        servicePrincipal: {
          displayName,
          applicationId,
          objectId,
...
var workspaceClient;
var workspaceControlPlaneReader = async (path, query) => {
  const { WorkspaceClient } = await import("./vendor-databricks-sdk-experimental.mjs");
  if (!workspaceClient) {
...
async function readControlPlaneIdentityMetadata(input, deps = {}) {
  ...
  const workspaceHost2 = normalizeWorkspaceHost(input.workspaceHost ?? process.env.DATABRICKS_HOST);
  const now = deps.now ?? Date.now();
  const reader = deps.read ?? workspaceControlPlaneReader;
  const [user, app] = await Promise.all([
    readUser(input.email.trim(), workspaceHost2, reader, now),
    readAppContext(appName, workspaceHost2, reader, now)
  ]);
  return {
    user,
    app: {
...
var REPRESENTATIVE_ANSWER_CAVEAT = "No MLflow trace was recorded for this answer, so it cannot be opened in MLflow.";
...
var AskBody = external_exports.object({
  conversationId: external_exports.string().min(1),
  prompt: external_exports.string().min(2).max(5e3),
  approvedPlanId: external_exports.string().min(1).optional(),
  executePlan: external_exports.boolean().optional()
});
var FeedbackBody = external_exports.object({
  messageId: external_exports.string().min(1),
  sentiment: external_exports.enum(["up", "down"]).optional(),
  usefulness: external_exports.number().int().min(1).max(5).optional(),
  comment: external_exports.string().max(2e3).optional()
});
var BenchmarkRunBody = external_exports.object({
...
var FigureSchema = external_exports.looseObject({
  label: external_exports.string(),
  value: external_exports.number(),
  display: external_exports.string(),
  comparison: external_exports.string()
});
var SourceSchema = external_exports.looseObject({ name: external_exports.string(), freshness: external_exports.string(), role: external_exports.string().optional() });
var ChartSchema = external_exports.looseObject({
  id: external_exports.string(),
  title: external_exports.string(),
  /** Derived by the agent from the traces, so it cannot disagree with `data`. */
  kind: external_exports.string(),
  data: external_exports.array(external_exports.record(external_exports.string(), external_exports.unknown())),
  layout: external_exports.record(external_exports.string(), external_exports.unknown())
});
var StageSchema = external_exports.looseObject({
  id: external_exports.string(),
  name: external_exports.string(),
  kind: external_exports.string(),
  ...
  depth: external_exports.number().default(0),
  ...
  token_usage: external_exports.object({
    ...
    cachedReadTokens: external_exports.number().int().nonnegative().optional(),
    cacheWriteTokens: external_exports.number().int().nonnegative().optional(),
    cacheStatus: external_exports.enum(["used", "not-used", "unavailable"]),
    attempts: external_exports.number().int().positive(),
    totalMismatch: external_exports.boolean()
  }).optional()
});
var GenieSpaceSchema = external_exports.looseObject({ id: external_exports.string(), title: external_exports.string().default("") });
var ResourceCallSchema = external_exports.looseObject({
...
var TokenInvocationSchema = external_exports.object({
  invocationId: external_exports.string().min(1),
...
var TraceSchema = external_exports.looseObject({
  id: external_exports.string(),
  ...
  token_invocations: external_exports.array(TokenInvocationSchema).optional(),
  token_reconciliation: external_exports.object({
    attributedTokens: external_exports.number().int().nonnegative(),
    attributedCalls: external_exports.number().int().nonnegative(),
    overviewTokens: external_exports.number().int().nonnegative().optional(),
    coveragePercent: external_exports.number().nonnegative().optional(),
    unattributedTokens: external_exports.number().int().nonnegative().optional(),
    nestedAggregateTokens: external_exports.number().int().nonnegative(),
    mismatchCount: external_exports.number().int().nonnegative(),
    cachedReadTokens: external_exports.number().int().nonnegative().optional(),
    cacheCoveredInputTokens: external_exports.number().int().nonnegative().optional(),
    cacheHitPercent: external_exports.number().nonnegative().optional()
  }).optional()
});
var DerivationSchema = external_exports.looseObject({
  source: external_exports.string().default(""),
  metric: external_exports.string().default(""),
  window: external_exports.string().default(""),
  filter: external_exports.string().default("")
});
var DerivationEntrySchema = DerivationSchema.catch((ctx) => {
  console.warn(
    `[contract] An answer carried a provenance entry this app could not read, so that entry states nothing. Entry: ${JSON.stringify(ctx.value).slice(0, 200)}`
  );
...
var LiveAnswerSchema = external_exports.looseObject({
  ...
  derivation: external_exports.array(DerivationEntrySchema).default([]),
...
function keysOutsideShape(value, shape, prefix) {
  ...
  return Object.keys(value).filter((key) => !(key in shape)).map((key) => `${prefix}${key}`);
}
function undeclaredAnswerKeys(answer) {
...
var PlanStepSchema = external_exports.looseObject({
  id: external_exports.string().min(1),
  title: external_exports.string().min(1),
  description: external_exports.string().min(1),
  kind: external_exports.enum(["context", "definitions", "data", "synthesis"])
});
var AnalysisPlanSchema = external_exports.looseObject({
  id: external_exports.string().min(1),
  question: external_exports.string().min(1),
  summary: external_exports.string().min(1),
  steps: external_exports.array(PlanStepSchema),
  requires_approval: external_exports.boolean().default(true),
  uses_conversation_context: external_exports.boolean().default(false),
  uses_attachment_context: external_exports.boolean().default(false)
});
function undeclaredPlanKeys(plan) {
...
var ClarificationSchema = external_exports.looseObject({
  id: external_exports.string().min(1),
  question: external_exports.string().min(1),
...
var ALLOWED_ATTACHMENT_TYPES = /* @__PURE__ */ new Set(["txt", "md", "csv", "json"]);
var MAX_ATTACHMENT_BYTES = MAX_PDF_BYTES;
var MAX_ATTACHMENT_TEXT = 5e4;
var MAX_CONVERSATION_ATTACHMENT_TEXT = 8e4;
var parseAttachmentBody = (0, import_express.raw)({ type: "application/octet-stream", limit: MAX_ATTACHMENT_BYTES });
function parseBoundedAttachment(req, res, next) {
  parseAttachmentBody(req, res, (error) => {
    const type = error && typeof error === "object" && "type" in error ? error.type : void 0;
...
var PLAN_APPROVAL_MESSAGE = "Approved the proposed analysis plan.";
var SHARED_RUN_OWNER = "Another team member";
var RUNS_QUERY = `
  ...
  ORDER BY created_at DESC
  LIMIT 200`;
async function callerReadsEveryRun(store, email) {
  const { role } = await resolveRole(store, email);
  return opensAdminSurfaces(role);
}
async function callerReadsSharedConversations(store, email, rolesReady) {
  await rolesReady?.();
...
var TraceStageDetailSchema = StageSchema;
var TraceDetailSchema = TraceSchema.extend({ stages: external_exports.array(TraceStageDetailSchema) });
var ToolStageSchema = external_exports.looseObject({
  id: external_exports.string(),
  name: external_exports.string(),
  status: external_exports.enum(["complete", "running", "partial", "failed", "cancelled", "awaiting_approval"]),
...
var MlflowReferenceSchema = external_exports.looseObject({
  traceId: external_exports.string(),
  experimentId: external_exports.string().nullable(),
  url: external_exports.string().nullable()
});
var BenchmarkMetricsSchema = external_exports.looseObject({
  suiteId: external_exports.string().nullable(),
  passed: external_exports.number().nullable(),
...
var RunTraceSchema = external_exports.looseObject({
  runId: external_exports.string(),
  kind: external_exports.enum(["conversation", "benchmark"]),
  /** 'trace' when real stages were found; 'no-trace' when the run has none to show. */
  state: external_exports.enum(["trace", "no-trace"]),
  /** Whether the stored answer came from the agent or from the offline fallback. */
  mode: external_exports.enum(["live", "representative"]).nullable(),
  conversationId: external_exports.string().nullable(),
  createdAt: external_exports.string(),
  prompt: external_exports.string().nullable(),
  stakeholder: external_exports.string().nullable(),
  takeaway: external_exports.string(),
  narrative: external_exports.string(),
  sql: external_exports.string(),
  /** Canonical Plotly specs for the chart-building stage, when this answer had any. */
  charts: external_exports.array(ChartSchema).optional(),
  sources: external_exports.array(SourceSchema),
  /**
   ...
   */
  derivation: external_exports.array(DerivationSchema).optional(),
  trace: TraceDetailSchema.nullable(),
  /** Tool-tagged stages. The agent's own call counter is `trace.toolCalls`. */
  toolStages: external_exports.array(ToolStageSchema),
...
function withTokenAttribution(run, attribution) {
...
async function cancelTaggedWarehouseQueries(input) {
  ...
  try {
    ...
    console.warn("[cancel] Tagged warehouse queries could not be swept:", error.message);
    return {
      result: NO_WAREHOUSE_CANCELLATION,
...
function mlflowReference(traceId, experimentId) {
  ...
  const host = workspaceHost();
  const url = named && host ? `${host}/ml/experiments/${encodeURIComponent(named)}/traces?selectedEvaluationId=${encodeURIComponent(traceId)}` : null;
...
function toolStagesFromTrace(stages) {
  return stages.filter((stage) => stage.kind === "tool").map((stage) => ({
    id: stage.id,
    name: stage.name,
    status: stage.status,
...
function conversationRunTrace(row2, experimentId) {
  const identity = {
    ...
    kind: "conversation",
    conversationId: text9(row2.conversation_id),
    createdAt: timestamp(row2.created_at),
    prompt: text9(row2.prompt),
    stakeholder: text9(row2.stakeholder)
  };
  ...
  if (record.type === "clarification") {
    ...
    if (clarification.success && asked?.success) {
      ...
      const process3 = withoutUntracedTimeline(asked.data);
      return {
        ...identity,
        state: "trace",
        mode,
  ...
  const charts = external_exports.array(ChartSchema).safeParse(record.charts);
  const trace = answer.success ? TraceDetailSchema.safeParse(answer.data.trace) : TraceDetailSchema.safeParse(record.trace);
  if (!trace.success) {
    return runWithoutTrace(
      identity,
  ...
  const recorded2 = isMlflowTraceId(trace.data.id);
  const process2 = withoutUntracedTimeline(trace.data);
  return normalizeReaderAnswer({
    ...
    trace: process2,
    toolStages: recorded2 ? toolStagesFromTrace(trace.data.stages) : [],
    mlflow: recorded2 ? mlflowReference(trace.data.id, experimentId) : null,
    benchmark: null,
    note: !recorded2 ? "" : mode === "representative" ? "This run was answered offline from the representative dataset, so these are reference stages rather than a live agent run." : "",
    undeclaredKeys: answer.success ? undeclaredAnswerKeys(answer.data) : [],
    runtimeUsed
  });
}
function benchmarkRunTrace(row2) {
  const metrics = parseStoredJson(row2.metrics_json) ?? {};
  const identity = {
    runId: String(row2.id),
    kind: "benchmark",
    conversationId: null,
    createdAt: timestamp(row2.created_at),
    prompt: text9(metrics.prompt) ?? `Benchmark suite: ${text9(row2.suite_id) ?? "unknown"}`,
    stakeholder: text9(row2.user_email)
  };
  return {
    ...runWithoutTrace(
      identity,
      "A benchmark run records a per-case outcome for every case in the suite rather than one set of agent stages, so there is no single trace to walk. Each case carries its own MLflow trace id, open one of those, or a conversation run, to inspect a live trace."
    ),
    benchmark: {
      ...
      // depend on.
      ...metrics,
      suiteId: text9(metrics.suiteId) ?? text9(row2.suite_id),
...
var RUN_TRACE_BENCHMARK_QUERY = `
  ...
  FROM ${APP_SCHEMA}.benchmark_runs b
  WHERE b.id = $1`;
var PreflightStatus = external_exports.enum(["ok", "failed", "unverified"]);
function withGuidance(value) {
...
var PreflightRemedySchema = external_exports.preprocess(
  withGuidance,
...
var PreflightCheckSchema = external_exports.looseObject({
  id: external_exports.string(),
  kind: external_exports.string(),
  name: external_exports.string(),
  label: external_exports.string(),
  ...
  detail: external_exports.string(),
  checked_with: external_exports.string(),
   ...
   * reported".
   */
  content_at: external_exports.string().optional(),
  /**
   ...
   * refusal.
   */
  stopped: external_exports.enum(["refused", "unreachable", "unasked"]).optional(),
  /**
...
var PreflightConfigurationSchema = external_exports.looseObject({
  key: external_exports.string(),
  env_var: external_exports.string().default(""),
  value: external_exports.unknown().default(""),
  source: external_exports.string().default(""),
  mutability: external_exports.string().default(""),
  baked: external_exports.boolean().default(false),
  required: external_exports.boolean().default(false)
});
var PreflightReportSchema = external_exports.looseObject({
  checked_at: external_exports.string(),
  status: PreflightStatus,
  principal: external_exports.string(),
  principal_resolved: external_exports.boolean(),
  table_source: external_exports.string(),
  ...
  build_sha: external_exports.string().default(""),
  configuration: external_exports.array(PreflightConfigurationSchema).default([]),
  checks: external_exports.array(PreflightCheckSchema),
  assumptions: external_exports.array(external_exports.string())
});
function candidateAcknowledgement(value) {
  const custom = value && typeof value === "object" ? value.custom_outputs : void 0;
  ...
  return {
    ...
    echoed: Array.isArray(echoed) ? echoed : []
  };
}
function extractPreflightReport(value) {
  ...
  if (custom && typeof custom === "object") {
    const parsed = PreflightReportSchema.safeParse(custom.preflight);
...
function extractConfigurationReport(value) {
  ...
  const custom = record.custom_outputs && typeof record.custom_outputs === "object" ? record.custom_outputs : null;
  const nestedReport = custom?.preflight && typeof custom.preflight === "object" ? custom.preflight : null;
...
function agentEndpointCheck(endpointName, outcome) {
  return {
    id: "agent-endpoint",
    ...
    duration_ms: 0,
    error: outcome.error ?? "",
    remedy: outcome.status === "ok" || !endpointName ? null : outcome.remedy ?? {
      kind: "cli",
      statement: `databricks permissions update serving-endpoints ${endpointName} --json '{"access_control_list":[{"service_principal_name":"${appPrincipal()}","permission_level":"CAN_QUERY"}]}'`,
...
var DEVELOPMENT_IDENTITY = "local-development@app.invalid";
function isDeployed() {
  return true;
}
var IdentityUnavailableError = class extends Error {
  constructor() {
    super(
      "This request carries no end-user identity. Databricks Apps sets x-forwarded-email on authenticated traffic, so its absence means there is no user to act as and no rows that may be read or written."
    );
...
var IDENTITY_OPTIONAL_ROUTES = /* @__PURE__ */ new Set([
  "/api/preflight",
  "/api/storage",
  "/api/settings",
...
function requireIdentity(req, res, next) {
  ...
  try {
    ...
    console.error(
      `[identity] REFUSED ${req.method} ${req.path}: no x-forwarded-email on the request, so there is no user to scope conversations, attachments, feedback or benchmark runs to. Serving no data rather than guessing an owner. Expected for non-interactive calls; if a signed-in browser sees this, the app is behind a proxy path that drops the header.`
    );
    res.status(401).json({
      error: "identity_unavailable",
      detail: "This request has no signed-in user. The app scopes every conversation, document and benchmark run to the person who created it, and will not fall back to another identity."
    });
    return;
  }
  next();
}
var SHARED_CONVERSATION_RAIL_ENV = "PLAYER_INSIGHTS_SHARED_CONVERSATION_RAIL";
function resolveSharedConversationRail(raw2) {
...
var sharedRail = { shared: false, raw: "", reason: "unset" };
function sharedConversationRail() {
  return sharedRail.shared;
}
function announceSharedConversationRail(resolution) {
  sharedRail = resolution;
  if (resolution.reason === "unrecognised") {
    console.error(
      `[rail] ${SHARED_CONVERSATION_RAIL_ENV} is set to ${JSON.stringify(resolution.raw)}, which is not a value this app recognises, so it has been IGNORED and the rail stays scoped to each user. The only value that turns sharing on is "true". Nothing is broken and nothing is exposed: but if a shared rail was intended, it is not on.`
    );
    return;
  }
  if (resolution.shared) {
    console.warn(
      `[rail] SHARED CONVERSATION RAIL IS ON (${SHARED_CONVERSATION_RAIL_ENV}=${JSON.stringify(resolution.raw)}). Administrators and super administrators can see and open every user's conversations; consumers remain strictly scoped to their own. This is a deliberate setting for a shared evaluation workspace and it is not the default. Deleting, asking and uploading remain scoped to the owner.`
    );
    return;
  }
  console.log(
    `[rail] Conversations are scoped to each user (${SHARED_CONVERSATION_RAIL_ENV} ${resolution.reason === "unset" ? "is unset" : `= ${JSON.stringify(resolution.raw)}`}).`
  );
}
async function settleSharedConversationRail(appkit) {
  ...
  console.warn(
    `[rail] This deploy carried ${SHARED_CONVERSATION_RAIL_ENV}=${JSON.stringify(preserved.authored ?? "")} because Deploy-from-Git replaces app.yaml with the public artifact's copy, which cannot state a deployment's own decision. The recorded decision is ${JSON.stringify(preserved.value ?? "")}, so that is what the rail uses. Release the bundle to change it; a Git deploy never records one.`
  );
  announceSharedConversationRail(resolution);
}
var CONVERSATION_RAIL_LIMIT = 100;
var DEFAULT_CONVERSATION_MESSAGE_LIMIT = 50;
var MAX_CONVERSATION_MESSAGE_LIMIT = 100;
function encodeConversationMessageCursor(cursor) {
  return Buffer.from(JSON.stringify([cursor.createdAt, cursor.id]), "utf8").toString("base64url");
}
function decodeConversationMessageCursor(value) {
  try {
    const decoded = JSON.parse(Buffer.from(value, "base64url").toString("utf8"));
    if (!Array.isArray(decoded) || decoded.length !== 2) return null;
    const createdAt = decoded[0];
    const id = decoded[1];
...
var CONVERSATION_LIST_COLUMNS = "c.id, c.title, c.updated_at, c.user_email, verdict.status, verdict.truncated, verdict.duration_ms, persona.persona_id, persona.persona_name, persona.recorded_at AS persona_recorded_at";
function conversationListQuery(email, readsShared) {
  return readsShared ? {
...
function conversationMessagesQuery(conversationId, email, readsShared, page) {
  ...
  if (page) {
    ...
    const params = page.cursor ? [conversationId, email, page.cursor.createdAt, page.cursor.id, page.limit + 1] : [conversationId, email, page.limit + 1];
    return {
      sql: `${select}
...
var CONVERSATION_RUN_STATUS_QUERY = `SELECT run_id, state, created_at, updated_at, terminal_code, terminal_message_id
  FROM ${APP_SCHEMA}.runs
  WHERE conversation_id = $1 AND ($3 OR user_email = $2)
  ORDER BY created_at DESC
  LIMIT 1`;
function isEndpointError(record) {
  const status = record.status ?? record.statusCode;
  return Boolean(record.error) || typeof record.error_code === "string" || typeof status === "number" && status >= 400;
}
  ...
  for (const key of ["content", "output", "messages", "choices"]) {
    ...
    if (Array.isArray(items)) {
      for (const item of items) {
        const text10 = extractLiveText(item);
        if (text10) return text10;
      }
    }
  }
  for (const key of ["message", "data", "response", "result", "body"]) {
    if (record[key]) {
      const text10 = extractLiveText(record[key]);
      if (text10) return text10;
    }
  }
  return null;
...
function buildServingHistory(rows) {
  return rows.filter(
    (row2) => (row2.role === "user" || row2.role === "assistant") && typeof row2.content === "string"
  ).slice(-12).map((row2) => {
    if (row2.role === "user") return { role: row2.role, content: row2.content };
    let response = row2.response_json;
    ...
    if (response && typeof response === "object") {
      ...
      if (typeof record.takeaway === "string") {
        const narrative = typeof record.narrative === "string" ? record.narrative : row2.content;
        return {
          role: row2.role,
...
${narrative}`.slice(0, 4e3)
    ...
    return { role: row2.role, content: row2.content.slice(0, 4e3) };
  });
}
...
function analyticalExecution(req, signedInAs) {
...
function servingInvocationPath(endpointName) {
  return `/serving-endpoints/${encodeURIComponent(endpointName)}/invocations`;
}
var workspaceClient2;
function appEntitlementLookup() {
  return entitlementLookupVia(async (path, query) => {
    const { WorkspaceClient } = await import("./vendor-databricks-sdk-experimental.mjs");
    if (!workspaceClient2) workspaceClient2 = new WorkspaceClient({});
...
var appWarehouseWarmup = createWarehouseWarmup({
  warehouseId: appWarehouseId,
  transport: appWarmupTransport()
});
var genieWarehouseWarmup = createGenieWarehouseWarmup();
function warmWarehouseForArrival(warmup) {
  warmup.warm().then((outcome) => {
    ...
    console.warn(`[warmup] The warm-up threw, which it should not: ${error.message}`);
  });
}
function warmGenieWarehousesForArrival(req) {
  const token = executionToken(req);
  const host = workspaceHost();
  ...
  genieWarehouseWarmup.warm({
    ...
    console.warn(`[warmup] The adopted Genie warm-up threw, which it should not: ${error.message}`);
  });
}
function createServingTransport(resolveClient) {
  return async ({ path, payload, onStage, userToken, signal }) => {
    ...
    const invoke = (asStream) => client.request({
      path,
      method: "POST",
      headers: new Headers({
...
var workspaceServingTransport = createServingTransport(async (userToken) => {
  if (userToken) {
    const { WorkspaceClient } = await import("./vendor-databricks-sdk-experimental.mjs");
  ...
  if (!workspaceClient2) {
    const { WorkspaceClient } = await import("./vendor-databricks-sdk-experimental.mjs");
    workspaceClient2 = new WorkspaceClient({});
  }
  return workspaceClient2.apiClient;
});
function buildAskServingBody({
  history,
  prompt,
  conversationId,
  approvedPlanId,
  ...
  if (executePlan !== void 0) custom_inputs.execute_plan = executePlan;
...
var SERVING_INVOKE_TIMEOUT_MS = 24e4;
...
function readStageTitle(stage) {
  const name = stage.name;
  return typeof name === "string" && name.trim() ? name.trim() : "an unnamed step";
}
function isRunningStage(stage) {
  return stage.status === "running";
}
function agentEndpointEvidence(error, context) {
...
var SERVICE_PRINCIPAL_FALLBACK_CAVEAT = "Data access scope: this answer used the application’s Unity Catalog grants, which may include data outside the signed-in account’s direct access.";
var AuthorizationRefused = class extends Error {
  code;
  httpStatus;
  /**
   ...
   */
  providerStatus;
  constructor(code, detail, providerStatus) {
    super(detail);
...
function rejectionStatus(error) {
  ...
  const message = error instanceof Error ? error.message : text9(error) ?? "";
...
async function invokeServingAsUser(appkit, payload, userToken, onStage, timeoutMs = SERVING_INVOKE_TIMEOUT_MS, endpointName, signal) {
  try {
    ...
    console.warn(
      `[identity] The endpoint refused this request under the signed-in user's own credential (${code}): ${detail}. Nothing was retried: the app's service principal is not an identity any analytical request may be executed under.`
    );
    throw new AuthorizationRefused(code, detail, carriedStatus(error));
  }
}
var endpointMetadataFlights = /* @__PURE__ */ new WeakMap();
...
async function safeQuery(appkit, sql, params = []) {
  const read = await readStored(appkit, describeSql(sql), sql, params);
  return { rows: read.available ? read.rows : [] };
}
async function respondWithStored(appkit, res, route, sql, params) {
...
var MIGRATIONS = buildMigrations(schemaStatements);
async function applySchema(appkit) {
...
var MIGRATE_ON_BOOT_ENV = "PLAYER_INSIGHTS_MIGRATE_ON_BOOT";
function bootMigrationMode(env = process.env) {
  return (env[MIGRATE_ON_BOOT_ENV] ?? "").trim().toLowerCase() === "verify" ? "verify" : "apply";
}
async function prepareStore(appkit) {
  markSchemaPending(true);
  try {
    await applySchema(appkit);
  } catch (error) {
    console.error(
      `[lakebase] SCHEMA SETUP THREW rather than reporting: ${error.message}. The app is serving already, and every read of the store will report itself unavailable until this is fixed.`
    );
  } finally {
  ...
  try {
    ...
    console.warn(
      `[lakebase] The conversation-title repair did not run: ${error.message}. Titles cut to 80 characters by an older version stay cut; nothing else is affected.`
    );
  }
}
function setupInsightsRoutes(appkit, options = {}) {
  ...
  if (isDeployed()) {
    ...
    console.warn(
      `[identity] DEVELOPMENT MODE: requests without x-forwarded-email act as ${DEVELOPMENT_IDENTITY}, and rows written now are owned by that address. This path does not exist when NODE_ENV=production, which is what app.yaml runs.`
    );
  }
  appkit.server.extend((app) => {
    ...
    app.post("/api/app-user-api-scopes", async (req, res) => {
      ...
      const outcome = await allowAstrolabeUserApiScopes({
        ...
        appName,
        userToken,
        additionalScopes: requestedScope ? [requestedScope] : void 0
      });
    ...
    app.post("/api/access-mode", (req, res) => {
      ...
      if (!isAccessMode(requested) || requested === "user-verified") {
        res.status(400).json({
          ...
          message: "Choose `service-principal` or `skipped`. Verified access is established by running the checks at /api/access-verification, not by asking for it."
        });
        return;
      }
      ...
      const decision = declareAccessMode(
        email,
        requested,
        requested === "skipped" ? "The user skipped the access gate. Nothing was checked under their token. Who runs questions is set by the deployment, not by this choice." : "The user chose service-principal mode at the gate: own access was not verified. Who runs questions is set by the deployment, not by this choice."
      );
    ...
    app.post("/api/access-verification", async (req, res) => {
      ...
      const servingChecked = [];
      const host = workspaceHost();
      const missing = !warehouseId ? {
        kind: "not-configured",
        summary: "This app has no SQL warehouse attached, so there is nowhere to run a statement as you. Attach one to the app and restart it. Nothing was checked, and this says nothing about your permissions.",
        layer: "app configuration"
      } : !host ? {
        kind: "not-configured",
        summary: "The app does not know its own workspace URL (DATABRICKS_HOST is unset), so it cannot call the SQL API as you. This is an app environment problem and is unrelated to your permissions.",
      ...
      if (!isVerified(outcome)) {
        ...
        console.warn(
          `[access] ${email} not verified: ${outcome.blocked ? outcome.blocked.kind : `${outcome.denied} denied, ${outcome.errored} unknown, ${genieDenied} Genie space(s) refused`}`
        );
    ...
    app.delete("/api/conversations/:id", async (req, res) => {
      ...
      if (!ownership.available) {
        ...
        res.status(503).json({
          ...
          conversationId,
          message: "This conversation could not be deleted right now, because the store could not confirm who owns it. Nothing was removed. Try again shortly."
        });
        return;
      }
      ...
      try {
        ...
        console.warn(`[lakebase] Conversation ${conversationId} could not be deleted:`, error.message);
        res.status(503).json({
          error: "conversation_delete_failed",
          conversationId,
          message: "This conversation could not be fully deleted right now. It is still in the rail, and deleting it again will remove whatever is left. Try again shortly."
        });
      }
    });
    app.get("/api/conversations/:id/messages", async (req, res) => {
      ...
      const paged = rawLimit !== void 0 || rawCursor !== void 0;
      if (rawLimit !== void 0 && typeof rawLimit !== "string" || rawCursor !== void 0 && typeof rawCursor !== "string") {
        res.status(400).json({ error: "invalid_message_page", message: "Message pagination parameters are invalid." });
        return;
      }
      const limit = rawLimit === void 0 ? DEFAULT_CONVERSATION_MESSAGE_LIMIT : Number(rawLimit);
      const cursor = rawCursor === void 0 ? null : decodeConversationMessageCursor(rawCursor);
      if (!Number.isInteger(limit) || limit < 1 || limit > MAX_CONVERSATION_MESSAGE_LIMIT || rawCursor !== void 0 && !cursor) {
        res.status(400).json({ error: "invalid_message_page", message: "Message pagination parameters are invalid." });
        return;
      }
      ...
      const nextCursor = hasMore && (typeof createdAt === "string" || createdAt instanceof Date) && typeof oldestId === "string" ? encodeConversationMessageCursor({
    ...
    app.get("/api/conversations/:id/attachments", async (req, res) => {
      ...
      if (!read.available) {
        ...
        res.status(503).json({
          ...
          message: "The documents attached to this conversation could not be read just now, so this is not a list of them. Anything already attached is still attached. Try again shortly."
        });
        return;
      }
    ...
    app.post("/api/conversations/:id/attachments", parseBoundedAttachment, async (req, res) => {
      ...
      try {
        ...
        console.warn("[lakebase] Attachment could not be stored:", error.message);
        res.status(503).json({
    ...
    app.delete("/api/conversations/:conversationId/attachments/:attachmentId", async (req, res) => {
      ...
      try {
        ...
        console.warn("[lakebase] Attachment could not be removed:", error.message);
        res.status(503).json({
          error: "attachment_delete_failed",
          conversationId,
          attachmentId,
          message: "This report could not be removed right now, so it is still attached and will still reach the agent. Try again shortly."
        });
      }
    });
    app.delete("/api/conversations/:conversationId/attachments", async (req, res) => {
      ...
      try {
        ...
        console.warn("[lakebase] Attachments could not be cleared:", error.message);
        res.status(503).json({
    ...
    app.post("/api/insights/ask", async (req, res) => {
      ...
      try {
        ...
        console.warn(`[app-budget] Admission status was unavailable; allowing the Ask: ${error.message}`);
      }
...
${String(row2.extracted_text)}`).join("\n\n").slice(0, MAX_CONVERSATION_ATTACHMENT_TEXT);
      ...
      if (admission.kind === "replay") {
        ...
        console.warn(
          `[run-ledger] Run ${admission.run.runId} was asked again and has no answer to replay (${replay.kind}). Answering with ${code} rather than running it a second time.`
        );
      ...
      try {
        ...
        try {
          ...
          if (refused) {
            console.warn(
              `[identity] Agent refused request ${identity.correlationId} with ${refused.code}${refused.execution_identity ? `, executing as ${refused.execution_identity.mode}` : ""}. Returning unavailable rather than an answer.`
            );
          ...
          if (isRunCancelledError(error)) {
            console.info(
              `[serving] Run ${error.runId} was cancelled. App-side consumption stopped and no replacement invocation was started; the current Model Serving invocation may still finish server-side.`
            );
            reply.status(409).json({
              type: "cancelled",
              state: "CANCELLED",
            ...
            reply.status(error.httpStatus).json(
              unavailableResult({
                ...
                evidence: {
                  ...
                  ...error.providerStatus === void 0 ? {} : { status: error.providerStatus },
        ...
        void readBenchmarkSettings(appkit).then(async (settings) => {
          ...
          console.warn("[eval-live-scores] Sampled Ask scoring was not started:", error.message);
        });
        reply.json({
          type: "answer",
          ...disclosed,
    ...
    app.get("/api/storage", (_req, res) => {
      ...
      res.status(health2.state === "unavailable" ? 503 : 200).json(health2);
    });
    const traceRequestId = (req) => req.get("x-request-id") ?? `req-${crypto.randomUUID()}`;
    app.get("/api/runs/:id/trace", async (req, res) => {
      ...
      try {
        ...
        console.warn("[lakebase] Run trace could not be read:", error.message);
    ...
    app.post("/api/feedback", async (req, res) => {
      ...
      if (!written.available) {
        ...
        res.status(503).json({
          ...
          message: "This rating was not recorded, because the store did not accept it. Nothing was saved, so it is worth giving again shortly rather than assuming it landed."
        });
        return;
      }
    ...
    app.post("/api/benchmarks/run", async (req, res) => {
      ...
      if (!agentEndpoint) {
        res.status(503).json({
          ...
          message: "DATABRICKS_SERVING_ENDPOINT_NAME is not set, so there is no endpoint to benchmark. No run was started: scoring the offline fallback would report figures for an agent that never ran."
        });
        return;
      }
      ...
      const started = await startBenchmarkRun({
        ...
        askAgent: async (request) => {
          ...
          try {
            raw2 = identity.token ? await invokeServingAsUser(
              ...
              agentEndpoint
            ) : await invokeServing(appkit, payload, void 0, SERVING_INVOKE_TIMEOUT_MS, void 0, agentEndpoint);
          } catch (error) {
          ...
          const refused = readAgentRefusal(raw2, { requestId: identity.correlationId });
          if (refused) {
            return {
              type: "refused",
              code: refused.code,
        ...
        judge: {
          ...
          // passes.
          invoke: (payload) => transport({
            path: servingInvocationPath(judgeEndpoint),
            payload,
            userToken: identity.token || void 0
          }),
          judgeEndpoint
        },
