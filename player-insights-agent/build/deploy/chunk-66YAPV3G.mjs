...
function buildUserMonitoringPage(input) {
  ...
  const runs = new Map(input.runs.map((row2) => [row2.email.toLowerCase(), row2]));
  const activity = new Map(input.activity.map((row2) => [row2.email.toLowerCase(), row2]));
  const interactions = new Map((input.interactions ?? []).map((row2) => [row2.email.toLowerCase(), row2]));
  const emails = /* @__PURE__ */ new Set([...profiles.keys(), ...runs.keys(), ...activity.keys(), ...interactions.keys()]);
  ...
  const authorizedRows = [...emails].filter((email) => email.includes("@")).map((email) => {
    ...
    const activeTimes = [interaction?.lastActive ?? "", run?.lastActive ?? "", active?.lastActive ?? ""].filter(Boolean).sort();
...
function appCostSummary(payload, unit = "USD") {
  ...
  const included = payload.tiles.filter(
    (tile) => deploymentAttribution(tile) && (unit === "DBU" ? typeof tile.dbus === "number" && Number.isFinite(tile.dbus) : tile.quality !== "unknown" && typeof tile.amount === "number" && Number.isFinite(tile.amount) && (tile.pricing?.match === void 0 || tile.pricing.match === "priced" || tile.pricing.match === "none"))
  );
  ...
  const activeMissing = payload.tiles.some(
    (tile) => Boolean(tile.resourceId.trim()) && (tile.attribution !== "deployment" || (unit === "DBU" ? typeof tile.dbus !== "number" || !Number.isFinite(tile.dbus) : typeof tile.amount !== "number" || !Number.isFinite(tile.amount)))
  );
...
function readFoundationBillingRows(data) {
  ...
  return {
    ...
    complete: pricing.match === "priced" && unmapped === 0 || pricing.match === "none" && finite(billingRows) === 0 && allRequests === 0
  };
}
function foundationCostTile(ids, result, reason = "") {
  ...
  return {
    ...
    unavailable: reason || (!ids.foundationModel ? "Configured foundation model unavailable." : result?.pricing.match === "partial" ? "Foundation-model request or price coverage is partial; spend is withheld." : "Foundation-model billing could not be attributed."),
    remedy: "",
    note: "",
    evidence: {
      billingRows: null,
...
async function queryMeasurement(appkit, req, period, now) {
  ...
  } = await import("./ops-routes-CE62SHUH.mjs");
  ...
  const built = (await import("./ops-billing-AS2A7UK4.mjs")).buildCostStatement(resolved.ids, range);
