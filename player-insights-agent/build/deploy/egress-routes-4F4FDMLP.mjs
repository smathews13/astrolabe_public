...
// server/lib/egress-classification.ts
var CLASSIFY_TABLE_LIMIT = 12;
var NO_TOKEN_REASON = "Not checked. This session has no forwarded sign-in token, so the catalog was not asked. It is not asked with the app's own identity, deliberately.";
var NO_WAREHOUSE_REASON = "Not checked. This deployment has no SQL warehouse configured, so the catalog could not be asked.";
...
async function classifyTables(run, tables, options = {}) {
  ...
  for (const table of considered) {
    ...
    const group = byCatalog.get(parts.catalog) ?? { schemas: /* @__PURE__ */ new Set(), tables: /* @__PURE__ */ new Set() };
  ...
  for (const [catalog, group] of byCatalog) {
    ...
    if (!answered) {
      ...
      console.warn(
        `[egress] The catalog could not be asked about ${catalog}: ${refusal?.message ?? "no message"}. Every table in it is reported as not checked, which is what it is. It is NOT reported as carrying no personal data.`
      );
    }
  ...
  const classifications = considered.map((table) => {
    if (unusable.includes(table)) {
      return notChecked(
        table,
        "Not checked. This name is not a plain three-part Unity Catalog name, so the catalog was not asked about it."
      );
    }
    const parts = nameParts(table);
    const entry = parts ? gathered.get(`${parts.catalog}\0${key(parts.schema, parts.table)}`) : void 0;
    ...
    const state = columns.length > 0 || entry.rowFilter ? "classified" : "not-classified";
    return { table, state, columns, rowFilter: entry.rowFilter, notChecked: "" };
  });
  return { classifications, blocked: "" };
}
function notChecked(table, reason) {
  return { table, state: "not-checked", columns: [], rowFilter: null, notChecked: reason };
}
function columnsFrom(entry) {
  const names = /* @__PURE__ */ new Set([...entry.tags.keys(), ...entry.masked]);
...
// server/routes/egress-routes.ts
var EGRESS_ADMIN_ROUTES = [
  "/api/egress/admin/controls",
  "/api/egress/admin/events",
  "/api/egress/admin/classification"
];
var EGRESS_OPEN_ROUTES = ["/api/egress/controls", "/api/egress/events"];
function readerRunner(req) {
...
var ReportBody = external_exports.object({
  channel: external_exports.string().trim().max(64),
  surface: external_exports.string().trim().max(64).default(""),
  runId: external_exports.string().trim().max(128).nullish(),
  conversationId: external_exports.string().trim().max(128).nullish(),
  itemCount: external_exports.number().finite().nullish()
}).strict();
var ControlBody = external_exports.object({
  channel: external_exports.string().trim().max(64),
  allowed: external_exports.boolean()
}).strict();
var EventsQuery = external_exports.object({
  limit: external_exports.coerce.number().int().min(1).max(50).default(20),
  cursor: external_exports.string().trim().max(512).optional()
}).strict();
function setupEgressRoutes(appkit, deps) {
  if (typeof deps?.isAdminRoute !== "function") {
    console.error(
      "[egress] NOT REGISTERED: no admin-route predicate was supplied, so there is no way to confirm the admin paths are guarded. They serve what every person has exported. Pass isAdminRoute."
    );
    return;
  }
  ...
  if (uncovered.length > 0) {
    console.error(
      `[egress] NOT REGISTERED: the admin guard does not cover ${uncovered.join(", ")}. Add '/api/egress/admin' to ADMIN_ROUTE_PREFIXES in lib/admin-roles.ts. Registering these unguarded would serve the record of what everyone has exported to any signed-in reader.`
    );
    return;
  }
  ...
  if (guarded.length > 0) {
    console.error(
      `[egress] NOT REGISTERED: the admin guard covers ${guarded.join(", ")}, which have to stay open to every signed-in reader. A consumer refused on those cannot report an export, so the record would quietly narrow to administrators only while continuing to look complete.`
    );
    return;
  }
  ...
  appkit.server.extend((app) => {
    app.get("/api/egress/controls", async (_req, res) => {
    ...
    app.post("/api/egress/events", async (req, res) => {
    ...
    app.get("/api/egress/admin/events", async (req, res) => {
    ...
    app.put("/api/egress/admin/controls", async (req, res) => {
      ...
      try {
        ...
        console.error(`[egress] A control could not be saved: ${error.message}`);
        res.status(503).json({
          error: "egress_store_unavailable",
          detail: "The control was not saved. Reporting success here would leave a switch on screen that no reload would keep."
        });
        return;
      }
