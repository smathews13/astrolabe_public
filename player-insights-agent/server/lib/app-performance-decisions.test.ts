...
const MESSAGE_SOURCE = readFileSync(new URL('../../client/src/conversation-messages.ts', import.meta.url), 'utf8');
const HOME_SOURCE = readFileSync(new URL('../../client/src/HomePage.tsx', import.meta.url), 'utf8');
const ANSWER_LOADER_SOURCE = readFileSync(new URL('../../client/src/stored-answer-loader.ts', import.meta.url), 'utf8');
const VITE_SOURCE = readFileSync(new URL('../../client/vite.config.ts', import.meta.url), 'utf8');
...
describe('the app performance decision log', () => {
  ...
  it('keeps final frontend delivery and zero-wait answer decisions aligned', () => {
    ...
    const ask = HOME_SOURCE.slice(
      HOME_SOURCE.indexOf('async function ask'),
      HOME_SOURCE.indexOf('function startNewConversation')
    );
    ...
    expect(DECISIONS).toContain('Initial JS `2,234,552 / 709,813 gzip`');
    expect(DECISIONS).toContain('lazy stored/final-answer graph `75,402 / 25,222`');
  });
});
