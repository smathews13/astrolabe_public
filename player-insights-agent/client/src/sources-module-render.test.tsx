...
import { EntityParts } from './DataEntityLinks';
...
describe('the source list', () => {
  it('names the module and source as a heading plus one bullet', () => {
    const markup = renderToStaticMarkup(
      <SourcesModule sources={[{ name: NAME, freshness: FRESHNESS }]} caveats={[]} />
    );
  ...
  it('gives each source its own bullet instead of one wrapping paragraph', () => {
    const markup = renderToStaticMarkup(
      <SourcesModule sources={QUERIED.map((name) => ({ name, freshness: FRESHNESS }))} caveats={[]} />
    );
    const rendered = text(markup);
...
describe('the row that names a source', () => {
  it('uses the shared entity parts and marks the table segment for emphasis', () => {
    const parts = renderToStaticMarkup(
      <EntityParts text="player_insights.gold_title_daily_summary" entity={QUERIED[0]} />
    );
    expect(parts).toContain('class="entity-token entity-schema" data-entity-part="schema"');
    expect(parts).toContain('class="entity-token entity-table" data-entity-part="table"');

    const source = renderToStaticMarkup(
      <SourcesModule sources={[{ name: QUERIED[0], freshness: FRESHNESS }]} caveats={[]} />
    );
    expect(source).toContain('class="entity-token entity-catalog source-name-qualifier" data-entity-part="catalog"');
    expect(source).toContain('class="entity-token entity-schema source-name-qualifier" data-entity-part="schema"');
    expect(source).toContain('class="entity-token entity-table source-name-short" data-entity-part="table"');
  });
  ...
  it('spells the whole three-part name, unbroken, in the order the run read them', () => {
    ...
    const rendered = text(
      renderToStaticMarkup(
        <SourcesModule sources={[NAME, ...QUERIED].map((name) => ({ name, freshness: FRESHNESS }))} caveats={[]} />
      )
    );
  ...
  it('says it does not know the role rather than guessing at it', () => {
    ...
    const rendered = text(
      renderToStaticMarkup(
        <SourcesModule
          sources={[
            { name: NAME, freshness: FRESHNESS },
            { name: QUERIED[0], freshness: FRESHNESS },
          ]}
          caveats={[]}
        />
      )
