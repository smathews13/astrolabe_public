...
/**
 * How many connections the counts describe, which is the rows on screen.
 *
 * The six status counts and nothing else. `drifted` and `pending` come off the
 * markers, a second axis a row carries alongside its badge, so adding them here
 * would count a drifted reachable row twice and put this number above the rows.
 */
function connectionTotal(counts: ConnectionCounts): number {
  return (
    counts.reachable
    + counts.blocked
    + counts.refused
    + counts.unreachable
    + counts.notChecked
    + counts.nothingToReach
  );
}

/**
 * Which of the three things the status block is saying.
 *
 * Decided beside the sentence so the icon and the words cannot come apart: the
 * tick and the triangle are the two most readable marks on the page, and a green
 * tick over a red sentence is worse than either alone.
 */
export type ConnectionsHeadlineState = 'unread' | 'clear' | 'unreachable' | 'blocked';

/**
 * `unreachable` outranks a clear line and is outranked by `blocked`.
 *
 * "Nothing is blocked" stays true of a refusal: the workspace answered, and the
 * answer was about a permission rather than about the dependency. It is NOT
 * honest over a dependency nothing could reach. The claim is that no fault was
 * found, and a call that timed out found neither a fault nor its absence, so the
 * tick would be certifying a gap. Blocked still leads, because a check that ran
 * to a failure is the harder fact of the two.
 */
export function connectionsHeadlineState(counts: ConnectionCounts): ConnectionsHeadlineState {
  if (connectionTotal(counts) === 0) return 'unread';
  if (counts.blocked > 0) return 'blocked';
  return counts.unreachable > 0 ? 'unreachable' : 'clear';
}

/**
 * The headline over the CONNECTIONS, as opposed to over the checks.
 *
 * `checksHeadline` in `preflight.ts` summarises checks, and it is right about
 * them: on this deployment twenty-four checks answered and none failed, so it
 * says "Every dependency is reachable". The trouble is where that sentence was
 * being printed -- at the top of a list of nineteen CONNECTIONS, eight of which
 * have no verdict at all. So the page's own headline is computed over the rows
 * the page draws, and `checksHeadline` is kept only for the case where there is
 * no payload and the checks are all there is.
 *
 * TWO SENTENCES, NOT SIX, which is the design's headline and is also the honest
 * one. This function used to pick between five readings of the same counts --
 * "N of M connections are blocked", "One connection is not using what it was
 * configured with", "N of M connections answered", "Every connection answered,
 * and a change is waiting on a release" -- so the first line of the page was a
 * different sentence on almost every deployment, and four of the five restated
 * something the count line directly below already said in fewer words. A reader
 * who reads the top and stops needs one thing from here: whether anything is
 * blocked. The drift, the not-checked and the pending counts are facts, they are
 * on the next line, and they each have a group of their own further down.
 *
 * The narrowing is what earns the tick. "Nothing is blocked" is a claim the
 * counts can settle outright, where "Every connection answered" was one this
 * page could not honestly make while eight rows carried no verdict at all. The
 * one exception is an unreachable row -- see `connectionsHeadlineState`, which is
 * where that carve-out is decided.
 */
export function connectionsHeadline(counts: ConnectionCounts): string {
  const state = connectionsHeadlineState(counts);
  if (state === 'unread') return 'No connection has been read yet';
  if (state === 'blocked') return `${counts.blocked} blocked`;
  // Not "nothing is blocked, N unreachable": the point of narrowing this line to
  // one claim was that a reader who reads the top and stops gets one fact, and the
  // fact worth having here is the one that needs somebody.
  if (state === 'unreachable') return `${counts.unreachable} unreachable`;
  return 'Nothing is blocked';
}

/**
 * Whether the headline has earned its tick.
 *
 * Separate from the sentence so that neither can drift from the other. It used
 * to be far stricter than the sentence beside it -- no drift, nothing pending,
 * every row verdicted -- because the sentence claimed everything had answered,
 * and a tick beside that claim would have certified something the page could not
 * establish. The sentence now claims only that nothing is blocked, which these
 * counts settle, so the two agree on one condition instead of disagreeing on
 * five.
 */
export function connectionsSettled(counts: ConnectionCounts): boolean {
  return connectionsHeadlineState(counts) === 'clear';
}
