...
describe('the composer form', () => {
  // The composer is Ask PIA's, which App.tsx held until the pages were split out.
  const COMPOSER_SOURCE = readFileSync(new URL('./HomePage.tsx', import.meta.url), 'utf8');

  it('spreads the shared opt-out onto the form, not a hand-copied list', () => {
    expect(COMPOSER_SOURCE).toContain('{...PASSWORD_MANAGER_OPT_OUT}');
  });
  ...
  it('leaves Return-to-send wired to the textarea', () => {
    expect(COMPOSER_SOURCE).toContain('if (!submitsOnEnter(event)) return;');
  });
});
