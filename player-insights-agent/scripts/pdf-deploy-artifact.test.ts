...
describe('PDF deploy artifact', () => {
  it('keeps unpdf out of the normal startup graph and ships no install manifest', () => {
    ...
    const serverModules = readdirSync(deploy)
      .filter((name) => name.endsWith('.mjs') && name !== path.basename(workerArtifact))
      .map((name) => readFileSync(path.join(deploy, name), 'utf8'));

    expect(server).not.toContain('vendor-unpdf.mjs');
    expect(server).not.toContain('node_modules/unpdf/');
    expect(serverModules.some((source) => source.includes('pdf-text-worker.mjs'))).toBe(true);
